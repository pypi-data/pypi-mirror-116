...
SENT_EMAILS=[]
EMAIL_BLACKLIST=KeakL()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",Keaks(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=Keakt,from_name=Keakt,smtp_host=Keakt,smtp_user=Keakt,smtp_pass=Keakt,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if KeakP(recipients,Keakz)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=Keakt):
 ...
 result["From"]=formataddr((Keaks(Header(from_name,"utf-8")),from_email))
 return result
