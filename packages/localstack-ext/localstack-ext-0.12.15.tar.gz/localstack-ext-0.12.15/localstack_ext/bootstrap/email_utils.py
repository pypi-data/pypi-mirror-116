...
SENT_EMAILS=[]
EMAIL_BLACKLIST=iSNeM()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",iSNeJ(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=iSNeO,from_name=iSNeO,smtp_host=iSNeO,smtp_user=iSNeO,smtp_pass=iSNeO,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if iSNeg(recipients,iSNel)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=iSNeO):
 ...
 result["From"]=formataddr((iSNeJ(Header(from_name,"utf-8")),from_email))
 return result
