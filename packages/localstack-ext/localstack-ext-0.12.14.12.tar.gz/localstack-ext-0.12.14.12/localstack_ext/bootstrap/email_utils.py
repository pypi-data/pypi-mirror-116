...
SENT_EMAILS=[]
EMAIL_BLACKLIST=fxlFo()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",fxlFm(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=fxlFH,from_name=fxlFH,smtp_host=fxlFH,smtp_user=fxlFH,smtp_pass=fxlFH,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if fxlFP(recipients,fxlFE)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=fxlFH):
 ...
 result["From"]=formataddr((fxlFm(Header(from_name,"utf-8")),from_email))
 return result
