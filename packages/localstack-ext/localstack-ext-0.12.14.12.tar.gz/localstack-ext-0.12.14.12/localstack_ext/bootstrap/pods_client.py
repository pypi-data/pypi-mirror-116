...
class CloudPodManager(bgcUY):
 BACKEND="_none_"
 def __init__(self,pod_name=bgcUF,config=bgcUF):
  self.pod_name=pod_name
  self._pod_config=config
 def push(self):
  raise bgcUV("Not implemented")
 def pull(self):
  raise bgcUV("Not implemented")
 ...
 def restart_container(self):
  ...
  def check_status():
   ...
   assert bgcUy(statuses)==bgcUy(["running"])
 ...
 def get(cls,pod_name):
  ...
  raise bgcUV('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 @bgcUJ
 def data_dir(cls):
  ...
  if not config.DATA_DIR:
   raise bgcUV("Working with local cloud pods requires $DATA_DIR configuration")
...
class CloudPodManagerManaged(CloudPodManager):
 ...
 def push(self):
  ...
  data_dir=self.data_dir()
  zip_data_content=create_zip_file(data_dir,get_content=bgcUz)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(bgcUQ(zip_data_content)/1000.0)))
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise bgcUV("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  print("Done.")
 def pull(self):
  data_dir=self.data_dir()
 ...
 def presigned_url(self,mode):
  ...
  if response.status_code>=400:
   raise bgcUV("Unable to push cloud pod to API (code %s): %s"%(response.status_code,content))
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b"HEAD" not in repo or repo.refs.allkeys()==bgcUy([b"HEAD"])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=bgcUz)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise bgcUV('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
  ...
  for folder in PERSISTED_FOLDERS:
   ...
   cp_r(src_folder,tgt_folder,rm_dest_on_conflict=bgcUz)
  self.restart_container()
 def client(self):
  client,path=get_transport_and_path_from_url(self.pod_config.get("url"))
  return client,path
 def local_repo(self):
  self.clone_dir=bgcUE(self,"clone_dir",bgcUF)
...
class PodConfigManagerMeta(bgcUN):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=bgcUF
   for manager in cls.CHAIN:
    try:
     tmp=bgcUE(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif bgcUX(tmp,bgcUn)and bgcUX(result,bgcUn):
       result.extend(tmp)
    except bgcUV:
     pass
   if result is not bgcUF:
    return result
   raise bgcUV('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(bgcUY,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @bgcUJ
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise bgcUV('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(bgcUY):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=bgcUz).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=bgcUy([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=bgcUS):
  try:
   return yaml.load(to_str(load_file(self.CONFIG_FILE)))
  except bgcUV:
   if safe:
    return{}
   raise bgcUV('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(bgcUY):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise bgcUV("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise bgcUV("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def pull_state(pod_name,args):
 if not pod_name:
  raise bgcUV("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name)
 backend.pull()
def list_pods(args):
