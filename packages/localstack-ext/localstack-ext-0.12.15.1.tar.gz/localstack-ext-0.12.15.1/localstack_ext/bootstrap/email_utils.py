...
SENT_EMAILS=[]
EMAIL_BLACKLIST=JqSnP()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",JqSnf(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=JqSnC,from_name=JqSnC,smtp_host=JqSnC,smtp_user=JqSnC,smtp_pass=JqSnC,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if JqSnH(recipients,JqSnv)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=JqSnC):
 ...
 result["From"]=formataddr((JqSnf(Header(from_name,"utf-8")),from_email))
 return result
