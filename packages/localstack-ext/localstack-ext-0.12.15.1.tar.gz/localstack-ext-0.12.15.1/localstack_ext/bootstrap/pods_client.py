...
class CloudPodManager(FVxeW):
 BACKEND="_none_"
 def __init__(self,pod_name=FVxem,config=FVxem):
  self.pod_name=pod_name
  self._pod_config=config
 def push(self):
  raise FVxeI("Not implemented")
 def pull(self):
  raise FVxeI("Not implemented")
 ...
 def get(cls,pod_name,pre_config=FVxem):
  ...
  raise FVxeI('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 @FVxeg
 def data_dir(cls):
  if not config.DATA_DIR or in_docker():
   config.DATA_DIR=get_data_dir_from_container()
  if not config.DATA_DIR:
   raise FVxeI("Working with local cloud pods requires $DATA_DIR configuration")
...
class CloudPodManagerFilesystem(CloudPodManager):
 ...
 def pull(self):
  ...
  with FVxen("datadir.zip","rb")as fileobj:
   data=fileobj.read()
...
class CloudPodManagerManaged(CloudPodManager):
 ...
 def push(self):
  ...
  data_dir=self.data_dir()
  zip_data_content=create_zip_file(data_dir,get_content=FVxeP)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(FVxes(zip_data_content)/1000.0)))
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise FVxeI("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  print("Done.")
 def pull(self):
 ...
 def presigned_url(self,mode):
  ...
  if response.status_code>=400:
   raise FVxeI("Unable to push cloud pod to API (code %s): %s"%(response.status_code,content))
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b"HEAD" not in repo or repo.refs.allkeys()==FVxeT([b"HEAD"])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=FVxeP)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise FVxeI('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
  ...
  for folder in PERSISTED_FOLDERS:
   ...
   cp_r(src_folder,tgt_folder,rm_dest_on_conflict=FVxeP)
  self.restart_container()
 def client(self):
  client,path=get_transport_and_path_from_url(self.pod_config.get("url"))
  return client,path
 def local_repo(self):
  self.clone_dir=FVxeH(self,"clone_dir",FVxem)
...
class PodConfigManagerMeta(FVxeJ):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=FVxem
   for manager in cls.CHAIN:
    try:
     tmp=FVxeH(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif FVxeb(tmp,FVxec)and FVxeb(result,FVxec):
       result.extend(tmp)
    except FVxeI:
     pass
   if result is not FVxem:
    return result
   raise FVxeI('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(FVxeW,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @FVxeg
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise FVxeI('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(FVxeW):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=FVxeP).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=FVxeT([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=FVxeN):
  try:
   return yaml.load(to_str(load_file(self.CONFIG_FILE)))
  except FVxeI:
   if safe:
    return{}
   raise FVxeI('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(FVxeW):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise FVxeI("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise FVxeI("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def push_state(pod_name,args,**kwargs):
 pre_config=kwargs.get("pre_config",FVxem)
...
def pull_state(pod_name,args,**kwargs):
 pre_config=kwargs.get("pre_config",FVxem)
 if not pod_name:
  raise FVxeI("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull()
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->FVxeE:
 try:
...
def get_persisted_resource_names()->List[FVxeE]:
 if not config.DATA_DIR:
  config.DATA_DIR=get_data_dir_from_container()
 names=[]
 ...
 with os.scandir(config.DATA_DIR+"/api_states")as entries:
  for entry in entries:
   if entry.is_dir()and FVxes(os.listdir(entry.path))>0:
    names.append(entry.name)
 LOG.debug(f"{names} are found to contain state")
 return names
