...
SENT_EMAILS=[]
EMAIL_BLACKLIST=CtgUz()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",CtgUB(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=CtgUI,from_name=CtgUI,smtp_host=CtgUI,smtp_user=CtgUI,smtp_pass=CtgUI,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if CtgUN(recipients,CtgUE)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=CtgUI):
 ...
 result["From"]=formataddr((CtgUB(Header(from_name,"utf-8")),from_email))
 return result
