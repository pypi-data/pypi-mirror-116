...
SENT_EMAILS=[]
EMAIL_BLACKLIST=PplRB()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",PplRm(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=PplRk,from_name=PplRk,smtp_host=PplRk,smtp_user=PplRk,smtp_pass=PplRk,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if PplRf(recipients,PplRY)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=PplRk):
 ...
 result["From"]=formataddr((PplRm(Header(from_name,"utf-8")),from_email))
 return result
