...
class PodInfo:
 def __init__(self,name=nCbBi,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(nCbBA):
 BACKEND="_none_"
 def __init__(self,pod_name=nCbBi,config=nCbBi):
  self.pod_name=pod_name
  self._pod_config=config
 def push(self)->PodInfo:
  raise nCbBY("Not implemented")
 def pull(self):
  raise nCbBY("Not implemented")
 ...
 def get(cls,pod_name,pre_config=nCbBi):
  ...
  raise nCbBY('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=nCbBV
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=nCbBv)
  ...
  if result.status_code>=400:
   raise nCbBY("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 def get_state_zip_from_instance(self,get_content=nCbBV):
  url=f"{self.get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise nCbBY("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
 ...
 def get_pod_info(self,pod_data_dir:nCbBx=nCbBi):
...
class CloudPodManagerManaged(CloudPodManager):
 ...
 def push(self)->PodInfo:
  ...
  zip_data_content=self.get_state_zip_from_instance(get_content=nCbBv)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(nCbBl(zip_data_content)/1000.0)))
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise nCbBY("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=nCbBl(zip_data_content)
  return result
 def pull(self):
 ...
 def presigned_url(self,mode):
  ...
  if response.status_code>=400:
   raise nCbBY("Unable to push cloud pod to API (code %s): %s"%(response.status_code,content))
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b"HEAD" not in repo or repo.refs.allkeys()==nCbBt([b"HEAD"])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=nCbBv)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise nCbBY('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=nCbBc(self,"clone_dir",nCbBi)
...
class PodConfigManagerMeta(nCbBW):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=nCbBi
   for manager in cls.CHAIN:
    try:
     tmp=nCbBc(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif nCbBN(tmp,nCbBu)and nCbBN(result,nCbBu):
       result.extend(tmp)
    except nCbBY:
     pass
   if result is not nCbBi:
    return result
   raise nCbBY('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(nCbBA,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @nCbBD
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise nCbBY('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(nCbBA):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=nCbBv).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=nCbBt([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=nCbBV):
  try:
   return yaml.load(to_str(load_file(self.CONFIG_FILE)))
  except nCbBY:
   if safe:
    return{}
   raise nCbBY('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(nCbBA):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise nCbBY("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise nCbBY("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def push_state(pod_name,args,**kwargs):
 pre_config=kwargs.get("pre_config",nCbBi)
...
def pull_state(pod_name,args,**kwargs):
 pre_config=kwargs.get("pre_config",nCbBi)
 if not pod_name:
  raise nCbBY("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull()
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->nCbBx:
 try:
...
def get_persisted_resource_names(data_dir)->List[nCbBx]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and nCbBl(os.listdir(entry.path))>0:
    names.append(entry.name)
