...
SENT_EMAILS=[]
EMAIL_BLACKLIST=IPLvp()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",IPLvx(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=IPLvD,from_name=IPLvD,smtp_host=IPLvD,smtp_user=IPLvD,smtp_pass=IPLvD,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if IPLvB(recipients,IPLvm)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=IPLvD):
 ...
 result["From"]=formataddr((IPLvx(Header(from_name,"utf-8")),from_email))
 return result
