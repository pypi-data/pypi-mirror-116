...
SENT_EMAILS=[]
EMAIL_BLACKLIST=FNeit()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",FNeiy(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=FNeiX,from_name=FNeiX,smtp_host=FNeiX,smtp_user=FNeiX,smtp_pass=FNeiX,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if FNeiu(recipients,FNeiT)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=FNeiX):
 ...
 result["From"]=formataddr((FNeiy(Header(from_name,"utf-8")),from_email))
 return result
