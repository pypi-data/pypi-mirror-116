...
def register_localstack_plugins():
 ...
 if OKsxo([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
...
def load_plugin_files():
 try:
  ...
  from localstack_ext.services.ec2 import ec2_starter
  ...
  register_plugin(Plugin("ec2",start=ec2_starter.start_ec2,priority=10))
  ...
  if "No module named" not in OKsxM(e):
...
def api_key_configured():
 return OKsxw(os.environ.get("LOCALSTACK_API_KEY"))
