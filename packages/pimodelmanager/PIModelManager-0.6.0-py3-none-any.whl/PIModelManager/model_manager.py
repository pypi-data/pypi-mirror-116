...
class ModelManager:
    ...
    def _initialize_model_from_config(cls, config: Dict[str, str]) -> None:
        ...
        if not cls._models[model_name][language][version]:
            ...
            cls._models[model_name][language][version] = cls._model_class_mapping[model_name](
                model_path, config)
            logger.info(f'Initializing {model_name} model with {config}')
    ...
    @classmethod
    def update_config_from_json(cls, token_info: Any, bg_task: Any = None) -> None:
        ...
        if check_has_changed or not check_tenant_present:
            if bg_task:
                ...
                return HTTPException(
                    503, 'Config has changed: updating models. This might take a couple of minutes.')
            else:
    ...
    def _check_validity_config(cls, token_info):
        ...
        for tenant_config in config:
            if not set(required_keys) == set(tenant_config.keys()):
                raise HTTPException(
                    400, detail=f'Config {tenant_config} does not contain required keys {required_keys}')
