...
class ModelManager:
    ...
    def update_config_from_json(cls, token_info: Any, bg_task: Any = None):
        ...
        """
        msg = JSONResponse(
            status_code=503, content={'message': 'Config has changed: updating models. This might take a couple of minutes.'})
    ...
    def _check_validity_config(cls, token_info):
        ...
        try:
            for tenant_config in config:
                assert not set(required_keys) == set(tenant_config.keys())
        except:
            raise HTTPException(
                400, detail=f'Config {config} does not contain required keys {required_keys}')
