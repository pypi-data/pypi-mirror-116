...
class ModelManager:
    ...
    def __init__(cls, model_class_mapping: Dict[str, Callable] = {}) -> None:
    ...
    @classmethod
    def update_config_from_json(cls, token_info: Any, config_key: str= 'ModelManager') -> None:
        ...
        check_has_changed = token_info.tenant_config[config_key]['has_changed']
        ...
        if check_has_changed or not check_tenant_present:
            config = token_info.tenant_config[config_key]['configurations']['MODELS_JSON']
            for tenant_config in config:
    ...
    def get_model(cls, model_name: str, language: str, token_info: Any = {}, path: bool = False) -> Any:
        ...
        try:
            version = ModelManager._tenant_configs[tenant_name][model_name][language]['version']
            client = ModelManager._tenant_configs[tenant_name][model_name][language]['client']
        except Exception as _:
            logger.warning(
                f'No tenant_config for {tenant_name}, {model_name}, {language}. Ask an administrator to set this within the App Config STS.')
            version = ModelManager._tenant_configs[cls._default_tenant_name][model_name][language]['version']
            client = ModelManager._tenant_configs[cls._default_tenant_name][model_name][language]['client']
            logger.warning(
    ...
    def __download_model_dataservices(cls, model: Dict[str, str]) -> None:
        ...
        matched_models = [
            f for f in cls._available_files if query in f'{f}.']
