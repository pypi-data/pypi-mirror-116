...
class MysqlHelper:
    ...
    def __init__(self, **kwargs):
        ...
        self.mysql_config = self.get_mysql_config(**kwargs)

    @classmethod
    def get_mysql_config(cls, **kwargs):
        return {
            ...
            "password": kwargs.get('password', os.getenv('MYSQL_PASSWORD', 'admin123')),
    ...
    def export(self, project_name='.', check_run_date=True, check_insert_update_time=True, **kwargs):
        self.connect()
        self.table = kwargs.get('table')
        '''
        check run date part
        '''
        sql_run_date = kwargs.get('sql_rundate', "select max(rundate) from {}".format(self.table))
        run_date_db = self.get_run_date(sql_run_date)
        if kwargs.get('schedule_interval') == 'hour':
            run_date_check = run_date_db.strftime('%Y-%m-%d-%H')
        elif kwargs.get('schedule_interval') == 'minute':
            run_date_check = run_date_db.strftime('%Y-%m-%d-%H-%M')
        else:
            run_date_check = run_date_db.strftime('%Y-%m-%d')

        hour_delta = kwargs.get('hour_delta', 0)
        check_time = datetime.datetime.now() + datetime.timedelta(hours=hour_delta)

        if check_run_date:
            if run_date_check not in str(check_time):
                raise ValueError("run date in db not qualified: db: {} now: {} ".format(run_date_db, check_time))
            else:
                print("run date qualified: db: {} check_time: {} ".format(run_date_db, check_time))
        else:
            print("do not check run date")

        if check_insert_update_time:
            '''
            check insert time part
            '''

            sql_last_insert_time = kwargs.get('sql_rundate',
                                              "select tid, InsertUpdateTime from {table} order by 1 desc limit 1".format(
                                                  table=self.table))
            ...
            print("do not check insert update time")

        '''
        download file part
        '''

        sql = kwargs.get('sql', '''select * from {table} where rundate = (select max(rundate) from {table} ) '''.format(
            table=self.table))
        file_type = kwargs.get('file_type', 'csv')
        file_name_template = kwargs.get('file_name_template', '{}_%s'.format(self.table))
        file_name = kwargs.get('file_name', file_name_template % run_date_check)
        stack = inspect.stack()
    ...
    def get_run_date(self, sql_rundate):
        cursor = self._cursor()
        try:
            cursor.execute(sql_rundate)
            result = cursor.fetchone()
            run_date_db = list(result.values())[0]
            return run_date_db
        finally:
            cursor.close()
...
class MySqlHook:
    ...
    def __init__(self, *args, **kwargs):
        self.conn_name_attr = args[0]
        pass

    def get_conn(self):
        hook_config = self.get_hook_config()
        mysql_config = MysqlHelper.get_mysql_config(**hook_config)
        conn = MySQLdb.connect(**mysql_config)
        return conn

    def get_hook_config(self):
        hook_config = {}
        config_file_path = os.path.join(os.path.expanduser('~'), '.degree72-simple', 'hook_config.json')
        if os.path.exists(config_file_path):
            with open(config_file_path, 'r') as fh:
                hook_config = json.load(fh)
        return hook_config.get(self.conn_name_attr, {})


if __name__ == '__main__':
    MySqlHook('mysql_conn_qa').get_conn()
