...
class InstagramProfile(InsomniacModel):
    ...
    def get_session_time_within_minutes(self, minutes) -> int:
        """
        Returns the amount of active-session-seconds, within the last 'minutes'
        """
        with db.connection_context():
            return get_session_time_within_minutes_for_profile(minutes, profile=self)
...
def get_session_time_within_minutes_for_profile(minutes=None, profile=None) -> int:
    """Returns the amount of session time in seconds per profile, on the last 'minutes' for 'profiles'"""

    total_session_time_by_profile_name = get_session_time_within_minutes(minutes, [profile] if profile is not None else None)
...
def get_session_time_within_minutes(minutes=None, profiles=None) -> dict:
    """Returns the amount of session time in seconds per profile, on the last 'minutes' for 'profiles'"""
    ...
    return get_session_time_from_to(timestamp_from, timestamp_to, profiles)


def get_session_time_from_to(timestamp_from=None, timestamp_to=None, profiles=None) -> dict:
