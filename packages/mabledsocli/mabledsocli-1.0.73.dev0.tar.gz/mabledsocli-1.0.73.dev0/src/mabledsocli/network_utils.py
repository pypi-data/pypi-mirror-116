...
def layout_subnet_plan(subnet_plan, summary=True, compress=True):
                ...
                if range_first_subnet is None:
                    ...
                    if summary:
                        result[key]['Ranges'][-1]['Subnets'].append({
                            'CIDR': range_first_subnet.with_prefixlen,
                            'Number': result[key]['Count'] + 1,
                            'Name': segment['labels'].get(str(result[key]['Count']), UNNAMED_SUBNET),
                        })
    ...
    add_segment_subnets(subnet_plan['segments'], 0, ipaddress.ip_network(subnet_plan['cidr']), result['SubnetPlanLayout'])
    return result
