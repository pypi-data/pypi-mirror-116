...
from one.alf.files import rel_path_parts, session_path_parts, get_session_path, get_alf_path
...
class RegistrationClient:
    ...
    def register_session(self, ses_path, users=None, file_list=True, **kwargs):
        ...
        if file_list:
            self.register_files(self.find_files(ses_path) if file_list is True else file_list)
    ...
    def register_files(self, file_list):
        ...
        for session_path, files in F.items():
            ...
            rel_path = Path(str(fn)[str(fn).find(str(session_path)):])
            F[session_path].append(str(rel_path.relative_to(session_path).as_posix()))
