...
class _QueryHelper:
    ...
    def query_features(self, join_keys: Mapping[str, Union[int, np.int_, str, bytes]]) -> "tecton.DataFrame":
        ...
        assert conf.get("FEATURE_SERVICE") is not None, "FEATURE_SERVICE must be configured"
        http_response = requests.post(
            urljoin(conf.get("FEATURE_SERVICE") + "/", "v1/feature-service/query-features"),
    ...
    def get_feature_vector(
        ...
        assert conf.get("FEATURE_SERVICE") is not None, "FEATURE_SERVICE must be configured"
        http_response = requests.post(
            urljoin(conf.get("FEATURE_SERVICE") + "/", "v1/feature-service/get-features"),
            data=MessageToJson(request),
            headers=self._prepare_headers(),
        )
