...
def _get_basic_spark_session_builder():
    ...
    if conf.get("AWS_ACCESS_KEY_ID") is not None:
        builder = builder.config("spark.hadoop.fs.s3a.access.key", conf.get("AWS_ACCESS_KEY_ID"))
        builder = builder.config("spark.hadoop.fs.s3a.secret.key", conf.get("AWS_SECRET_ACCESS_KEY"))
    if conf.get("HIVE_METASTORE_HOST") is not None:
        builder = (
            builder.enableHiveSupport()
            .config(
                "spark.hadoop.javax.jdo.option.ConnectionURL",
                f"jdbc:mysql://{conf.get('HIVE_METASTORE_HOST')}:{conf.get('HIVE_METASTORE_PORT')}/{conf.get('HIVE_METASTORE_DATABASE')}",
            )
            .config("spark.hadoop.javax.jdo.option.ConnectionDriverName", "com.mysql.cj.jdbc.Driver")
            .config("spark.hadoop.javax.jdo.option.ConnectionUserName", conf.get("HIVE_METASTORE_USERNAME"))
            .config("spark.hadoop.javax.jdo.option.ConnectionPassword", conf.get("HIVE_METASTORE_PASSWORD"))
        )
    for v in EXECUTOR_ENV_VARIABLES:
        if conf.get(v) is not None:
            builder = builder.config(f"spark.executorEnv.{v}", conf.get(v))
    return builder
...
def get_or_create_spark_session(custom_spark_options=None):
    ...
    if conf.get("SPARK_DRIVER_LOCAL_IP"):
        ...
        os.environ["SPARK_LOCAL_IP"] = conf.get("SPARK_DRIVER_LOCAL_IP")
