...
class RefreshFlow:
    ...
    def get_tokens(self, refresh_token):
        params = {
            ...
            "client_id": conf.get("CLI_CLIENT_ID"),
            "scope": "openid",
        }
        try:
...
def get_token_refresh_if_needed():
    token = conf.get("OAUTH_ACCESS_TOKEN")
    if not token:
        return None
    expiration = conf.get("OAUTH_ACCESS_TOKEN_EXPIRATION")
    if expiration and time.time() < int(expiration):
        return token
    else:
        f = RefreshFlow()
        refresh_token = conf.get("OAUTH_REFRESH_TOKEN")
