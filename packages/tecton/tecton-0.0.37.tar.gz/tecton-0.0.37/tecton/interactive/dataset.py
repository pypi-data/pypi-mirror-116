...
class Dataset(DataFrame):
    ...
    def _maybe_create(
        ...
        if save_as and save is False:
            raise errors.DATASET_NAME_PROVIDED_WHEN_SAVE_FALSE
        if not (save or save_as):
            return df
...
def get_dataset(name: str, workspace_name: Optional[str] = None) -> Dataset:
    ...
    request.workspace = workspace_name or conf.get("TECTON_WORKSPACE")
