...
class WTE_Input(InputData):
    def __init__(self, input_data_path=None, process_name='WTE', CommonDataObjct=None):
        ...
        self.process_data = pd.read_csv(Path(__file__).parent / "Data/WTE_Input_MaterialDependent.csv",
                                        index_col=0,
                                        header=0,
                                        skiprows=[1, 2, 3]).loc[CommonDataObjct.Index].astype(float)
        self.process_data.fillna(0, inplace=True)
        self.process_data_info = pd.read_csv(Path(__file__).parent / "Data/WTE_Input_MaterialDependent.csv",
                                             index_col=0,
                                             header=0,
                                             nrows=3)
