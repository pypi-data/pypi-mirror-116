        ...
        for i in range(len(peptides)):
            ...
            for q in range(len(strp)):
                ...
                if(len(c1)>0 and len(c2)>0 and len(common)>0):
                    ...
                    if s2==(s1+len(str1)+1) or len(str1)==0 or len(str2)==0:
                        start_ends= s1
...
# peptides=["MAMTINYQFGDVDAH", "MHVSFVMAYPEMLAA"] 
#p= PeptideSearch("Peptides.txt", "test.fa")
# df, pList9= p.OneMismatch(List)
# 
#d, s= PeptideSearch.SequenceSearch("sl", "abcsm")
