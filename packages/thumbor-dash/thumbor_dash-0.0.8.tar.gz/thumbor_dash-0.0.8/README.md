...
# Running thumbor_dash with Docker

This is the fastest way to run `thumbor_dash`

#### 1. Create a file named `thumbor.env.txt` and add these environment variables
...
#### 2. Start thumbor_dash server

   `docker run -p 80:80 --env-file thumbor.env.txt mayoreee/thumbor_dash`

Note: If running in Docker, `<thumbor_dash-server>` in the request URL should be set to `localhost` instead of `localhost:8888`.
