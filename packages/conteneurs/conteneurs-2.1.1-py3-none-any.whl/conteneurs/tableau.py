...
class Tableau :
    ...
    def affichage(self, largueur_colonne=15, print=True) :
        """Méthode qui permet d'afficher le tableau.
        
        On peut définir la largueur des colonnes, en nombre d'espaces, grâce au paramètre largueur_colonne. Si on préfère retourner une chaine qui représente le tableau pour l'affichage, on met print à False.
        """
        chaine = """"""
        if self._en_tetes_h :
            chaine += self._str_ligne(0, largeur_colonne=largueur_colonne)
        chaine += "\n"
        for i in range(1, len(self._tableau)) :
            chaine += self._str_ligne(i, largeur_colonne=largueur_colonne)
            chaine += "\n"
        if print :
            print(chaine)
        else :
