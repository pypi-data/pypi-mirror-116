...
re_index = re.compile(r"^[A-Z]{0,3}([1-9][0-9]{0,6})?$")#un index valide correspond toujours à cette re
re_index_ligne = re.compile(r"^[1-9][0-9]{0,6}$")#index correspondant à une ligne
re_index_colonne = re.compile(r"^[A-Z]{1,3}$")#index correspondant à une colonne
re_index_case = re.compile(r"^[A-Z]{1,3}[1-9][0-9]{0,6}$")#index corresondant à une case
re_index_partie_ligne = re.compile(r"[1-9][0-9]{0,6}$")
re_index_partie_colonne = re.compile(r"^[A-Z]{1,3}")
re_index_ligne_ou_colonne = re.compile(r"^[1-9][0-9]{0,6}|[A-Z]{1,3}$")
...
Vlargeur_colonne = Verificateur(int, 3, 200)
...
VA_str_en_tetes_h = VerificateurArguments()
VA_str_en_tetes_h.append(0, verificateur=Verificateur())#correspond à l'argument self
VA_str_en_tetes_h.append(1, "largeur_colonne", Vlargeur_colonne)
...
class Tableau :
    ...
    def __str__(self) :
        ...
        """
        chaine = """"""
        chaine += self._str_noms_colonnes()
        chaine += "\n"
        for i in range(1, len(self._tableau)) :
            chaine += self._str_ligne(i)
            chaine += "\n"
        return chaine
    ...
    def __contains__(self, valeur) :
        ...
        for i, ligne in enumerate(self._tableau) :
            for j, case in enumerate(ligne) :
                if i == 0 and j==0 :
                    if self._tableau[0][0] == valeur and self._en_tetes_h and self._en_tetes_v :
                        retour = True
                        break
                elif i==0 :#and j!=0
                    if self._tableau[0][j] == valeur and self._en_tetes_h :
                        retour = True
                        break
                elif j==0 : #and i!=0
                    if self._tableau[i][0] == valeur and self._en_tetes_v :
                        retour = True
                        break
                else :#i!=0 and j!=0
                    if self._tableau[i][j] == valeur :
                        retour = True
                        break
        return retour
    ...
    def set_ligne(self, index_ligne, nouvelle_valeur) :
        ...
        if not self._en_tetes_v :#s'il n'y a pas d'en-têtes, l'utilisateur ne saisi pas de case pour les en-têtes 
    ...
    def del_ligne(self, index_ligne) :
        ...
        La ligne est supprimée, c'est à dire que sa place dans le tableau est enlevée et les lignes qui sont en-dessous remontent d'un cran. Les dimensions du tableau changent.
        """
        Verificateur(int, 0, len(self._tableau)-1).controle_total(index_ligne)#vérifie que ligne est valide
    ...
    def set_colonne(self, index_colonne, nouvelle_valeur) :
        ...
        La colonne a pour valeur nouvelle_valeur, qui doit être une liste avec la même nombre d'éléments que les autres colonnes.
        """
        index_colonne_int = self._index_colonne_int(index_colonne)#conversion id colonne en chiffre
        longueur_liste = len(self._tableau)#le nombre de lignes ; la liste doit avoir autant d'éléments que ce nombre.
        if not self._en_tetes_h :#s'il n'y a pas d'en-têtes, l'utilisateur ne saisi pas de case pour les en-têtes 
            nouvelle_valeur.insert(0, self._tableau[0][index_colonne_int])#mais on en rajoute une en conservant la valeur de l'en-tête d'origineif self._en_tetes_v :
        VLliste = VerificateurListes(minimum=longueur_liste, maximum=longueur_liste)#Vérification de la longueur et du type de la liste
        nouvelle_valeur = VLliste.controle_total(nouvelle_valeur)
    ...
    def del_colonne(self, index_colonne) :
        ...
        La colonne est supprimée, c'est à dire que sa place dans le tableau est enlevée et les colonnes à sa droite sont décalés. Les dimensions du tableau changent.
        """
        index_colonne_int = self._index_colonne_int(index_colonne)#conversion id colonne en chiffre
        for index_ligne in range(len(self._tableau)) :
    ...
    def del_case(self, index) :
        ...
        La valeur de la case vaut alors None.
        """
        index_colonne_int, index_ligne = self._index_case_int(index)
    ...
    #Méthodes parcourant le tableau
    ...
    @controle_arguments(VA_str_en_tetes_h)
    def _str_noms_colonnes(self, largeur_colonne=15) :
        """Affiche la ligne des en-têtes horizontaux, si en_tetes_h vaux True.
        
        Affiche le nom du tableau si en_tetes_h et en_tetes_v valent True. Affiche les en-têtes horizontaux, si en_tetes_h vaux True.
        Le paramètre largeur_colonne est le nombre d'espaces qui définissent la largeur des colonnes.
        """
        chaine = """"""
        if self._en_tetes_h : #si en_tetes_h est vrai (vaut True)
            if self._en_tetes_v : #si en_tetes_v est vrai
                chaine += self._str_case(0,0, largeur_colonne)
                chaine+= "|"
            for i in range(1, len(self._tableau)) :
                chaine += self._str_case(0,i, largeur_colonne)
                chaine+= "|"
        return chaine

    @controle_arguments(VA_str_ligne)
    def _str_ligne(self, index_ligne, largeur_colonne=15) :
    ...
    def _str_case(self, index_ligne, index_colonne_int, largeur_colonne=15) :
        ...
        Le paramètre largeur_colonne est le nombre d'espaces qui définissent la largeur des colonnes. La chaine renvoyée correspond à la largeur de la colonne. Si la largueur de colonne ne suffit pas à afficher tout le contenu de la case, la chaine est tronquée.
        """
        Verificateur(int, 0, len(self._tableau)-1).controle_total(index_ligne)#vérifie que index_ligne est valide
    ...
    def _index_colonne_str(self, index_colonne_int) :
        ...
        Convertit un nombre entier en identifiant d'une colonne, composé d'une ou plusieurs majuscule, en nombre entier. Ce nombre entier est un index correspondant à la position de la colonne dans le tableau.
        """
