...
class RenderImageSequence(RenderWithAfterEffect):
    ...
    _files = flow.Parent(2)
    zip_folder_action = flow.Child(ZipFolder)
    ...
    def run(self, button):
        ...
        # Launch a session worker which will zip images after rendering
        ...
        # Zip images
        self.zip_folder_action.pid.set(pid)
        self.zip_folder_action.folder_path.set(self._output_folder)
        self.zip_folder_action.output_path.set(target_revision.get_path())
        result = self.zip_folder_action.run(None)
...
class MarkSequence(GenericRunAction):
    ...
    def run(self, button):
        ...
        # Get AE scene name from folder suffix
        ss = '_render'
        ae_scene_name = self._file.name()
        if ae_scene_name.endswith(ss):
            ae_scene_name = ae_scene_name[:-len(ss)]
        
        playblast_file = self._ensure_file(ae_scene_name + '_movie', 'mov')
        target_revision = self._ensure_revision(playblast_file, revision_name)
        
        file_name = playblast_file.complete_name.get() + '.' + playblast_file.format.get()
        audio_path = ''
