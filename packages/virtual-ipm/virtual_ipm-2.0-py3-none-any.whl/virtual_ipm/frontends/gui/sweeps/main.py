...
from anna.exceptions import IncompleteConfigurationError
                ...
                for full_config_path, config in configurations.items():
                    ...
                    try:
                        ...
                        for aware_param in model_params:
                            ...
                            self.meta[f'{full_config_path}/{param.name}'] = _extract_meta_data(param)
...
class SaveSweepDialog(QDialog):
    ...
    def confirm(self):
        ...
        if not directory.is_dir():
            ...
            return
        if next(directory.glob('*'), None) is not None:
