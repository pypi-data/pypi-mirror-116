...
class IVFPQTop1Cuda(CustomKernel):
  def __init__(
    ...
    self._top1_smart_probing_fn = cp.RawKernel(
      code = kernel,
      name = 'ivfpq_top1_smart_probing',
      options = (
        '--maxrregcount=255',
        '--use_fast_math'
      ),
    ...
    self._top1_residual_smart_probing_fn = cp.RawKernel(
      code = kernel,
      name = 'ivfpq_top1_residual_smart_probing',
      options = (
        '--maxrregcount=255',
        '--use_fast_math'
      ),
    ...
    self._top1_residual_precomputed_smart_probing_fn = cp.RawKernel(
      code = kernel,
      name = 'ivfpq_top1_residual_precomputed_smart_probing',
      options = (
        '--maxrregcount=255',
        '--use_fast_math'
      ),
  ...
  def top1(
      ...
      is_empty, cell_start, cell_size, n_candidates=1):
    """
      ...
      cell_start: shape=[n_query, n_probe], dtype=int32
      cell_size: shape=[n_query, n_probe], dtype=int32
    """
    n_data = data.shape[1]
  ...
  def top1_residual_smart_probing(
      self, data, precomputed, base_sims,
      is_empty, cell_start, cell_size, n_probe_list, n_candidates=1):
    """
      data: shape=[n_subvectors // n_cs, n_data, n_cs], dtype=uint8
    ...
    self._top1_residual_smart_probing_fn(
      grid=blocks_per_grid,
      block=threads_per_block,
      shared_mem = self.sm_size,
      args=[
  ...
  def top1_residual_precomputed_smart_probing(
    ...
    self._top1_residual_precomputed_smart_probing_fn(
      grid=blocks_per_grid,
      block=threads_per_block,
      shared_mem = self.sm_size,
      args=[
