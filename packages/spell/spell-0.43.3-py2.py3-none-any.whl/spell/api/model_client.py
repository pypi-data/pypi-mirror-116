...
class ModelClient(base_client.BaseClient):
    ...
    def download_model_version(self, model_version_obj, dest, echo, quiet):
        resources_client = ResourcesClient(
            self.base_url, self.version_str, owner=self.owner, token=self.token,
            spell_admin_auth_token=self.spell_admin_auth_token
        )
        ...
        if len(model_version_obj.files) == 0:
            ...
            todo = [
                f"{resource}/{r.resource_path}" for r in model_version_obj.files
            ]
