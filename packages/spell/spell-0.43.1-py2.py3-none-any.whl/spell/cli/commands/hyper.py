...
def list_hyper(ctx):
    ...
    for hyper in searches:
        data.append(
            (hyper.id, hyper.num_runs, prettify_time(hyper.created_at), hyper.creator.user_name)
        )
...
def grid(
    ...
    **kwargs,
    ...
    utf8 = ctx.obj["utf8"]
    click.echo(
        with_emoji("💫", "Casting hyperparameter search #{}".format(hyper.id), utf8) + ellipses(utf8)
    )
