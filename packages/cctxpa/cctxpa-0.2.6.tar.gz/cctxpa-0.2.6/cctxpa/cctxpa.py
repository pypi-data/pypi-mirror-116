...
class CCTXPcapAnalyser:
    def __init__(self, args):
        ...
        # 一个先入先出线程安全的队列
        self.queryTaskQueue = Queue(maxsize=100)
        # 退出事件，当pcap解析结束时，会设置这个事件，线程在处理完数据之后就会退出
    ...
    def doQuery(self, queryItems: [dict], extraParams: dict) -> [dict]:
        """
        执行一组查询任务。执行下列步骤
        ...
        if len(remainQueryItems) == 0:
            # 直接本地查询成功
            self.dealQueryResult(results, extraParams)
            return
            # return results

        # 如果还有没有命中缓存的查询任务需要查询，则向服务器后台发送查询请求
        self.queryTaskQueue.put({
            "remain": remainQueryItems,
            "cacheResult": results,
    ...
    def dealDNSRecord(self, item: DNSItem):
        """
        处理每个提取到的 DNS 解析记录
        :param item:
        :return:
        ...
        # query DNSItem
        self.doQuery([
            CCTXPcapAnalyser.buildQueryDomain(item.domain),
            CCTXPcapAnalyser.buildQueryAddress(item.value)
        ], {
            "type": "dns",
            "item": item,
        })
        # self.report.addDNSRecord(item, observables)
    ...
    def dealQueryResult(self, observableItems, extraParams: dict):
        """
        处理每个查询结果，包括本地和网路的查询结果
        :return:
        """
        if extraParams["type"] == 'dns':
            del extraParams["type"]
            self.report.addDNSRecord(observables=observableItems, **extraParams)
        elif extraParams["type"] == 'mail':
            del extraParams["type"]
            self.report.addEmail(observables=observableItems, **extraParams)
        elif extraParams["type"] == 'http':
            del extraParams["type"]
            self.report.addHttp(observables=observableItems, **extraParams)
        elif extraParams["type"] == 'ftp':
            del extraParams["type"]
            self.report.addFileHash(observables=observableItems, **extraParams)

    def dealNetworkRequest(self):
        """
        在单独的线程处理网络请求
        :return:
        """

        def buildAuthGet(self, url: str, params: dict):
            return grequests.get(url, params=params, headers={
                "Authorization": f"Bearer {self.token}"  # 在请求头中携带 token
            })

        def doMultiQuery(requestTask: []):
            responses = grequests.map(requestTask)
            results = []
            for r in responses:
                if not r or r.status_code == 401:
                    ...
                    # 通过这种方式，即便 token 的有效期很短，使用命令行工具的用户也感觉不到
                    self.doLogin()
                    return doMultiQuery(requestTask)
                elif r.status_code != 200:
                    print({
                        "code": 500,
                        "msg": r.reason,
        ...
        while not self.stopEvent.isSet():
            # 如果队列里面有待请求的事件，或者CCTXPA并没有设置停止标记，则继续阻塞读取
            tasks = []
            while True:
                try:
                    """
                    ...
                    """
                    task = self.queryTaskQueue.get(block=True, timeout=0.01)
                    tasks.append(task)
                    if len(tasks) == 100:
                        break
                except queue.Empty:
            ...
            for task in tasks:
                ...
                """
                requestTask.append(buildAuthGet(self, self.queryUrl, {
                    'queries': json.dumps(task["remain"])
                }))
            res = doMultiQuery(requestTask)
            # 处理结果
            for i in range(len(tasks)):
                ...
                if response['code'] == 200:
                    if response['data'] and len(response['data']) > 0:
                        for observable in response['data']:
                            self.queryCache[observable["value"]] = observable
                        self.dealQueryResult(response['data'], tasks[i]['extraParams'])
                else:
                    print(f'Query Observable failed, {response["msg"]}')
    ...
    def start(self):
        ...
        dealNetworkRequestThread = threading.Thread(target=self.dealNetworkRequest, name="DealNetworkRequestThread")
        dealNetworkRequestThread.start()
