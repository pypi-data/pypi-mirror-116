...
class ModelCheckpoint(Callback):
    ...
    def on_epoch_end(self, epoch, logs: dict = {}):
        value = None
        if "test" in logs and self.indicator in logs["test"]:
            value = logs["test"][self.indicator]
        elif "train" in logs and self.indicator in logs["train"]:
            value = logs["train"][self.indicator]
    ...
    def save_best_model(self, model_root, value, epoch, inverse=False):
        ...
        torch.save(model.module.state_dict(), model_file)
        self.logger.info("save best_model_path in:{}".format(model_file))
