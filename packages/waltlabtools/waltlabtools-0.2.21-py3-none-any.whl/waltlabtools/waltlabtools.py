...
def flatten(data, on_bad_data="warn"):
    ...
    on_bad_data : {"error", "ignore", "warn"}, optional
        ...
        ndarray. Options are as follows:
            ...
            - ``"ignore"`` : Returns a list or, failing that, the \
            original object.
...
class Model:
    ...
    xscale, yscale : {"linear", "log", "symlog", "logit"}, default "linear"
        ...
        log-normally and is best visualized on a log scale.
...
def lod(blank_signal, inverse_fun, coefs=None, sds=3, corr="c4"):
    ...
    corr : {"n", "n-1", "n-1.5", "c4"} or numeric, optional
        ...
        Specifies how this should be addressed. Options:
            ...
            example, ``corr=0``, ``corr=1``, and ``corr=1.5`` are
            equivalent to ``corr="n"``, ``corr="n-1"``, and
            ``corr="n-1.5"``, respectively.
...
class CalCurve:
    ...
    def bound_lod(self, x_flat):
        ...
        -------
        array
            An array of the same shape and size as ``x_flat``, with all
            its values above the LOD if ``force_lod`` is True, or the
    ...
    def fun(self, x):
        ...
        Returns
        -------
        y : same as ``x`` or array
            Measurement readings, such as fluorescence, calculated from
            the values ``x`` using the calibration curve. If possible,
            ``y`` will be the same type, size, and shape as ``x``; if not,
            ``y`` will be a 1D array of the same size as ``x``.
    ...
    def inverse(self, y):
        ...
        Use ``inverse`` to convert measurement readings (e.g., fluorescence)
        to values (e.g., concentration) of the sample.
        ...
        Returns
        -------
        x : same as ``y`` or array
            Values, such as concentration, calculated from
            the values ``y`` using the calibration curve. If possible,
            ``x`` will be the same type, size, and shape as ``y``; if not,
            ``x`` will be a 1D array of the same size as ``y``.
    ...
    def from_data(cls, x, y, model, lod_sds=3, force_lod=False,
        ...
        lod_sds : numeric, default 3
            Number of standard deviations above blank at which the lower
            limit of detection is calculated. Common values are 2.5 and
            3.
        force_lod : bool, default False
            Should readings below the LOD be set to the LOD?
        use_inverse : bool, default False
