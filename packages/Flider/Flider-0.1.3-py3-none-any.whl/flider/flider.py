...
def Flider():
    ...
    read_names = ['index.flid', 'mysqlcontroller.flid', '__init__.flid', 'style.flid', 'scripts.flid', 'server.flid']
    for i in range(len(save_paths)):
        try:
            completeName = os.path.join(save_paths[i], file_names[i])
            write_file = open(completeName, 'w')
            write_file.write(pkg_resources.read_text(read_files, read_names[i]))
            write_file.close()
        except Exception as e:
            print(e)
