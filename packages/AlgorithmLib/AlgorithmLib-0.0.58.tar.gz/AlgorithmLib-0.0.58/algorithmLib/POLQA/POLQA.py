...
mosResult = copy.deepcopy(constMosResult)
import os
import xlwt
...
def VQTMSGfunc(msg):
    ...
    if 'VQT PAMS/PSQM Test Failed' in cs:
        ...
        if 'POLQA:' in cs:
            mosResult['mos'] = cs.split('POLQA:')[1].strip()
        if 'Speech Level Gain:' in cs:
            mosResult['Speech Level Gain'] = cs.split('Speech Level Gain:')[1].strip()
        if 'Noise Level Gain:' in cs:
            mosResult['Noise Level Gain'] = cs.split('Noise Level Gain:')[1].strip()
