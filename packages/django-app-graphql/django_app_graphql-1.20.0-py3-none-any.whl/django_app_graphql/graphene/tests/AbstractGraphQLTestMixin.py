...
class FileToUpload(object):
    ...
    def __init__(self, filepath: str, file_name_endpoint: str, encoding: str = "utf8", name: str = None, content_type: str = None):
        self.filepath = filepath
        self.encoding = encoding
        self.name = name or os.path.basename(self.filepath)
        self.content_type = content_type or "text/plain"
        self.file_name_endpoint = file_name_endpoint
        """
...
class AbstractGraphQLTestMixin(GraphQLFileUploadTestMixin, abc.ABC):
    ...
    def perform_graphql_query(self, graphql_query_info: Union[GraphQLRequestInfo, str]) -> GraphQLResponseInfo:
        ...
        if graphql_query_info.files is not None:
            ...
            for file_to_upload in graphql_query_info.files:
                with open(os.path.abspath(file_to_upload.filepath), encoding=file_to_upload.encoding, mode="rb") as f:
                    content = f.read()
                test_file = SimpleUploadedFile(name=file_to_upload.name, content=content, content_type=file_to_upload.content_type)
                files.append(dict(file_to_upload=file_to_upload, test_file=test_file))
            ...
            response = self.file_query(
                ...
                files={d["test_file"].file_name_endpoint:d["file_to_upload"] for d in files},
            )
        else:
            response = self.query(
                query=graphql_query_info.query,
    ...
    def perform_simple_upload_mutation(self, query: str, upload_filepath: str, upload_filepath_name: str = "test_file", content_type: str = "text/plain", upload_filepath_endpoint_name: str = "file", encoding: str="utf-8"):
        """
        ...
        :param upload_filepath_endpoint_name: name of the argument associated to the file
        ...
        graphql_query_info = GraphQLRequestInfo(
            ...
            files=[
                FileToUpload(
                    ...
                    file_name_endpoint=upload_filepath_endpoint_name,
                    encoding=encoding,
