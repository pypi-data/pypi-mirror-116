...
def _log_artifact_assets(
    ...
    artifact_remote_assets = artifact._remote_assets

    all_asset_ids = {
        artifact_asset.asset_id
        for artifact_asset in itertools.chain(artifact._assets, artifact_remote_assets)
    }
    ...
    total_size = sum(
        asset.size for asset in itertools.chain(artifact_assets, artifact_remote_assets)
    )
    ...
    for remote_artifact_preprocessor in IterationProgressCallback(
        artifact_remote_assets, progress_callback, frequency
    ):

        asset_id = remote_artifact_preprocessor.asset_id

        experiment._log_preprocessed_remote_asset(
            remote_artifact_preprocessor,
            artifact_version_id=artifact_version_id,
            critical=True,
            on_asset_upload=partial(
                _on_artifact_asset_upload,
        ...
        num_assets -= 1
        total_size -= remote_artifact_preprocessor.size
...
class Artifact(object):
    def __init__(
        ...
        self._assets = (
            []
        )  # type: List[Union[PreprocessedFileAsset, PreprocessedMemoryFileAsset]]
        self._remote_assets = []  # type: List[PreprocessedRemoteAsset]
    ...
    def add(
        ...
        self._assets.append(preprocessed)
    ...
    def add_remote(
        ...
        self._remote_assets.append(preprocessed)
    ...
    def download(
        ...
        for asset in raw_assets:
            ...
            if asset_remote is True:
                ...
                results.append((result, asset_filename, asset_path, asset_metadata))
