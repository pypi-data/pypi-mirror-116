...
def init_manifest_entry(task: common.BlockTask, task_key: str, capture:
    session.Capture) ->typ.Tuple[ManifestEntry, CaptureData]:
...
def _manifest_lines(manifest_entries: typ.List[ManifestEntry]) ->typ.Iterable[
    str]:
...
class ResultCache:
    ...
    def _reset_task_keys(self, task: common.BlockTask, entry: ManifestEntry
        ) ->None:
    ...
    def get_capture(self, task: common.BlockTask) ->typ.Optional[session.
        Capture]:
...
class LocalResultCache(ResultCache):
    ...
    def __init__(self, orig_files: MarkdownFiles) ->None:
