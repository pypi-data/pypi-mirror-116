...
class Dialogue:
    ...
    def as_dictionary(self) -> Dict:
        ...
        serialization."""
        return {"events": [event.as_dictionary() for event in self.events], "name": self.name}

    @classmethod
    def from_params(cls, parameters: Dict[Text, Any]) -> "Dialogue":
        ...
        return cls(
            parameters.get("name"),
            [Event.from_params(evt) for evt in parameters.get("events")],
        )
