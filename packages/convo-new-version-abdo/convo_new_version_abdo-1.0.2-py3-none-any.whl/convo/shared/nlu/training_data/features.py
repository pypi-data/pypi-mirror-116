...
from convo.shared.nlu.constants import VALIDATED_FEATURE_TYPES
...
class Features:
    ...
    def __init__(
        ...
        self.feature_type_validation(feature_type)
    ...
    @staticmethod
    def feature_type_validation(feature_type: Text) -> None:
        if feature_type not in VALIDATED_FEATURE_TYPES:
            raise ValueError(
                f"Invalid feature type '{feature_type}' used. Valid feature types are: "
                f"{VALIDATED_FEATURE_TYPES}."
            )

    def sparse_check(self) -> bool:
    ...
    def dense_check(self) -> bool:
        """Checks if features are dense or not.

        Returns:
            True, if features are dense, false otherwise.
        """
        return not self.sparse_check()

    def combine_features(self, additional_features: Optional["Features"]) -> None:
        ...
        if self.dense_check() and additional_features.dense_check():
            self.combining_dense_features(additional_features)
        elif self.sparse_check() and additional_features.sparse_check():
            self.combining_sparse_features(additional_features)
        else:
            raise ValueError("Cannot combine sparse and dense features.")

    def combining_dense_features(self, additional_features: "Features") -> None:
    ...
    def combining_sparse_features(self, additional_features: "Features") -> None:
    ...
    def __eq__(self, others: Any) -> bool:
        if not isinstance(others, Features):
            return False

        return (
            others.type == self.type
            and others.attribute == self.attribute
            and others.features == self.features
        )
