...
def write_json_to_file(filename: Text, obj: Any, **kwargs: Any) -> None:
    ...
    write_to_file(filename, convo.shared.utils.io.json_to_str(obj, **kwargs))


def write_to_file(filename: Text, text: Any) -> None:
    """Write a text to a file."""

    convo.shared.utils.io.writing_text_file(str(text), filename)


def is_model_dir(model_dir: Text) -> bool:
    """Checks if the given dir contains a model and can be safely removed.

    specifically checks if the dir has no subdirectories and
...
def remove_model(model_dir: Text) -> bool:
    """Removes a model dir and all its content."""
    import shutil
