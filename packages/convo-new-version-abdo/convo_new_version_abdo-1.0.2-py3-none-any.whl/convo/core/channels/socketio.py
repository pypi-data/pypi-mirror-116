...
class ChannelBluePrint(Blueprint):
...
class ChannelIoOutput(OutputSocket):
    @classmethod
    def name(cls) -> Text:
        return "socketio"
    ...
    async def _sent_msg(self, socket_id: Text, response: Any) -> None:
    ...
    async def send_text_message(
        ...
        for message_part in text.strip().split("\n\n"):
            await self._sent_msg(recipient_id, {"text": message_part})
    ...
    async def sent_img_url(
        ...
        msg = {"attachment": {"type": "image", "payload": {"src": image}}}
        await self._sent_msg(recipient_id, msg)
    ...
    async def send_text_with_buttons(
        ...
        **kwargs: Any,
        ...
        # replies to
        msg_part = text.strip().split("\n\n") or [text]
        msg = [{"text": message, "quick_replies": []} for message in msg_part]

        # attach all buttons to the last text fragment
        for button in buttons:
            msg[-1]["quick_replies"].append(
                {
                    "content_type": "text",
                    "title": button["title"],
        ...
        for message in msg:
            await self._sent_msg(recipient_id, message)

    async def sent_details(
    ...
    async def send_attachment(
        ...
        await self._sent_msg(recipient_id, {"attachment": attachment})


class ChannelIoInput(InputSocket):
    ...
    def fetch_output_channel(self) -> Optional["OutputSocket"]:
        if self.sio is None:
            convo.shared.utils.io.raising_warning(
                ...
                "workers or multiple Convo Open Source instances. "
                "Please use a different channel for external events in these "
                "scenarios."
            )
            return
        return ChannelIoOutput(self.sio, self.bot_message_evt)
    ...
    def blue_print(
        ...
        socketio_webhook = ChannelBluePrint(
            sio, self.socketio_path, "socketio_webhook", __name__
        )

        # make sio object static to use in fetch_output_channel
        self.sio = sio
        ...
        async def connect(sid: Text, _) -> None:
            log.debug(f"User {sid} connected to socketIO endpoint.")

        @sio.on("disconnect", namespace=self.namespace)
        async def disconnect(sid: Text) -> None:
            log.debug(f"User {sid} disconnected from socketIO endpoint.")
        ...
        async def handle_msg(sid: Text, data: Dict) -> Any:
            output_channel = ChannelIoOutput(sio, self.bot_message_evt)
