...
from convo_sdk.interfaces import Tracker, ActionNotFoundException, Action

from convo_sdk import utils

if typing.TYPE_CHECKING:  # pragma: no cover
    from convo_sdk.types import ActCall
...
time_stamp_module = namedtuple("TimestampModule", ["timestamp", "module"])


class ActExecutor:
    def __init__(self) -> None:
        self.actions: Dict[Text, Callable] = {}
        self._modules: Dict[Text, time_stamp_module] = {}
        self._loaded: Set[Type[Action]] = set()

    def register_action(self, act: Union[Type[Action], Action]) -> None:
        if inspect.isclass(act):
            act = cast(Type[Action], act)
            if act.__module__.startswith("convo."):
                log.warning(f"Skipping built in Action {act}.")
                return
            else:
                # Check if this class has already been loaded in the past.
                if act in self._loaded:
                    return

                # Mark the class as "loaded"
                self._loaded.add(act)
                act = act()

        if isinstance(act, Action):
            self.register_function(act.name(), act.run)
        else:
            raise Exception(
    ...
    def register_function(self, name: Text, f: Callable) -> None:
        ...
        if len(keys_valid) < 3:
            raise Exception(
                ...
                f"tracker, domain. Your function accepts only {len(keys_valid)} "
                "parameters."
            )

        if name in self.actions:
            log.info(f"Re-registered function for '{name}'.")
        else:
            log.info(f"Registered function for '{name}'.")
    ...
    def _import_submodules(
        ...
        for loader, name, is_pkg in pkgutil.walk_packages(package.__path__):  # type: ignore  # mypy issue #1422
            ...
            if recursive and is_pkg:
                self._import_submodules(fullname)
    ...
    def _import_module(self, name: Text) -> types.ModuleType:
        ...
        mod_file = getattr(module_name, "__file__", None)
        if mod_file:
            # If the module we're importing is a namespace package (a package
            # without __init__.py), then there's nothing to watch for the
            # package itself.
            time_stamp = os.path.getmtime(mod_file)
            self._modules[mod_file] = time_stamp_module(time_stamp, module_name)
    ...
    def register_package(self, package: Union[Text, types.ModuleType]) -> None:
        ...
        try:
            ...
            log.exception(f"Failed to register package '{package}'.")
            sys.exit(1)
    ...
    def _register_all_actions(self) -> None:
        ...
        act = utils.all_sub_classes(Action)

        for action in act:
            if (
                not action.__module__.startswith("convo_core.")
                and not action.__module__.startswith("convo.")
                and not action.__module__.startswith("convo_sdk.")
                and not action.__module__.startswith("convo_core_sdk.")
                and not inspect.isabstract(action)
            ):
                self.register_action(action)

    def _find_modules_to_reload(self) -> Dict[Text, time_stamp_module]:
        ...
        for path, (timestamp, module) in self._modules.items():
            try:
                time_stamp_new = os.path.getmtime(path)
            except OSError:
                # Ignore missing files
                continue

            if time_stamp_new > timestamp:
                to_rld[path] = time_stamp_module(time_stamp_new, module)
    ...
    def reload(self) -> None:
        ...
        be registered.
        """
        to_rld = self._find_modules_to_reload()
        other_mod_rlded = False

        for path, (timestamp, module) in to_rld.items():
            try:
                mod_new = importlib.reload(module)
                self._modules[path] = time_stamp_module(timestamp, mod_new)
                log.info(
                    f"Reloaded module/package: '{module.__name__}' "
                    f"(file: '{os.path.relpath(path)}')"
                )
                other_mod_rlded = True
            except (SyntaxError, ImportError):
                log.exception(
                    f"Error while reloading module/package: '{module.__name__}' "
                    f"(file: '{os.path.relpath(path)}'):"
                )
                log.info("Please fix the error(s) in the Python file and try again.")
            ...
            if isinstance(event, dict):
                if not event.get("event"):
                    log.error(
                        ...
                        "the helpers in `convo_sdk.events`! Event will"
                        f"be ignored!"
                    )
                    log.debug(f"Contents of ignored event: '{event}'")
                else:
                    valid.append(event)
            elif type(event).__module__ == "convo.core.events":
                warnings.warn(
                    "Your action should not return Convo actions within the "
                    "SDK. Instead of using events from "
                    "`convo.core.events`, you should use the ones "
                    "provided in `convo_sdk.events`! "
                    "We will try to make this work, but this "
                    "might go wrong!"
                )
                valid.append(event.as_dict())  # type: ignore
            else:
                log.error(
                ...
                log.debug(f"Contents of ignored event: '{event}'")
                # we won't append this to valid events -> will be ignored
        return valid

    async def run(self, action_call: "ActCall") -> Optional[Dict[Text, Any]]:
        from convo_sdk.interfaces import Tracker

        action_name = action_call.get("next_action")
        if action_name:
            log.debug(f"Received request to run '{action_name}'")
            action = self.actions.get(action_name)
            if not action:
                raise ActionNotFoundException(action_name)

            json_trackr = action_call["tracker"]
            domain_name = action_call.get("domain", {})
            tracker = Tracker.from_dict(json_trackr)
            fetch_dispatcher = CollectingDispatcher()

            if utils.co_routine_action_check(action):
                events_name = await action(fetch_dispatcher, tracker, domain_name)
            else:
                events_name = action(fetch_dispatcher, tracker, domain_name)

            if not events_name:
                # make sure the action did not just return `None`...
                events_name = []

            events_validated = self.validate_events(events_name, action_name)
            log.debug(f"Finished running '{action_name}'")
            return self._create_api_response(events_validated, fetch_dispatcher.messages)

        log.warning("Received an action call without an action.")
        return None
