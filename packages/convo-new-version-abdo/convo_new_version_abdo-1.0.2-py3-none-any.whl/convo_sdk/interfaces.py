...
if typing.TYPE_CHECKING:  # pragma: no cover
    from convo_sdk.types import DomainDictionary, TracerState

from convo_sdk.events import EventType

log = logging.getLogger(__name__)

ACT_LISTEN_NAME = "action_listen"
...
class Tracker:
    ...
    @classmethod
    def from_dict(cls, state: "TracerState") -> "Tracker":
    ...
    def last_executed_action_has(self, name: Text, skip: int = 0) -> bool:
        last = self.get_last_event_for(
            "action", exclude=[ACT_LISTEN_NAME], skip=skip
        )
    ...
    def slots_to_validate(self) -> Dict[Text, Any]:
        ...
        for event in reversed(self.events):
            # The `FormAction` in Convo Open Source will append all slot candidates
...
class Action:
    ...
    async def run(
        ...
        Returns:
            A dictionary of ``convo_sdk.events.Event`` instances that is
...
class ActExecutionRejection(Exception):
