...
from convo_sdk.events import SlotSet, EventType, active_loop
from convo_sdk.interfaces import Action, ActExecutionRejection

log = logging.getLogger(__name__)

if typing.TYPE_CHECKING:  # pragma: no cover
    from convo_sdk import Tracker
...
SLOT_REQUESTED = "requested_slot"

KEY_LOOP_INTERRUPTION = "is_interrupted"


class FormAction(Action):
    def __init__(self):
        warnings.warn(
            "Using the `FormAction` class is deprecated as of Convo Open "
            "Source version 2.0. Please see the migration guide "
            "for Convo Open Source 2.0 for instructions how to migrate.",
            FutureWarning,
        )
        super().__init__()
    ...
    def extract_other_slots(
        ...
        """
        slot_to_fill = tracker.get_slot(SLOT_REQUESTED)
            ...
            if slot != slot_to_fill:
                ...
                for other_slot_mapping in other_slot_mappings:
                    ...
                    if value is not None:
                        log.debug(f"Extracted '{value}' for extra slot '{slot}'.")
    ...
    def extract_requested_slot(
        ...
        """Extract the val of requested slot from a user input
        else return None
        """
        log.debug(f"Trying to extract requested slot '{slot_to_fill}' ...")

        # get mapping for requested slot
        requested_slot_mappings = self.get_mappings_for_slot(slot_to_fill)

        for requested_slot_mapping in requested_slot_mappings:
            log.debug(f"Got mapping '{requested_slot_mapping}'")
        ...
        log.debug(f"Failed to extract requested slot '{slot_to_fill}'")
        return {}
    ...
    async def validate(
        ...
        log.debug(f"Validating extracted slots: {slot_values}")
    ...
    def request_next_slot(
        ...
        for slot in self.required_slots(tracker):
            if self._should_request_slot(tracker, slot):
                log.debug(f"Request next slot '{slot}'")
                dispatcher.utter_message(template=f"utter_ask_{slot}", **tracker.slots)
                return [SlotSet(SLOT_REQUESTED, slot)]
    ...
    def deactivate(self) -> List[EventType]:
        ...
        log.debug(f"Deactivating the form '{self.name()}'")
        return [active_loop(None), SlotSet(SLOT_REQUESTED, None)]
    ...
    async def _activate_if_required(
        ...
        if tracker.active_loop.get("name") is not None:
            log.debug(f"The form '{tracker.active_loop}' is active")
        else:
        ...
        if tracker.active_loop.get("name") == self.name():
            ...
            log.debug(f"Activated the form '{self.name()}'")
            events = [active_loop(self.name())]
            ...
            if prefilled_slots:
                log.debug(f"Validating pre-filled required slots: {prefilled_slots}")
                events.extend(
                ...
                log.debug("No pre-filled required slots to validate.")
    ...
    async def _validate_if_required(
        ...
        need_validation = not tracker.active_loop or (
            ...
            and not tracker.active_loop.get(KEY_LOOP_INTERRUPTION, False)
        )
        if need_validation:
            log.debug(f"Validating user input '{tracker.latest_message}'")
    ...
    async def run(
        ...
        if active_loop(None) not in events:
            ...
            if next_slot_events is not None:
                ...
                log.debug(f"Submitting the form '{self.name()}'")
...
class FormValidationAction(Action, ABC):
    ...
    def slots_mapped_in_domain(self, domain: "DomainDictionary") -> List[Text]:
    ...
    async def next_requested_slot(
        ...
        if required_slots and required_slots == self.slots_mapped_in_domain(domain):
            ...
            # within Convo Open Source request the next slot.
            return None
        ...
        return SlotSet(SLOT_REQUESTED, next(missing_slots, None))
