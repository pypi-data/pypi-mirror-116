...
class IntCodec(Codec):
    ...
    def __repr__(self):
        return "IntCodec" + repr(self._range)[5:]
...
class TableCodec(Codec):
    ...
    def encode(self, table) -> str: # pylint: disable=missing-function-docstring
        if len(table) < 2:
            raise ValueError("Table requires at least 2 rows")
        columns = len(table[0])
        if columns < 2:
            raise ValueError("Table requires at least 2 columns")
...
def _form_parse(codecs, root):
    ...
    def integer(param):
        ...
        return IntCodec(minimum, maximum)
    ...
    def real(param):
        ...
        unit = param.get('unit')
        return RealCodec(minimum, maximum, unit)
