...
import sys
from PMAgg.ui import pmagg_ui, axis_edit_manager, default_setting_manager, legend_setting, \
...
__version__ = '1.0.0'
__description__ = 'A matplotlib GUI backend with interactive capabilities'
__author__ = 'jcl'
__author_email__ = '2195932461@qq.com'
__url__ = 'https://gitee.com/lvvl/pmagg'
...
class Window(QtWidgets.QMainWindow, pmagg_ui.Ui_MainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.config.value('font/gui_font'):
            font = QtGui.QFont()
            font.setFamily(str(self.config.value('font/gui_font')))
            self.app.setFont(font)
        # 设置图标
        self.icon_path = os.path.join(self.current_path, 'icons/science.png')
        icon = QtGui.QIcon()
        ...
        self.relative_pos = (0, 0)  # 记录相对位置，默认是0
    ...
    def get_canvas(self, figure):
        if not self.canvas:
            ...
            self.init_figure()
            self.canvas.draw()
        self.setWindowTitle('Figure {0}'.format(Window._PMAGG_FIG_NUM))
        Window._PMAGG_FIG_NUM += 1
    ...
    def help_slot(self):
        webbrowser.open("https://gitee.com/lvvl/pmagg")
...
class FigureManagerPMAgg(FigureManagerBase):
...
def new_figure_manager_given_figure(num, figure):
    ...
    manager = FigureManagerPMAgg(canvas, num)
    return manager
