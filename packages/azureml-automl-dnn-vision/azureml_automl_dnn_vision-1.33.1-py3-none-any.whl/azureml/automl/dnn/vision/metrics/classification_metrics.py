...
class ClassificationMetrics:
    ...
    def __init__(self, labels, multilabel=False):
        ...
        self._unsupported_metrics.add(constants.NORM_MACRO_RECALL)
        self._unsupported_metrics.add(constants.WEIGHTED_ACCURACY)
        self._unsupported_metrics.add(constants.BALANCED_ACCURACY)
        self._unsupported_metrics.add(constants.MATTHEWS_CORRELATION)
        self._unsupported_metrics.add(constants.LOG_LOSS)  # We use the log loss from System Meter

        if self._unsupported_metrics:
            logger.warning("Metrics not supported in configuration: {}."
                           .format(self._unsupported_metrics))
