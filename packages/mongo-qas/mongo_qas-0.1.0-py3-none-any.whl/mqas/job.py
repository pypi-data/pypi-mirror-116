...
class Job:
  ...
  def complete(self, result=None):
    ...
    if "on_success" in self.payload:
      ...
      try:
        ...
        self.logger("error", error_message=f"Error: {ex}")

    self.logger("completed", result=result)
    return True
  ...
  def error(self, msg=None):   
    ...
    self.logger("error", error_message=msg)
    return True

  def log(self, type, *args, **kwargs):
    if self.logger is None:
      if self.verbosity.__contains__(type):
        payload = self.payload
  ...
  def progress(self, percent, msg=None):
    ...
    self.logger("progress", percent=percent, progress_message=msg)
    return True
