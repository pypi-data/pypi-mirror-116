...
from jaison.helpers.templating import call, inline_dict, align
import autopep8
from jaison.api import dtype
import numpy as np
from jaison.api.types import (JsonAI, TypeInformation, StatisticalAnalysis, Feature,
...
def add_implicit_values(json_ai: JsonAI) -> JsonAI:
    ...
    imports = [
        'from jaison.model import Neural', 'from jaison.model import LightGBM',
        'from jaison.model import LightGBMArray', 'from jaison.model import SkTime',
        'from jaison.model import Unit', 'from jaison.model import Regression',
        'from jaison.ensemble import BestOf', 'from jaison.data import cleaner',
        'from jaison.data import transform_timeseries, timeseries_analyzer', 'from jaison.data import splitter',
        'from jaison.analysis import model_analyzer, explain',
        'from sklearn.metrics import r2_score, balanced_accuracy_score, accuracy_score', 'import pandas as pd',
        'from jaison.helpers.seed import seed', 'from jaison.helpers.log import log', 'import jaison',
        'from jaison.api import *', 'from jaison.model import BaseModel',
        'from jaison.encoder import BaseEncoder, __ts_encoders__',
        'from jaison.encoder import Array, Binary, Categorical, Date, Datetime, Float, Image, Integer, Quantity, Rich_Text, Short_Text, Tags', # noqa
        'from jaison.ensemble import BaseEnsemble', 'from typing import Dict, List',
        'from jaison.helpers.parallelism import mut_method_call',
        'from jaison.data.encoded_ds import ConcatedEncodedDs', 'from jaison import ProblemDefinition']
    ...
    for feature in [list(json_ai.outputs.values())[0], *json_ai.features.values()]:
        ...
        if '.' in encoder_import:
            continue
        imports.append(f'from jaison.encoder import {encoder_import}')

    if problem_definition.timeseries_settings.use_previous_target:
        imports.append('from jaison.encoder import TimeSeriesPlainEncoder')
...
self.statistical_analysis = jaison.data.statistical_analysis(data, self.dtype_dict, {json_ai.identifiers},
...
encoded_ds_arr = jaison.encode(self.encoders, folds, self.target)
...
encoded_ds = jaison.encode(self.encoders, data, self.target)[0]
...
"""
    ...
    predictor_code = f"""
{imports}
from jaison.api import PredictorInterface
...
def validate_json_ai(json_ai: JsonAI) -> bool:
    from jaison.api.high_level import predictor_from_code, code_from_json_ai
    try:
