...
from jaison.constants.jaison import ColumnDataTypes
...
class jaisonEnsemble:
    def __init__(self, predictors=None, load_from_path=None):
        self.path_list = None
        if load_from_path is not None:
            with open(os.path.join(load_from_path, 'jaison_data'), 'rb') as pickle_in:
    ...
    def save(self, path_to):
        ...
        for i, model in enumerate(self.ensemble):
            path = os.path.join(path_to, f'jaison_predictor_{i}')
        ...
        with open(os.path.join(path_to, 'jaison_data'), 'wb') as file:
