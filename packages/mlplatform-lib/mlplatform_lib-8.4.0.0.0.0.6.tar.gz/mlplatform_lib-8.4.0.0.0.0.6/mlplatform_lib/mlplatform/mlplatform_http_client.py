...
class MlPlatformHttpClient:
    ...
    def send_request(
        ...
        if request_type == MlPlatformRequestType.READ:
            ...
            if isinstance(data, Path):
                response = requests.post(url, headers=headers, files={"form_field_name": open(data, "rb")})
            else:
    ...
    def insert_model_info(
        ...
        model_info.finished_time = datetime.now().strftime("%d:%m:%Y %H:%M:%S")
        ...
        return ModelInfoDto.from_dict(result.data)
