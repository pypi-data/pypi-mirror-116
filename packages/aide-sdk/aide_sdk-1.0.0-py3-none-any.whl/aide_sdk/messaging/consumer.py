...
class ModelConsumer(threading.Thread):
    def __init__(self, config: QueueConfiguration):
        ...
        self.on_consumption_callback = None

    def set_callback(self, callback):
        self.on_consumption_callback = callback
    ...
    def _on_queue_declareok(self, _unused_frame, userdata):
        ...
        """
        queue_name = userdata
        logger.info('Binding %s to %s with %s', self.config.model_exchange,
                    queue_name,
    ...
    def _run_message_callback(self, delivery_tag, body):
        logger.info('Running message execution %s', delivery_tag)
        self.on_consumption_callback(body)
