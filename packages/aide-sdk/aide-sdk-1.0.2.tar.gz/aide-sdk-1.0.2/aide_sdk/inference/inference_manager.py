...
class InferenceManager:
    def __init__(self, consumer: ModelConsumer,
    ...
    def monitor_adapters(self):
        while True:
            time.sleep(5)  # Checking every 5 seconds
            if self.publisher.is_alive() and not self.consumer.is_alive():
                # Consumer dead
                logger_audit.info("Consumer dead, shutting down.")
                self.publisher.stop()
                break

            if self.consumer.is_alive() and not self.publisher.is_alive():
                # Publisher dead
                logger_audit.info("Publisher dead, shutting down.")
                self.consumer.stop()
                break

            if not self.consumer.is_alive() and not self.publisher.is_alive():
                # Both dead already
                logger_audit.info("Consumer and publisher dead, shutting down.")
                break

        self.consumer.join(timeout=15)
        self.publisher.join(timeout=15)
        logger_audit.info("Consumer & publisher closed or not responding. Wrapping up.")
