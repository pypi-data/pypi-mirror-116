...
class ModelPublisher(threading.Thread):
    ...
    def _on_connection_open_error(self, _unused_connection, err):
        ...
        logger.error('Connection open failed, reconnect in {} seconds. '
                     'Error: {}.'.format(self.queue_config.connection_retry_wait, repr(err)))
        self._reconnect()
    ...
    def _on_connection_closed(self, _unused_connection, reason):
        ...
        if self._stopping:
            ...
            logger.warning('Connection closed, reopening in {} seconds. '
                           'Error: {}'.format(self.queue_config.connection_retry_wait, repr(reason)))
            self._reconnect()

    def _reconnect(self):
        if self._reconnect_counter >= self.queue_config.connection_retry_limit:
            logger.error("Reached reconnect limit. Aborting publisher.")
            self._shutdown()
        else:
            self._reconnect_counter += 1
            # Stopping the loop without setting _stopping will simply recreate a new loop
            time.sleep(self.queue_config.connection_retry_wait)
    ...
    def _publish_message(self, message, correlation_id: str):
        if self._channel is None or not self._channel.is_open:
            logger.error('Could not publish message. '
                         'Channel closed. collaboration_id %s', correlation_id)
            return
    ...
    def publish_message(self, message, correlation_id: str):
        logger.info('Scheduled message for publishing: %s, collaboration_id %s',
                    message, correlation_id)
        cb = functools.partial(self._publish_message, message, correlation_id)
