...
class ChebConvFixed(MessagePassing):
    def __init__(self, coeffs, normalization="sym", **kwargs):
...
def load_data(adj_path, df_path):
    adj = load_npz(adj_path)
    adj_coo = adj.tocoo()
    edge_index = torch.tensor([adj_coo.row, adj_coo.col], dtype=torch.long)
    adata = anndata.read_h5ad(df_path)
    X = torch.from_numpy(adata.X.toarray()).to(torch.float).T
    # y = torch.from_numpy(np.array(adata.obs)).to(torch.float)
    return Data(x=X[:, :10], edge_index=edge_index)
...
if __name__ == "__main__":
    ...
    np.random.seed(42)
    n = 1000
    data_dir = "/data/lab/datasets/MIMIC"
    start = time.time()
    data = load_data(
        os.path.join(data_dir, "processed/adjacency_v1.npz"),
        os.path.join(data_dir, "processed/dataframe_small_v1.h5ad"),
    )
    end = time.time()
    print("loaded in %0.2f seconds" % (end - start))
    ...
    output = loss_fn(data)
    end2 = time.time()
    print("ran in %0.2f seconds" % (end2 - end))
    print(output)
    print(input, target)
    print(loss_fn)
