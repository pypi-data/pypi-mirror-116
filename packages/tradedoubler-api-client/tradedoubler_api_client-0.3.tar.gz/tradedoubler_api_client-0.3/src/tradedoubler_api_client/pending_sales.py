...
import json
from tradedoubler_api_client.utilities import progres_bar, save_list_of_dicts_to_csv, save_dict_to_json
...
class Pending_Sales:
    ...
    def get_all(self, start='', end=''):
        ...
        offset = 100
        if total > 100:
            progres_bar(total, offset, f' {offset}/{total}   ')
        while total > offset:
        ...
        progres_bar(10, 10, ' done! :)          ')
        return List_Of_Pending_Sales(all_items)
...
class List_Of_Pending_Sales:
    def __init__(self, list_of_sales):
...
class Updated_Sales:
    ...
    def csv(self, status='fail', path=''):
        if status == 'fail':
            ...
            save_list_of_dicts_to_csv(self.get_fails(), filename, path)
        elif status == 'success':
            filename = f'update-report-successes-{datetime.strftime(datetime.now(), "%Y-%m-%d")}.csv'
            save_list_of_dicts_to_csv(self.get_success(), filename, path)
        else:
            filename = f'update-report-all-{datetime.strftime(datetime.now(), "%Y-%m-%d")}.csv'
            save_list_of_dicts_to_csv([{"status": '', "transactionId": '', "action": '', "reason": '', "error": ''}] + self.items, filename, path)

    def json(self, status='fail', path=''):
        if status == 'fail':
            filename = f'update-report-fails-{datetime.strftime(datetime.now(), "%Y-%m-%d")}.json'
            save_dict_to_json(self.get_fails(), filename, path)
        elif status == 'success':
            filename = f'update-report-successes-{datetime.strftime(datetime.now(), "%Y-%m-%d")}.json'
            save_dict_to_json(self.get_success(), filename, path)
        else:
            filename = f'update-report-all-{datetime.strftime(datetime.now(), "%Y-%m-%d")}.json'
            save_dict_to_json({'total': self.total, 'success': self.success, 'fail': self.fail, 'items': self.items}, filename, path)
