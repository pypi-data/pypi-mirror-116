            ...
            if background == None:
                with Image.open(BytesIO(await gettemplate("normal"))).convert('RGB') as background:
                    ...
                    if len(("{} / {} XP".format(currentxp, nextxp))) > 8:
                        ...
                        xqaxis = 740 - (23*changeaxis)
                    draw.text((xqaxis, 55), "Rank #{} | Level {}".format(rank, level), (255, 255, 255), font=medium_font)
                ...
                with Image.open(BytesIO(await getbackground(background))) as background:
                    ...
                    xyaxis = 740
                    changeaxis = len("Rank #{} | Level {}".format(rank, level)) - 6
                    xyaxis = 740 - (23*changeaxis)
                    draw.text((xyaxis, 55), "Rank #{} | Level {}".format(rank, level), (255, 255, 255), font=medium_font)
                    if isinstance(user, discord.Member) or isinstance(user, discord.User):
                        colour = user.colour.to_rgb()
                        if colour == (0, 0, 0):
                            colour = (37, 115, 189)
