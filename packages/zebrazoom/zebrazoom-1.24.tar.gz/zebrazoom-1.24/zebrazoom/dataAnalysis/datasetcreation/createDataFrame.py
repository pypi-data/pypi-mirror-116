...
def createDataFrame(dataframeOptions):
  ...
  frameStepForDistanceCalculation = int(dataframeOptions['frameStepForDistanceCalculation']) if 'frameStepForDistanceCalculation' in dataframeOptions else 4
  ...
  # Global parameters
  globParam  = ['BoutDuration', 'TotalDistance', 'Speed', 'NumberOfOscillations', 'meanTBF', 'maxAmplitude', 'deltaHead', 'xstart', 'xend', 'xmean', 'firstBendTime', 'firstBendAmplitude', 'IBI']
  # Initial raw data
  rawData = ['HeadX', 'HeadY', 'Heading', 'TailAngle_Raw', 'TailAngle_smoothed', 'Bend_Timing', 'Bend_TimingAbsolute', 'Bend_Amplitude']
  ...
  # Creating an empty dataframe that will contain the parameters for the whole set of videos
  dfParam = pd.DataFrame(columns=dfCols)
  dfParam = dfParam.astype({'Trial_ID': str})
  if True:
    toConvert = {param: object for param in rawData}
    dfParam = dfParam.astype(toConvert)
  ...
  for videoId in range(0, len(excelFile)):
    ...
    for Well_ID, Cond in enumerate(condition):
      if include[Well_ID]:
        ...
        dfParamForWell = dfParamForWell.astype({'Trial_ID': str})
        if True:
          toConvert = {param: object for param in rawData}
          dfParamForWell = dfParamForWell.astype(toConvert)
        curBoutId = 0
          ...
          for NumBout, dataForBout in enumerate(supstruct["wellPoissMouv"][Well_ID][fishId]):
            if not("flag" in dataForBout) or dataForBout["flag"] == 0:
              ...
              if "Bend_Timing" in dataForBout and type(dataForBout["Bend_Timing"]) == list and len(dataForBout["Bend_Timing"]) >= minNbBendForBoutDetect:
                ...
                toPutInDataFrame       = toPutInDataFrame       + gatherInitialRawData(dataForBout, rawData)
