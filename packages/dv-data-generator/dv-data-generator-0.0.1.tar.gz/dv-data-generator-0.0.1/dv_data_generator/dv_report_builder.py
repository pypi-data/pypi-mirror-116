...
from dv_data_generator.double_click_bid_manager import DoubleClickBidManager
from dv_data_generator.assertions import is_list_of_string
...
class DvReportBuilder(DoubleClickBidManager):
    ...
    def set_timeframe(
        ...
        self._query["reportDataStartTimeMs"] = (start_date.timestamp(),)
        self._query["reportDataEndTimeMs"] = (end_date.timestamp(),)

        self._query["metadata"]["dataRange"] = "CUSTOM_DATERANGE"

        return self

    def execute(self):
        return super().execute_query(self._query)
...
if __name__ == "__main__":
    a = DvReportBuilder().set_metrics([""]).set_dimensions([""]).query
    print(a)
