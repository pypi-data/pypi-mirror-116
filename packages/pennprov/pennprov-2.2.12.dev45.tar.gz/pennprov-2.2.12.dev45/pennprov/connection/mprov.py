...
class MProvConnection:
    ...
    def __init__(self, user=None, password=None, host=None):
        ...
        check_cache_thread = threading.Thread(target=self.check_cache)
    ...
    def check_cache(self):
        ...
        while True:
            ...
            last_flush = self.log.last_flush
            diff = now - last_flush if last_flush else datetime.timedelta(seconds=check_secs + 1)
            print(f'CHECKING {id(self)} CACHE')
            if diff.total_seconds() > check_secs:
                self.flush()
                print(f'TIMED {id(self)} FLUSH')
