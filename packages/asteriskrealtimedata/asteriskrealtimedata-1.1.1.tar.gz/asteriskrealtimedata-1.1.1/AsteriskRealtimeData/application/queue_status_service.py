...
class QueueStatusService:
    ...
    @inject()
    def list_queue_status(self, repository: Provide[QueueStatusRepository]) -> list[QueueStatusVo]:
        result: list = []
    ...
    def get_by_search_criteria(
        self, search_criteria: dict, repository: Provide[QueueStatusRepository]
    ) -> QueueStatusVo:
        queue_status = repository.get_by_criteria(search_criteria)
        return QueueStatusVo(status_code=queue_status["status_code"], description=queue_status["description"],)
