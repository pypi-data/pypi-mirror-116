...
class CallService:
    ...
    @inject()
    def list_call(self, repository: Provide[CallRepository]) -> list[CallVo]:
        result: list = []
    ...
    @inject
    def get_by_search_criteria(self, search_criteria: dict, repository: Provide[CallRepository]) -> CallVo:
        call = repository.get_by_criteria(search_criteria)
        return CallVo(
            peer_name=call["peer_name"],
            client_id=call["client_id"],
            dialnumber=call["dialnumber"],
    ...
    @inject
    def delete_call(self, call_linkedid: str, repository: Provide[CallRepository]) -> CallVo:
        repository.delete_by_criteria({"call_linkedid": call_linkedid})
        return CallVo(
            call_linkedid=call_linkedid,
            peer_name="",
            client_id="",
            dialnumber="",
            lastevent="",
