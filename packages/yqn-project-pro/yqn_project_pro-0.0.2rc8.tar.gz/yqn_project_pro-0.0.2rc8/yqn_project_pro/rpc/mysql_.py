...
from yqn_project_pro.utils.strings import parse_mysql_string
...
class MySQLClient(RPCConfigBase):
    ...
    def connect(self, **kwargs):
        if self.connector is None:
            assert not isinstance(self.client_config, str), "client_config need obj"
            # if self.use_conf_management and isinstance(self.client_config, str):
            #     self.client_config = parse_mysql_string(self.server_manager.get_value(self.client_config))
            if self.use_conf_management:
                self.mysql_username = self.server_manager.get_value(self.client_config.mysql_username)
                self.mysql_password = self.server_manager.get_value(self.client_config.mysql_password)
                self.mysql_host = self.server_manager.get_value(self.client_config.mysql_host)
                self.mysql_port = self.server_manager.get_value(self.client_config.mysql_port)
                self.mysql_database = self.server_manager.get_value(self.client_config.mysql_database)
                self.mysql_charset = self.server_manager.get_value(self.client_config.mysql_charset, "utf8")
            else:
                self.mysql_username = self.client_config.mysql_username
                self.mysql_password = self.client_config.mysql_password
                self.mysql_host = self.client_config.mysql_host
                self.mysql_port = self.client_config.mysql_port
                self.mysql_database = self.client_config.mysql_database
                self.mysql_charset = self.client_config.mysql_charset
        # if self.connector is None:
