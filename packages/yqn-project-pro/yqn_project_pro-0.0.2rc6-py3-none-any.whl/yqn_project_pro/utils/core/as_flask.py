...
from yqn_project_pro.utils.core.parser import DAJsonBase
from yqn_project_pro.utils.core.logger import Logger
...
class Services(object):
    ...
    def __init__(self, d=None):
        if d is None:
            d = {}
        self.__dict__["server"] = d

    def __getattr__(self, key):
        value = self.__dict__["server"][key]
        if isinstance(value, dict):
            return Services(value)
        return value

    def __setattr__(self, key, value):
        self.__dict__["server"].update({key: value})
...
class DAFlask(Flask):
    ...
    def register_services(self, *servers):
        for _ in servers:
            ...
            if isinstance(server, ApolloConfig):
                self.services.__setattr__(_.name, self.server_manager[1])
            else:
                self.services.__setattr__(_.name, server)
