...
class LayerNorm(Module):
    ...
    def build(self, input_shape):
        ...
        if self.gamma_init is not None:
            self.gamma = self._get_weights(var_name='gamma', shape = self.normalized_shape, init=self.gamma_init, order=True)
        ...
        self.layernorm = tl.ops.layernorm(self.normalized_shape, self.gamma, self.beta, self.eps, input_shape)
        self.act_init_flag = False
        if self.act:
            self.act_init_flag = True
...
if __name__ == '__main__':
    input = tl.layers.Input((20,5,10,10))
    l = tl.layers.LayerNorm(normalized_shape=input.shape[1:])
    out = l(input)
