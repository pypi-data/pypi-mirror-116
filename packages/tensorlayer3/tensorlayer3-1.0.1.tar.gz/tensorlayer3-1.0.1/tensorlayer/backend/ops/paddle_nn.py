...
class layernorm(object):
    ...
    def __call__(self, input):
        ...
        output = pd.nn.functional.layer_norm(input, normalized_shape=self.normalized_shape,
                                             weight=scale, bias=offset, epsilon=self.eps)
        return output
...
class multiheadattention(object):
    ...
    def __call__(self, q, k, v, attn_mask, key_padding_mask):
        ...
        if self.batch_first:
            ...
            k = pd.transpose(q, perm=(1,0,2))
            v = pd.transpose(q, perm=(1,0,2))
        ...
        if head_dim * self.num_heads != embed_dim:
            raise ValueError(
                "embedding dimension {} not divisible by num_heads {}".format(embed_dim, self.num_heads)
            )
        ...
        if attn_mask is not None:
            ...
            elif attn_mask.dtype not in ('float32', 'float64', 'bool'):
                raise TypeError("attn_mask tensor dtype should be in ('float32', 'float64', 'bool'),"
                                "but got {}".format(attn_mask.dtype))
            if attn_mask.dim() == 2:
                if attn_mask.shape != (tgt_len, src_len):
                    raise ValueError("The shape of 2D attn_mask should be {}, but got {}.".format((tgt_len, src_len), attn_mask.shape))
                attn_mask = pd.unsqueeze(attn_mask, axis=0)
            elif attn_mask.dim() == 3:
                size_3d = (batch_size * self.num_heads, tgt_len, src_len)
                if attn_mask.shape != size_3d:
                    raise ValueError("The shape of 3D attn_mask should be {}, but got {}.".format(size_3d, attn_mask.shape))
            else:
        ...
        if key_padding_mask is not None:
            if key_padding_mask.shape != (batch_size, src_len):
                raise ValueError(
                    "Expecting key_padding_mask shape is {}, but got {}.".format((batch_size, src_len), key_padding_mask.shape)
                )

            if key_padding_mask.dtype == 'uint8':
                pass
