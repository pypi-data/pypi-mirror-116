...
def tsbasismom(start_date, **kwargs):
    ...
    for i in range(start_index, len(calendar) - 1):
        ...
        data_t['POS'] = data_t['return_diff'] / abs(data_t['return_diff'])
        ...
        data_t = ts_weight(data_t)
        pos = data_t[
            ['TDATE', 'EXCHANGE', 'PCODE', 'UCODE', 'CLOSE', 'POS', 'return_diff', 'CLOSE1', 'RETURN', 'WEIGHT']
        ].rename(columns={'return_diff': 'FACTORVALUE'})
