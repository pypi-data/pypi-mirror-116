...
class Value(Generic[T]):
    ...
    def get(self) -> Any:
        ...
        if self.valueType == ValueType.ARRAY:
            ...
            return result
    def set(self, var : Any):
        if isinstance(var, float):
            self.valueType = ValueType.NUMBER
            self.number = var
        if isinstance(var, str):
            self.valueType = ValueType.TEXT
            self.text = var
        if isinstance(var, datetime):
            self.valueType = ValueType.DATE
            self.date = var
        if isinstance(var, bool):
            self.valueType = ValueType.BOOLEAN
        if isinstance(var, list[list[T]]):
            self.valueType = ValueType.ARRAY
            self.array = []
