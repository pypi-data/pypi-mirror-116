...
@LastEditTime: 2021-08-10 11:04:10
...
class AuthorizeHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        user_base_model = UserBaseModel(context=self)
        invoke_result_data = user_base_model.update_user_info(app_id, act_id, tb_user_id, open_id, user_nick, avatar)
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
        else:
...
class GetCouponPrizeHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        if access_token == "":
            return self.reponse_json_error("error", "未授权请联系客服授权")
        ...
        if not prize_roster_dict or prize_roster_dict['open_id'] != open_id:
            return self.reponse_json_error("error", "奖品不存在")
        ...
        if not tao_coupon_dict or tao_coupon_dict['right_ename'] == "":
            return self.reponse_json_error("error", "奖品不是优惠券,无需领取")
...
class UserAssetListHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        return self.response_json_success(asset_base_model.get_user_asset_list(app_id, act_id, tb_user_id, open_id))
...
class PrizeRosterListHandler(TaoBaseHandler):
    ...
    def get_async(self):
        ...
        user_base_model = UserBaseModel(context=self)
        return self.response_json_success(user_base_model.get_prize_roster_list(app_id, act_id, tb_user_id, open_id, order_no, goods_type, prize_type, logistics_status, prize_status, pay_status, page_size, page_index))
