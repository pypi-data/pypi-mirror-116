...
@LastEditTime: 2021-08-06 12:37:43
                ...
                if len(asset_log_list) > 0:
                    for asset_log in asset_log_list:
                        try:
                            ...
                            asset_warn_notice.request_id = asset_log.request_id
                            ...
                            logger_error.error(f"用户资产负数预警队列{mod_value}异常,json串:{SevenHelper.json_dumps(asset_log)},ex:{ex.decode()}")
                            continue
                else:
                    time.sleep(10)
            except Exception as ex:
