...
@LastEditTime: 2021-08-12 10:54:49
...
class FrameBaseModel():
    ...
    def business_process_executing(self,app_id,act_id,module_id,user_id,login_token,handler_name,check_new_user=False,check_user_nick=True,continue_request_expire=5,acquire_lock_name=""):
        ...
        if continue_request_expire > 0:
            if SevenHelper.is_continue_request(f"request_business_executing:{handler_name}_{act_id}_{module_id}_{user_id}",expire=continue_request_expire) == False:
                invoke_result_data.success = False
        ...
        if act_info_dict["start_date"] != "" and act_info_dict["start_date"] != "1900-01-01 00:00:00":
            if TimeHelper.format_time_to_datetime(now_date) < act_info_dict['start_date']:
                invoke_result_data.success = False
                invoke_result_data.error_code = "error"
                invoke_result_data.error_message = "活动将在" + str(act_info_dict['start_date']) + "开启"
                return invoke_result_data
        if act_info_dict["end_date"] != "" and act_info_dict["end_date"] != "1900-01-01 00:00:00":
            if TimeHelper.format_time_to_datetime(now_date) > act_info_dict['end_date']:
                invoke_result_data.success = False
                invoke_result_data.error_code = "error"
        ...
        if module_id > 0:
            ...
            if act_module_dict['start_date'] != "" and act_module_dict['start_date'] != "1900-01-01 00:00:00":
                if TimeHelper.format_time_to_datetime(now_date) < act_module_dict['start_date']:
                    invoke_result_data.success = False
                    invoke_result_data.error_code = "error"
                    invoke_result_data.error_message = "活动将在" + str(act_module_dict['start_date']) + "开启"
                    return invoke_result_data
            if act_module_dict['end_date'] != "" and act_module_dict['end_date'] != "1900-01-01 00:00:00":
                if TimeHelper.format_time_to_datetime(now_date) > act_module_dict['end_date']:
                    invoke_result_data.success = False
                    invoke_result_data.error_code = "error"
                    invoke_result_data.error_message = "活动已结束"
                    return invoke_result_data
        ...
        if check_new_user == True and user_info_dict["is_new"] == 0:
                invoke_result_data.success = False
                invoke_result_data.error_code = "error"
                invoke_result_data.error_message = "新用户才能领取"
                return invoke_result_data
