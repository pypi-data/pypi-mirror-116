...
@LastEditTime: 2021-08-02 09:39:09
...
class StatBaseModel():
    ...
    def get_stat_report_list(self,act_id,module_id,start_date,end_date,order_by="sort_index asc"):
        ...
        :last_editors: HuangJianYi
        """
        condition = "((act_id=%s and module_id=%s) or (act_id=0 and module_id=0)) and is_show=1"
        ...
        stat_orm_list = StatOrmModel(context=self.context).get_list(condition, order_by=order_by, params=params)
        ...
        behavior_report_list = stat_report_model.get_dict_list(f"key_name in({key_name_s}))", group_by="key_name", field="key_name,SUM(key_value) AS key_value", params=params)
        #公共映射组（未去重）
    ...
    def get_trend_report_list(self,act_id,module_id,start_date,end_date,order_by="sort_index asc"):
        ...
        key_name_s = ','.join(["'%s'" % str(stat_orm.key_name) for stat_orm in stat_orm_list])
        stat_report_model = StatReportModel(context=self.context)
        stat_report_list = stat_report_model.get_dict_list(f"key_name in({key_name_s}))", field="key_name,key_value,DATE_FORMAT(create_date,'%%Y-%%m-%%d') AS create_date", params=params)
