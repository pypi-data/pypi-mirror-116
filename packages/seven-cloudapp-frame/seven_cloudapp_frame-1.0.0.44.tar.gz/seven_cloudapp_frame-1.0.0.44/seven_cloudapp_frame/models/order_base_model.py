...
@LastEditTime: 2021-08-18 15:13:36
...
class OrderBaseModel(FrameBaseModel):
    ...
    def sync_tao_pay_order(self,app_id,act_id,module_id,user_id,login_token,handler_name,request_id,asset_type=3,goods_id="",sku_id="",ascription_type=1,app_key="",app_secret="",is_log=False,check_user_nick=True,continue_request_expire=5,asset_sub_table=None):
        """
                ...
                if len(order_data)>0: 
                    ...
                    if len(tao_pay_order_list) > 0:
                        ...
                        if result == True and buy_num > 0: 
                            ...
                            only_id =  f"sync_tao_pay_order_{handler_name}_{request_id}" if handler_name and request_id else ""
                            asset_base_model = AssetBaseModel(context=self.context)
                            asset_invoke_result_data = asset_base_model.update_user_asset(app_id,act_id,module_id,user_id,user_info_dict["open_id"],user_info_dict["user_nick"],asset_type,buy_num,asset_object_id,1,"购买","",only_id,handler_name,request_id,info_json={},sub_table=asset_sub_table)
