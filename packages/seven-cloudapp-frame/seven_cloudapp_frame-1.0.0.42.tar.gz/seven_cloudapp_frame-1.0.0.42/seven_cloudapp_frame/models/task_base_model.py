...
@LastEditTime: 2021-08-17 15:55:15
...
class TaskBaseModel(FrameBaseModel):
    ...
    def get_client_task_list(self,app_id,act_id,module_id,user_id,task_types="",app_key="",app_secret="",is_log=False,task_sub_table=None,):
        ...
        for task_info_dict in task_info_list:
            ...
            result["reward_value"] = int(task_config["reward_value"]) if task_config.__contains__("reward_value") else 0
            ...
            #每周签到(status:0当天未签到1当天已签到2已完成)
                ...
                max_sign_day = int(max(task_config, key=task_config.get)) if len(task_config.keys()) > 0 else 0
                if task_count.complete_count >= max_sign_day:
                    result["status"] = 2
                else:
                    result["status"] = 1 if task_count.now_value > 0 else 0
                result["text"] = ["签到", "已签到","已完成"]
                result_list.append(result)
            #邀请新用户
            elif task_info_dict["task_type"] == TaskType.invite_new_user.value:
                limit_num = int(task_config["limit_num"]) if task_config.__contains__("limit_num") else 0
            ...
            #收藏商品
                ...
                if task_info_dict["complete_type"] == 1:
                    ...
                    params = [act_id,module_id,user_id, now_day]
    ...
    def process_receive_reward(self,app_id,act_id,module_id,user_id,login_token,task_id,task_sub_type,handler_name,request_id,check_new_user=False,check_user_nick=True,continue_request_expire=5,task_sub_table=None,asset_sub_table=None,invite_sub_table=None,collect_sub_table=None,browse_sub_table=None):
        """
        ...
        try:
            ...
            if invoke_result_data.success == True:
                ...
                if task_invoke_result_data.success == True:
                    ...
                    satisfy_num = task_config["satisfy_num"] if task_config.__contains__("satisfy_num") else 1
                    limit_num = task_config["limit_num"] if task_config.__contains__("limit_num") else 1
                    ...
                    if reward_value <= 0 or task_count.complete_count <= 0 or task_count.now_count <=0:
                        ...
                        if task_sub_type and task_type == int(TaskType.arbitrarily_consumption.value) or task_type == int(TaskType.totle_consumption.value):
                            sub_task_config = [sub_task_config for sub_task_config in task_config["reward_list"] if str(sub_task_config["key"]) == str(task_sub_type)]
                            if not sub_task_config or len(sub_task_config) <= 0:
                                invoke_result_data.success = False
                                invoke_result_data.error_code = "error"
                                invoke_result_data.error_message = "任务不存在,无法领取奖励"
                            else:
                                sub_task_config = sub_task_config[0]
                                reward_value = int(sub_task_config["reward_value"])
                                if sub_task_config.__contains__("money"):
                                    info_json["sub_task_config"] = sub_task_config
                                    info_json["gear_price"] = str(sub_task_config["money"])
                                    remark = SevenHelper.json_loads(task_count.remark)
                                    info_json["order_price"] = remark["order_price"] if remark.__contains__("order_price") else 0
                                    info_json["goods_ids"] = remark["goods_ids"] if remark.__contains__("goods_ids") else ""
                                    info_json["title"] = "达成任务档位：" + info_json["gear_price"] + ",订单金额：" + info_json["order_price"] + "元"
                                    if info_json["goods_ids"]!="":
                                        info_json["title"] += ",商品ID:" + info_json["goods_ids"]
                                    log_title = "消费满"+info_json["gear_price"]+"任务"
                                else:
                                    invoke_result_data.success = False
                                    invoke_result_data.error_code = "error"
                                    invoke_result_data.error_message = "任务不存在,无法领取奖励"           
                        elif task_type == int(TaskType.invite_new_user.value) or task_type == int(TaskType.invite_join_member.value):     
                            ...
                            #获取邀请用户数
                            if task_info_dict["complete_type"] == 1:
                                invite_list = invite_log_model.get_list("act_id=%s and module_id=%s and user_id=%s and is_handle=0 and create_day=%s",limit=str(cur_complete_count * satisfy_num), params=[act_id,module_id,user_id, now_day])
                            elif task_info_dict["complete_type"] == 2:
                                start_day = int(TimeHelper.get_first_day_of_the_week().strftime('%Y%m%d'))
                                end_day = int(TimeHelper.get_last_day_of_the_week().strftime('%Y%m%d'))
                                invite_list = invite_log_model.get_list("act_id=%s and module_id=%s and user_id=%s and is_handle=0 and create_day>=%s and create_day<=%s",limit=str(cur_complete_count * satisfy_num), params=[act_id,module_id,user_id, start_day,end_day])
                            else:
                                invite_list = invite_log_model.get_list("act_id=%s and module_id=%s and user_id=%s and is_handle=0",limit=str(cur_complete_count * satisfy_num), params=[act_id,module_id,user_id])
                            ...
                            if task_type == int(TaskType.collect_goods.value):
                                ...
                                #获取邀请用户数
                                if task_info_dict["complete_type"] == 1:
                                    collect_list = collect_log_model.get_list("act_id=%s and module_id=%s and user_id=%s and is_handle=0 and create_day=%s",limit=str(cur_complete_count * satisfy_num), params=[act_id,module_id,user_id, now_day])
                                elif task_info_dict["complete_type"] == 2:
                                    start_day = int(TimeHelper.get_first_day_of_the_week().strftime('%Y%m%d'))
                                    end_day = int(TimeHelper.get_last_day_of_the_week().strftime('%Y%m%d'))
                                    collect_list = collect_log_model.get_list("act_id=%s and user_id=%s and is_handle=0 and create_day>=%s and create_day<=%s",limit=str(cur_complete_count * satisfy_num), params=[act_id,module_id,user_id, start_day,end_day])
                                else:
                                    collect_list = collect_log_model.get_list("act_id=%s and user_id=%s and is_handle=0",limit=str(cur_complete_count * satisfy_num), params=[act_id,module_id,user_id])
                                ...
                                #获取邀请用户数
                                if task_info_dict["complete_type"] == 1:
                                    browse_list = browse_log_model.get_list("act_id=%s and module_id=%s and user_id=%s and is_handle=0 and create_day=%s",limit=str(cur_complete_count * satisfy_num), params=[act_id,module_id,user_id, now_day])
                                elif task_info_dict["complete_type"] == 2:
                                    start_day = int(TimeHelper.get_first_day_of_the_week().strftime('%Y%m%d'))
                                    end_day = int(TimeHelper.get_last_day_of_the_week().strftime('%Y%m%d'))
                                    browse_list = browse_log_model.get_list("act_id=%s and user_id=%s and is_handle=0 and create_day>=%s and create_day<=%s",limit=str(cur_complete_count * satisfy_num), params=[act_id,module_id,user_id, start_day,end_day])
                                else:
                                    browse_list = browse_log_model.get_list("act_id=%s and user_id=%s and is_handle=0",limit=str(cur_complete_count * satisfy_num), params=[act_id,module_id,user_id])
                        ...
                        if invoke_result_data.success == True:
                            try:
                                ...
                                self.logging_link_error("process_receive_reward:" + str(ex))
                            ...
                            if reward_value > 0:
                                ...
                                key_list_dict = {}
                                if task_type == int(TaskType.buy_goods.value):
                                    key_list_dict["BuyGoodsUserCount"] = 1 #完成人数
                                    key_list_dict["BuyGoodsCount"] = 1 #完成次数
                                    key_list_dict["BuyGoodsRewardCount"] = reward_value #奖励值
                                elif task_type == int(TaskType.arbitrarily_consumption.value):
                                    key_list_dict["ArbitrarilyConsumptionUserCount"] = 1 #完成人数
                                    key_list_dict["ArbitrarilyConsumptionCount"] = 1 #完成次数
                                    key_list_dict["ArbitrarilyConsumptionRewardCount"] = reward_value #奖励值
                                elif task_type == int(TaskType.totle_consumption.value):
                                    key_list_dict["TotleConsumptionUserCount"] = 1 #完成人数
                                    key_list_dict["TotleConsumptionCount"] = 1 #完成次数
                                    key_list_dict["TotleConsumptionRewardCount"] = reward_value #奖励值
                                elif task_type == int(TaskType.collect_goods.value):
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if task_info_dict["complete_type"] == 1 and task_count.modify_day == now_day:
                        ...
                        if reward_value > 0:
                            ...
                            asset_base_model = AssetBaseModel(context=self.context)
                            asset_invoke_result_data = asset_base_model.update_user_asset(app_id,act_id,module_id,user_id,user_info_dict["open_id"],user_info_dict["user_nick"],asset_type,reward_value,asset_object_id,2,task_type,task_info_dict["task_name"],"",only_id,handler_name,request_id,info_json={},sub_table=asset_sub_table)
                            if asset_invoke_result_data.success == False:
                                reward_value = 0
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if task_info_dict["complete_type"] == 1 and task_count.modify_day == now_day:
                        ...
                        if reward_value > 0:
                            ...
                            asset_type = self.get_task_asset_type(act_info_dict["task_asset_type_json"],task_type)
                            asset_base_model = AssetBaseModel(context=self.context)
                            asset_invoke_result_data = asset_base_model.update_user_asset(app_id,act_id,module_id,user_id,user_info_dict["open_id"],user_info_dict["user_nick"],asset_type,reward_value,asset_object_id,2,task_type,task_info_dict["task_name"],"签到1天",only_id,handler_name,request_id,info_json={},sub_table=asset_sub_table)
                            if asset_invoke_result_data.success == False:
                                reward_value = 0
                        task_count.id = task_count_id
                        task_count.app_id = app_id
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if task_count.modify_day == now_day:
                        ...
                        if reward_value > 0:
                            ...
                            asset_type = self.get_task_asset_type(act_info_dict["task_asset_type_json"],task_type)
                            asset_base_model = AssetBaseModel(context=self.context)
                            asset_invoke_result_data = asset_base_model.update_user_asset(app_id,act_id,module_id,user_id,user_info_dict["open_id"],user_info_dict["user_nick"],asset_type,reward_value,asset_object_id,2,task_type,task_info_dict["task_name"],f"签到{task_count.complete_count}天",only_id,handler_name,request_id,info_json={},sub_table=asset_sub_table)
                            if asset_invoke_result_data.success == False:
                                reward_value = 0
                        invoke_result_data.data = reward_value
                        if reward_value > 0:
    ...
    def process_collect_goods(self,app_id,act_id,module_id,user_id,login_token,goods_id,handler_name,check_user_nick=True,continue_request_expire=5,task_sub_table=None,collect_sub_table=None):
        ...
        try:
            ...
            if invoke_result_data.success == True:
                ...
                is_collect = collect_log_model.get_total("act_id=%s and user_id=%s and goods_id=%s", params=[act_id, user_id, goods_id])
                if is_collect > 0:
                    invoke_result_data.success = False
                    invoke_result_data.error_code = "error"
                    ...
                    if task_invoke_result_data.success == True:
                        ...
                        limit_num = task_config["limit_num"] if task_config.__contains__("limit_num") else 0
                        satisfy_num = int(task_config["satisfy_num"]) if task_config.__contains__("satisfy_num") else 1
                        goods_ids_list = list(set(str(task_config["goods_ids"]).split(','))) if task_config.__contains__("goods_ids") else []
                        task_count_model = TaskCountModel(context=self.context,sub_table=task_sub_table)
                        task_count_id = self._get_task_count_id(act_id,module_id,task_info_dict["task_type"],"",user_id)
                        task_count = task_count_model.get_entity_by_id(where="id=%s",params=[task_count_id])
                        task_count = TaskCount() if not task_count else task_count
                        ...
                        is_collect_limit = True if collect_count >= limit_num else False
    ...
    def process_browse_goods(self,app_id,act_id,module_id,user_id,login_token,goods_id,handler_name,check_user_nick=True,continue_request_expire=5,task_sub_table=None,browse_sub_table=None):
        ...
        try:
            ...
            if invoke_result_data.success == True:
                ...
                is_browse = browse_log_model.get_total("act_id=%s and user_id=%s and goods_id=%s", params=[act_id, user_id, goods_id])
                if is_browse > 0:
                    invoke_result_data.success = False
                    invoke_result_data.error_code = "error"
                    ...
                    if task_invoke_result_data.success == True:
                        ...
                        is_browse_limit = True if browse_count >= limit_num else False
    ...
    def process_favor_store(self,app_id,act_id,module_id,user_id,login_token,handler_name,request_id,check_user_nick=True,continue_request_expire=5,asset_sub_table=None):
        """
            ...
            if invoke_result_data.success == True:
                ...
                if user_info_dict["is_favor"] ==1:
                    ...
                    if task_invoke_result_data.success == True:
                        task_info_dict = task_invoke_result_data.data
                        task_config = task_info_dict["task_config"]
                        reward_value = task_config["reward_value"] if task_config.__contains__("reward_value") else 0
                        asset_object_id = task_config["asset_object_id"] if task_config.__contains__("asset_object_id") else ""
                        if reward_value > 0:
                            only_id = self.get_only_id(task_info_dict["complete_type"],task_type)
                            asset_type = self.get_task_asset_type(act_info_dict["task_asset_type_json"],task_type)
    ...
    def process_join_member(self,app_id,act_id,module_id,user_id,login_token,handler_name,request_id,check_user_nick=True,continue_request_expire=5,asset_sub_table=None):
        """
