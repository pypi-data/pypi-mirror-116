...
@LastEditTime: 2021-08-17 17:37:42
...
class TaskBaseModel(FrameBaseModel):
    ...
    def process_buy_goods(self,app_id,act_id,module_id,user_id,login_token,handler_name,app_key="",app_secret="",is_log=False,check_user_nick=True,continue_request_expire=5,task_sub_table=None):
        """
        :description: 处理浏览商品任务
        :param app_id:应用标识
        :param act_id:活动标识
        ...
        :last_editors: HuangJianYi
        """
        acquire_lock_name = f"process_buy_goods:{act_id}_{module_id}_{user_id}"
        identifier = ""
        task_type = TaskType.buy_goods.value
        now_day = SevenHelper.get_now_day_int()
        now_datetime =  SevenHelper.get_now_datetime()
        try:
            invoke_result_data = self.business_process_executing(app_id,act_id,module_id,user_id,login_token,handler_name,True,check_user_nick,continue_request_expire,acquire_lock_name)
            if invoke_result_data.success == True:
                ...
                if task_invoke_result_data.success == True:
                    ...
                    if task_config.__contains__("effective_date_start") == True and task_config.__contains__("effective_date_end") == True and task_config["effective_date_start"] != "" and task_config["effective_date_end"] != "":
                        invoke_result_data = top_base_model.get_buy_order_list(user_info_dict["open_id"], access_token,app_key,app_secret, task_config["effective_date_start"], task_config["effective_date_end"],is_log=is_log)
                        if invoke_result_data.success == True:
