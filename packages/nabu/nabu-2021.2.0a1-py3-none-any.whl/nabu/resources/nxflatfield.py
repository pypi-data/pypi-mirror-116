...
def update_dataset_info_flats_darks(dataset_info, processes_file=None, output_dir=None, force_flatfield=False):
    """
    ...
    output_dir: str, optional
        Directory where
    force_flatfield: bool, optional
        Wether to force use flats/darks from the file in 'processes_files'.
        Default is False
    """
    ...
    def _get_tomwer_process_file_name(scan):
    ...
    def _get_results_file():
        if processes_file not in (None, ""):
            if is_writeable(processes_file):
    ...
    def _compute_or_load_flats():
        ...
        nxflatfield = NXFlatField(
            dataset_info._get_dataset_hdf5_url(),
            dataset_info.dataset_scanner.image_key,
            lookup_files=lookup_files,
            results_url=results_url,
            force_load_existing_results=force_flatfield,
    ...
    def _get_flats_darks():
        if len(dataset_info.flats) == 0 and not(force_flatfield):
