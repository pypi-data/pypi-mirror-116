...
class PkulawdCrawler:
    def __init__(self, cookie: str, query_base64_request: str, year: list = None, page_size: int = 100, data_dir: str = None):
    ...
    def download_search_pages(self, level: str, keyword: str, based64_request: str, page_num: int = 0,
        ...
        try:
            count = int(html.xpath('//div[@class="l-search min-height-30"]/h3/span/text()')[0])
    ...
    def download_policy_html(self, pool: bool = False):
        ...
        for file in os.listdir(self.search_dir):
            if not file.endswith('.xlsx'):
                continue
            df = read_excel(f'{self.search_dir}/{file}')
    ...
    def parse_policy_info(self, to_file: str = None):
        ...
        for file in self.scan_directory_contents(f'{self.html_dir}'):
            ...
            result['标题'] = title
            for li in response.xpath('//div[@class="fields"]/ul/li'):
        ...
        df = DataFrame(data=data)
        search_objs = [read_excel(file) for file in self.scan_directory_contents(self.search_dir, suffix='xlsx')]
