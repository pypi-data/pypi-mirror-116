...
# Monkey-patch get_source_segment into earlier Pythons in a bad way
if not hasattr(ast, "get_source_segment"):
    ast.get_source_segment = lambda src, node: (
        src.splitlines()
            [node.lineno - 1]
            [node.col_offset:]
    )
    print(
        "Warning: ast module does not have a get_source_segment"
        " function; expression values will include extra code. Use"
        " Python version 3.8 or greater for ideal results."
    )
...
def testCase(expr):
    ...
    1. When multiple calls to `testCase` appear on a single line of the
        ...
        the use of `testCase` multiple times on line line within
...
def print_success_message(
    ...
    """
    Prints an expectation success message for the given test context,
    with the given details to be printed if the detail level is 1 or
    higher. Unless `include_expr_details` is set to False, details of
    the test expression will also be printed (but only when the detail
    ...
    if COLORS:
        print("\x1b[34m", end="", file=sys.stderr)
        suffix = "\x1b[0m"
    else:
        suffix = ""
    ...
    if DETAIL_LEVEL < 1:
        ...
        print(
            f"✓ expectation from {tag} met for test at {ctag}",
            file=sys.stderr
        )
        detail_msg = indent(details, 2)
        if detail_msg.startswith('\n'):
            detail_msg = detail_msg[1:]

        # Expression details unless suppressed
        if include_expr_details:
            expr_base, expr_extra = expr_details(context)
            detail_msg += indent(expr_base, 2)
            detail_msg += indent(expr_extra, 2)

        print(f"{detail_msg}{suffix}", file=sys.stderr)
...
def print_failure_message(
    ...
    """
    Prints an expectation failure message for the given test context,
    with the given details to be printed if the detail level is 0 or
    higher, and the given extra details to also be printed if the detail
    level is 1 or higher. Unless `include_expr_details` is set to false,
    details of the expression tested will be included automatically. The
    tag should be a filename:lineno string indicating where the
    ...
    # Make the whole message bold and red
    if COLORS:
    ...
    if DETAIL_LEVEL < 1:
        ...
        print(
            f"✗ expectation from {tag} NOT met for test at {ctag}",
            file=sys.stderr
        )
    ...
    if DETAIL_LEVEL >= 0:
        if not detail_msg.endswith('\n'):
            detail_msg += '\n'
        detail_msg += indent(details, 2)
    if DETAIL_LEVEL >= 1 and extra_details:
        if not detail_msg.endswith('\n'):
            detail_msg += '\n'
        detail_msg += indent(extra_details, 2)
...
def expr_details(context):
    ...
    expectation. Both strings will start with a newline.
    """
    ...
    if context.get("relevant"):
        msg += f"\nIn expression {short_expr}, values were:"
        for key in sorted(
            context["relevant"],
            key=lambda fragment: expr.index(fragment)
        ):
            val = context.get("values", {}).get(key, "???")
            msg += f"\n  {key} = {val}"
    else:
        msg += f"\nIn expression {short_expr}, values were:"
    ...
    extra_values = sorted(
        [
            key
            for key in context.get("values", {}).keys()
...
def check_expectation(exp, set_success=True):
    ...
    also print a success or failure message, and will return True on
    ...
    if exp["type"] == "result":
        ...
        if result == expected:
            ...
            print_failure_message(exp["tag"], context, msg, extra_msg)
            if set_success:
                exp["met"] = False
            return False
...
def get_external_calling_frame():
    ...
    called into the `expectations` module. Returns None if it can't find
    an appropriate call frame in the current stack.
...
def get_my_context(function_or_name):
    ...
    try:
        ...
        expr_src = ast.get_source_segment(src, arg_expr)
        ...
        for node in ast.walk(arg_expr):
            ...
            if isinstance(
                ...
                key = ast.get_source_segment(src, node)
...
def recheck_expectation(expectation, new_context):
    ...
    the original message, and will return True or False depending on
    ...
    return check_expectation(new_expectation, set_success=False)
