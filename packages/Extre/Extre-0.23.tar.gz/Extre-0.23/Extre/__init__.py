...
if Var.Client2:
    session_name = str(Var.Client2)
    client = TelegramClient(StringSession(session_name), Var.CLIENT_API, Var.CLIENT_API_HASH)
else:
    session_name = "startup"
    client = TelegramClient(session_name, Var.CLIENT_API, Var.CLIENT_API_HASH)
...
if bool(ENV):
    ...
    if CONSOLE_LOGGER_VERBOSE:
        ...
        basicConfig(format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
                    level=INFO)
    LOGS = getLogger("EXTREMEPRO_DATABASE")
