...
def run(params):
    ...
    def see_handler():
        ...
        if get_suffix(file_path) == 'py':
            ...
            response.headers["Content-Type"] = "text/html;charset=utf-8"
            return response
        elif get_suffix(file_path) == 'c' or get_suffix(file_path) == 'cpp' or get_suffix(
                file_path) == 'h' or get_suffix(file_path) == 'hpp':
    ...
    def setting_handler():
        ...
        response_content=open(os.path.join(mydir, "./html/setting.html"), 'r', encoding="UTF-8").read().replace("{theme}",support_theme)
        if config_content.get("open_and_edit","no")=="no":
            response_content=response_content.replace("{open_and_edit}",'''<option value="no">仅以只读模式打开</option><option value="yes">尝试以文本文件模式编辑</option>''')
        else:
            response_content=response_content.replace("{open_and_edit}",'''<option value="yes">尝试以文本文件模式编辑</option><option value="no">仅以只读模式打开</option>''')
