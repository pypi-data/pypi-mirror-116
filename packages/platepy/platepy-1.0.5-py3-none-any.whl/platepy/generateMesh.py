...
def generateMesh(self,showGmshMesh=False,showGmshGeometryBeforeMeshing = False, elementDefinition='MITC-4-N', \
    meshSize=0, nEdgeNodes=0, targetNelements =400, order='linear', meshDistortion = False, distVal = 100,\
        deactivateRotation=False, maxNelements = np.inf):
    ...
    try:
        ...
        #     gmsh.model.geo.synchronize()
            ...
            if meshSize == 0: # try to guess the right mesh size
                area = _polyArea(self.plates[0].outlineCoords)
                meshSize = np.sqrt(area/targetNelements) # ynElements=area/(meshSizes)**2
    ...
    if maxNelements<np.inf:
        _, elemTags, _ = gmsh.model.mesh.getElements(2,1)
        nElements = len(elemTags[0])
        if nElements>maxNelements:
            return nElements
            raise TypeError('The mesh has too many elements!')
    ...
    self.mesh.polyArea = _polyArea(self.plates[0].outlineCoords)
    self.mesh.nElements = len(elementsList)
...
def _averageSideLength(polygonCoords):
    nPoints = polygonCoords.shape[0]
    dists = np.zeros(nPoints-1)
    for i in range(0,nPoints-1):
        dists[i] = np.linalg.norm(polygonCoords[i,:]-polygonCoords[i+1,:])
    return np.average(dists)

def _polyArea(polygonCoords):
    x=polygonCoords[:,0]
    y=polygonCoords[:,1]
