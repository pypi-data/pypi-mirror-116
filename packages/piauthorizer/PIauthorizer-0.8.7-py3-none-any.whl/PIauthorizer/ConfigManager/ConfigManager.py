    ...
    def get_config_for_tenant(self, delegated_token: str, tenant_name: str, tenant_id: str) -> dict:
        ...
        for config_name in self.config_dict:
            if not tenant_specific_configs[config_name]['configurations']:
                ...
                default_config = self.config_dict[config_name][self.default_tenant_name].get_configuration().get('configurations', None)
                if not default_config:
                    logger.warning(f"Application {config_name} is empty for default tenant {self.default_tenant_name} - {self.default_tenant_id}")
