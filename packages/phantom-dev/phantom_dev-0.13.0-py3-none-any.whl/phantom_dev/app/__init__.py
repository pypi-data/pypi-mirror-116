...
class App:
	...
	@classmethod
	def _infer_action_metadata(cls, handler: action_handler.ActionHandler):
		metadata = {}
		...
		if handler.data_contains_map is not None:
			...
			for key, type_obj in handler.data_contains_map.items():
				output_data[f'action_result.data.*.{key}'] = {
					...
					'data_type': cls._get_data_type(type_obj)
				}
		...
		for name, parameter in signature.parameters.items():
			...
			inferred_data = cls._infer_parameter_metadata(parameter)
