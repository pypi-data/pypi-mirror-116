...
def class_averaged_score(score_func: Callable[..., float],
    ...
    if metric_name == constants.NORM_MACRO_RECALL:
        ...
        if (not multilabel and average is not None) or metric_name == constants.AVERAGE_PRECISION_CLASSWISE \
            ...
            if len(one_class_vector) > 0:
                module_logger.info("y_test_bin: {}".format(y_test_bin))
                y_test_bin = np.delete(y_test_bin, one_class_vector, 1)
                y_pred_proba = np.delete(y_pred_proba, one_class_vector, 1)
                dropped_msg_fmt = "{} classes where there are no positive and negative samples were ignored."
                dropped_msg = dropped_msg_fmt.format(metric_name)
                module_logger.info(dropped_msg)
                ignored_classes_fmt = "{} classes were ignored."
            ...
            if y_pred_proba.shape[1] > 0:  # ensure atleast one column
                ...
                dropped_msg_fmt = "No classes qualified for classwise {} computation.Returning np.nan."
                dropped_msg = dropped_msg_fmt.format(metric_name)
                module_logger.info(dropped_msg)
                return float(np.nan)
...
class ClassificationDataDto:
    ...
    def get_targets(self, encoded: Optional[bool] = True,
                    classwise: Optional[bool] = False) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        if encoded:
            return self.y_test_encoded, self.y_pred_encoded, self.class_labels_encoded
        elif classwise:
            return self.y_test_encoded, self.y_pred_encoded, self.class_labels_original
        else:
