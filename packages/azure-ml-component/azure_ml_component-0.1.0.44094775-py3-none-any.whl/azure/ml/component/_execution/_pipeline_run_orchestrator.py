...
class PipelineRunOrchestrator:
    ...
    def start_orchestrate(self, working_dir, tracker, visualizer=None, show_output=False,
                          continue_on_step_failure=None, mode=RunMode.Docker, raise_on_error=True):
        """
        ...
        pipeline_run_info = self.PipelineRunInfo(working_dir=working_dir, show_output=show_output,
                                                 ...
                                                 visualizer=visualizer, raise_on_error=raise_on_error, tracker=tracker)
        ...
        try:
            ...
            ComponentRunHelper.download_datasets(self.input_dataset, self.workspace,
                                                 working_dir, self.pipeline_parameters)
    ...
    class PipelineRunInfo:
        ...
        def __init__(self, working_dir, tracker, show_output=False, continue_on_step_failure=None,
                     mode=RunMode.Docker, raise_on_error=True, visualizer=None):
    ...
    def _execute_component_node(self, node, working_dir, tracker, pipeline_run_info):
        ...
        try:
            ...
            status = component_run_helper.component_run(raise_on_error=True)
    ...
    def _execute_pipeline_component_node(self, node, working_dir, tracker, pipeline_run_info):
        ...
        try:
            ...
            pipeline_run_status = pipeline_component_orchestrator.start_orchestrate(
                working_dir, tracker, pipeline_run_info.visualizer, pipeline_run_info.show_output,
                pipeline_run_info.continue_on_step_failure,
                pipeline_run_info.mode, pipeline_run_info.raise_on_error)
    ...
    def _execute_sub_pipeline(self, sub_pipeline, working_dir, child_tracker, pipeline_run_info):
        ...
        try:
            ...
            run_result = orchestrator.start_orchestrate(working_dir,
                                                        child_tracker,
                                                        pipeline_run_info.visualizer,
                                                        pipeline_run_info.show_output,
                                                        pipeline_run_info.continue_on_step_failure,
                                                        pipeline_run_info.mode,
                                                        pipeline_run_info.raise_on_error)
            # Update pipeline component outputs
