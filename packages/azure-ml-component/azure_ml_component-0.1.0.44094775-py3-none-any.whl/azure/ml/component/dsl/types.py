...
class Output:
    ...
    @classmethod
    def _from_dict(cls, dct: dict):
        """Convert a dict to an Output object."""
        name = dct.pop('name')
        obj = cls(**dct)
        obj._name = name
        return obj
...
def _get_annotation_by_value(val):
    ...
    if _is_dataset(val):
        ...
        if not annotation:
            raise UserErrorException(f"Can't infer dsl.types from type {type(val)!r} value {val!r}")
    return annotation
...
def _get_param_with_standard_annotation(cls_or_func, is_func=False, non_pipeline_parameter_names=None):
    ...
    def _get_fields(annotations):
        ...
        for name, annotation in annotations.items():
            ...
            if not _is_dsl_type_cls(annotation) and not _is_dsl_types(annotation):
                annotation = _get_annotation_cls_by_type(annotation, raise_error=True)
    ...
    def _merge_field_keys(annotation_fields, defaults_dict):
        ...
        for key in anno_keys:
            if key in dict_keys:
                ...
                if seen_default:
                    raise SyntaxError(f'Non-default argument {key!r} follows default argument.')
                all_keys.append(key)
    ...
    def _filter_out_non_pipeline_parameters_from_dict(dct):
        return {k: v for k, v in dct.items() if k not in non_pipeline_parameter_names}

    # From annotations get field with type
    annotations = _filter_out_non_pipeline_parameters_from_dict(getattr(cls_or_func, '__annotations__', {}))
    ...
    if not is_func:
        ...
        defaults_dict = _filter_out_non_pipeline_parameters_from_dict(defaults_dict)
