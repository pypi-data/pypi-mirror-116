...
class Pipeline(Component, Visible, WorkspaceTelemetryMixin):
    ...
    def _copy_nodes(self, original_nodes, _copied_id_field, **kwargs):
        ...
        def update_component_extra_settings(_pipeline_parameters, complete_node, unfinished_node):
            ...
            unfinished_node._regenerate_output = complete_node._regenerate_output
    ...
    def _register_pipeline_component(self, graph: GraphDraftEntity,
        ...
        if anonymous_creation is True:
            repr_dict = {
                ...
                "subscriptionId": self._workspace.subscription_id,
    ...
    def _validate_pipeline_component(self, is_local, pipeline_parameters=None, default_compute=None):
        ...
        def validate_datastore_compute(pipeline: Pipeline, default_datastore, default_compute):
            ...
            for node in pipeline.nodes:
                if isinstance(node, Pipeline):
                    ...
                    validate_datastore_compute(node, default_datastore, default_compute)
                else:
        ...
        if not is_local:
            ...
            validate_datastore_compute(self, self._resolve_default_datastore(), default_compute)
    ...
    def _run(self, experiment_name=None, working_dir=None, mode=RunMode.Docker.value, track_run_history=True,
        ...
        with RunHistoryTracker.without_definition(self.workspace, experiment_name, track_run_history) as tracker:
            ...
            pipeline_run_success = pipeline_run.start_orchestrate(short_working_dir, tracker, visualizer, show_output,
                                                                  continue_on_step_failure, run_mode, raise_on_error)
