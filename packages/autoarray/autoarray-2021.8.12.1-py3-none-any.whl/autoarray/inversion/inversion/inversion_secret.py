...
@decorator_util.jit()
def w_tilde_interferometer_from(
    sigma_real: np.ndarray, uv_wavelengths: np.ndarray, grid_radians_slim: np.ndarray
) -> np.ndarray:
    """
    ...
    for i in range(w_tilde.shape[0]):
        for j in range(i, w_tilde.shape[1]):
            ...
            for vis_1d_index in range(uv_wavelengths.shape[0]):
                ...
                w_tilde[i, j] += sigma_real[vis_1d_index] ** -2.0 * np.cos(
                    2.0
                    * np.pi
                    * (
...
@decorator_util.jit()
def w_tilde_preload_interferometer_from(
    ...
    w_tilde_preload = np.zeros((y_shape * 2, x_shape * 2))
    ...
    for i in range(y_shape):
        for j in range(x_shape):
            ...
            for vis_1d_index in range(uv_wavelengths.shape[0]):
                w_tilde_preload[i, j] += sigma_real[vis_1d_index] ** -2.0 * np.cos(
                    2.0
                    * np.pi
                    * (
    ...
    for i in range(y_shape):
        for j in range(x_shape):
            ...
            if i > 0 and j > 0:
                ...
                for vis_1d_index in range(uv_wavelengths.shape[0]):
                    w_tilde_preload[-i, -j] += sigma_real[
                        vis_1d_index
                    ] ** -2.0 * np.cos(
                        2.0
                        * np.pi
    ...
    return w_tilde_preload


@decorator_util.jit()
def w_tilde_interferometer_via_preload_from(
...
@decorator_util.jit()
def curvature_matrix_via_w_tilde_preload_interferometer_from(
    ...
    for ip0 in range(image_pixels):
        ...
        for ip1 in range(image_pixels):
            ...
            curvature_matrix[sp0, sp1] += w_tilde_preload[y_diff, x_diff]
