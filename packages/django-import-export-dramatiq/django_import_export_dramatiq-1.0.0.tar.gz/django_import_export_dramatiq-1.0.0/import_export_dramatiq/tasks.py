...
from import_export_dramatiq import models
from import_export_dramatiq.model_config import ModelConfig

logger = logging.getLogger(__name__)


importables = getattr(settings, "IMPORT_EXPORT_DRAMATIQ_MODELS", {})
...
def _run_import_job(import_job, dry_run=True):
    ...
    if dry_run:
        ...
        summary += "</head>"
        summary += "<body>"
        # TODO refactor the existing template so we can use it for this
        ...
        if not result.invalid_rows:
            ...
            def cols(row):
                return "</td><td>".join([field for field in row.diff])
            ...
            def cols(row):
                return "</td><td>".join([str(field) for field in row.values])
...
def run_import_job(pk, dry_run=True):
    run_import_job.logger.info("Importing %s dry-run %s" % (pk, dry_run))
...
def run_export_job(pk):
    run_export_job.logger.info("Exporting %s" % pk)
