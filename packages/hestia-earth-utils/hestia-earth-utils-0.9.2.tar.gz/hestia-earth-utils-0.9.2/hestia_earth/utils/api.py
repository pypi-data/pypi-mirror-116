...
def node_to_path(node_type: SchemaType, node_id: str, data_state=None):
    ...
    return jsonld_path if data_state is None or len(data_state) == 0 else os.path.join(data_state, jsonld_path)
...
def download_hestia(node_id: str, node_type=SchemaType.TERM, data_state='', mode='') -> dict:
    ...
    try:
        ...
        return json.loads(_load_from_bucket(AWS_BUCKET, jsonld_path)) if AWS_BUCKET else fallback()
    except ImportError:
        return fallback()
