...
def processValueMatch(value, match, condition=None):
    ...
    if condition:
        ...
        if condition == "$or":
            logger.debug("'OR' condition found")
            for field, deeper_match in match:
                value = utils.dict_lookup(
                    context, field
                )  # TODO: Is this broken?  context isn't in this function
        ...
        if condition == "$and":
            logger.debug("'AND' condition found")
            for field, deeper_match in match:
                # TODO: context is not defined - does this even work?
                value = utils.dict_lookup(context, field)
