...
import urllib, requests, flask, wsgiserver
    ...
    def serial_open_gateway(self):
        ...
        if self.is_open:
            ...
            try:
                ...
                error_string = 'SerialManager -' + self.port + ' cannot be opened: resources busy - trying another port.'
                self.problem_logger.error(error_string)
                try:
                    self.close()
                except:
        ...
        while not self.stop_event.is_set():
            ...
            while self.msg_queue.qsize() > 0:
                ...
                try:
                    ...
                    ds_utc_dt = datetime.datetime.strptime(ds_utc, '%d-%H:%M:%S')
                ...
                try:
                    ...
                    if self.test_align_flag:
                        if self.test_flag:
                            ...
                            conn_name = '/post_conn'
                            with urllib.request.urlopen(post_req, timeout=5) as post_conn:
                                try:
                                    if self.interval_sec > 0:
                                        time.sleep(self.interval_sec/4)
...
#                                 HTTPresponse_flag = int(post_conn.read().decode('utf-8'))
                    ...
                    if not self.test_flag:
                        ...
                        ms_URL = self.full_URLstring.replace('175.139.231.133:8096',ms_dest).replace('http','https')
                        if self.no_auth_flag:
                        ...
                        with urllib.request.urlopen(ss_main_req, timeout=5) as post_conn_3:
                            try:
                                if self.interval_sec > 0:
                                    time.sleep(self.interval_sec/4)
#                                 HTTPresponse_flag = int(post_conn_3.read().decode('utf-8'))
                                HTTPresponse_code = int(post_conn_3.getcode())
                                if HTTPresponse_code is 200:
                                    self.glob_ACK_counter += 1
                            except Exception as error:
                                raise
...
# class MainMsgServerThread(threading.Thread):
#     '''
#     A super-type Thread designed to grant it the ability to manage its variables and resources within a MULTIthreading environment.
#     This thread focuses on running a MQTT Client that establishes a 2-way communication with a Messaging Broker/Server that is directed using a DDNS name.
#     Statistics are also collected daily on the main loop to observe the MQTT link performances.
#     Unused in the HTTPDEPLOY series of the PYGatewayListener. 
#     '''
#     def __init__(self, client_obj, packet_logger, simple_packet_logger, problem_logger, message_queue, mqtt_message_queue, msg_server_DDNS, msg_server_port, topic_header, client_name, interval, timeout_cnt):
#         '''Start up variables taken from INIT arguments. Also starts up Event control functions, connection control variables and statistics variables.'''
#         super(MainMsgServerThread, self).__init__()
#         self.serverlink = client_obj
#         self.packet_logger = packet_logger
#         self.simple_packet_logger = simple_packet_logger
#         self.problem_logger = problem_logger
#         self.msg_queue = message_queue
#         self.mqtt_msg_queue = mqtt_message_queue
#         self.msg_server_name = msg_server_DDNS
#         self.msg_server_port = msg_server_port
#         self.msg_topic = topic_header
...
#         self.glob_publish_counter = 0
#         self.glob_seq_counter = 0
#         self.ack_list = []
#     
#     def get_stats(self):
#         '''Returns an integer tuple stating the statistics of the total number of packets sent versus the total number of OK acknowledged messages.'''
#         return (self.glob_seq_counter, self.glob_publish_counter)
#     
#     def set_stats(self):
#         '''Resets the statistics integer tuple daily, which in this case, is controlled by an external caller.'''
#         self.glob_seq_counter = 0
#         self.glob_publish_counter = 0
#     
#     def get_ack_list(self):
#         '''Allows the caller to obtain a node list on the IDs in which their data is not acknowledged due to non-replies.'''
#         return self.ack_list
#     
#     def set_ack_list(self):
#         '''Resets the list for unacknowledged node IDs.'''
#         self.ack_list = []
#     
#     def status(self):
#         return self.stop_event.is_set()        
...
#     def run(self):
#         '''
#         Functions to be executed once the MainMsgServerThread is started.
#         While waiting for the data in msg_queue/mqtt_msg_queue, this thread will consistently go into sleep.
#         The connection parameters of the MQTT Client must be consistently monitored as it is a type of dedicated connection established between its broker.
#         This means that any abrupt disconnections observed via multiple flags must be captured for autonomous reconnection to sustain the client-server link.
#         msg_queue deals with all packet data meant to be published, and records the sending statistics.
#         mqtt_msg_queue deals with all acknowledgement data meant to be processed and validated, and reactively records the receiving statistics.
#         Both queues acquire data on a single Messaging topic/channel, quoted target_topic in this case. 
#         '''
#         self.connection_flag = self.serverlink.connect(self.msg_server_name, self.msg_server_port, keepalive=300)
#         while self.connection_flag != 0:
#             self.timeout_count += 1
#             self.serverlink.loop()
#             if self.timeout_count >= self.timeout_cnt:
#                 self.stop()
#                 break
#         target_topic = self.msg_topic + self.client_id
#         self.ack_list = []
#         while not self.stop_event.is_set():
#             self.timeout_count = 0
#             self.serverlink.loop()
#             if self.interval_sec > 0:
#                 time.sleep(self.interval_sec/2)
#             if self.serverlink.get_disconnect_status() == 1 or self.connection_flag == 1:
#                 try:
#                     self.connection_flag_obj = self.serverlink.reconnect()
#                 except socket.gaierror:
#                     spec_string = self.name + ' - DDNS server name cannot be resolved.'
#                     self.problem_logger.error(spec_string)
#                     continue
#                 except TimeoutError:
#                     self.connection_flag = 1
#                 if self.connection_flag & self.connection_flag_obj == 0:
#                     self.serverlink.set_disconnect_status()
...
#                     self.serverlink.loop()
#                     if self.timeout_count >= self.timeout_cnt:
#                         spec_string = self.name + ' - Reconnect threshold count reached, exiting...'
#                         self.packet_logger.debug(spec_string)
#                         self.problem_logger.error(spec_string)
#                         self.stop()
#                         break    
#             while self.msg_queue.qsize() > 0:
#                 self.glob_publish_counter += 1
#                 ack_seed, packet_data, _ = self.msg_queue.get()
#                 result_code , _ = self.serverlink.publish(target_topic, packet_data)
#                 time.sleep(self.interval_sec/4)
#                 if result_code == 0:
#                     self.ack_list.append(ack_seed)
#                     self.publish_flag = True
#                     self.msg_queue.task_done()
#                 else:
#                     self.msg_queue.task_done()
#                     self.msg_queue.put((ack_seed, packet_data, None))
#                     spec_string = b'MQTT - Publish failed, data ' + packet_data
#                     self.packet_logger.debug(spec_string)
#                     self.problem_logger.info(spec_string)
#                     try:
#                         self.connection_flag = self.serverlink.reconnect()
#                     except socket.gaierror:
#                         spec_string = 'MessagingServer - DDNS server name cannot be resolved.'
#                         self.problem_logger.error(spec_string)
#                         continue
#                     except TimeoutError:
#                         self.connection_flag = 1
#             while self.mqtt_msg_queue.qsize() > 0:
#                 if self.interval_sec > 0:
#                     time.sleep(self.interval_sec/4)
#                 ack_msg = self.mqtt_msg_queue.get()
#                 print(ack_msg)
#                 node_id_start_searcher = ack_msg.find(b'/')
#                 node_id_end_searcher = ack_msg.find(b'&')
#                 if node_id_start_searcher == -1 or node_id_end_searcher == -1:
#                     pass
#                 else:
#                     try:
#                         ack_nodeid = ack_msg[node_id_start_searcher+1:node_id_end_searcher].decode('utf-8')
#                         if ack_nodeid in self.ack_list:
#                             self.glob_seq_counter += 1
#                         else:
#                             pass
#                     except:
#                         pass
#                 self.mqtt_msg_queue.task_done()
#             self.publish_flag = False
#     
#     def stop(self):
#         '''Functions to be executed if an unexpected exit has occurred, to both the super thread or the overall script. Stop event is set to cleanly exit the super thread.'''
...
class MainListenerThread(threading.Thread):
    ...
    def checkserialhang(self):
        ...
        if command_searcher == -1:
            ...
            try:
                dummy_variable = packet_portion.decode('utf-8')
            except:
                hang_flag = True
        return hang_flag
    ...
    def run(self):
        ...
        try:
            ...
            if verified_rawpacket is not b'':
...
# msg_broker = param_dict['msg_broker'].replace('[','').replace(']','').replace('\'','').split(',')
...
def main(*args):
    ...
    mqtt_msg_queue = multiprocessing.JoinableQueue()
...
#     MQTTProcessObject.ini_run(my_logger, my_logger_simple, TLS_cert_location, MQTT_topic_header, MQTT_client_ID, mqtt_msg_queue)
    options_status_dict = {
        'DBUP': False,
        'GPSUP': False,
        'DEMOUP': False,
