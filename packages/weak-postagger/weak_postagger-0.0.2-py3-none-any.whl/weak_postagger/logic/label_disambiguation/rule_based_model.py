...
class RuleBasedDisambiguation:
    ...
    def label_disambiguation(self, sentence: str, labels: str) -> str:
        """
        ...
        :return:
        """
        corrected_labels = [label.split('|') if '|' in label else [label] for label in labels.split()]
        for index, label in enumerate(corrected_labels):
            if len(label) > 1:
