...
class MyMessageCallback(UserMessageCallback):
    ...
    def on_error(self, userMessageResult=None):
        try:
            for message in userMessageResult.get_message_list():
                logger.info(
                    "failed to put message: " + message + " we will retry to put it.")
...
class TalosProducerDemo:
    accessKey = "AKS363RG5KWQOL2RSB"
    accessSecret = "Rb5s7nZf3zdshvY6C3+e8aXqHSiOq0D+h3+Gx1Ks"
    topicName = "sdk-test"

    pro = dict()
    pro["galaxy.talos.service.endpoint"] = "http://cnbj1-talos.api.xiaomi.net"
    ...
    def __init__(self):
        ...
        # credential
        self.credential = Credential(UserType.DEV_XIAOMI, self.accessKey,
                                     self.accessSecret)
    ...
    def start(self):
        ...
        while i < self.toPutMsgNumber:
            message = Message(
                message="test message: this message is a text string.")
