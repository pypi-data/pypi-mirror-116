...
class Logger:
    ...
    def __new__(cls, path=None):
        if cls._instance is None:
            ...
            cls._log_path = cls._resolve_lot_path(path)

            cls._handlers = []

            cls.add_handler(handlers.FileHandler(cls._log_path))
    ...
    @classmethod
    def _resolve_lot_path(cls, path):
        if path is None:
    ...
    def format(self, *args, **kwargs):
        ...
        return prefix + sep.join(args) + end

    def info(self, *args, **kwargs):

        msg = self.format(*args, **kwargs)

        self._log(self._get_time_string("%d/%m/%y %H:%M:%S") +
                  f'[{inspect.stack()[1].function}][I] ' + msg,
    ...
    def warning(self, *args, **kwargs):
        ...
        self._log(self._get_time_string("%d/%m/%y %H:%M:%S") +
                  f'[{inspect.stack()[1].function}][W] ' + msg,
    ...
    def error(self, *args, **kwargs):
        ...
        self._log(self._get_time_string("%d/%m/%y %H:%M:%S") +
                  f'[{inspect.stack()[1].function}][E] ' + msg,
