...
def add_to_stream(examples, pt2tkn, last=False, unk_tkn='unk'):
    ...
    if last:
        ...
        examples['stream'] = [examples['stream'][i] + [pt2tkn.get(examples['patient_id'][i], unk_tkn)] for i in range(len(examples['stream']))]
    else:
        examples['stream'] = [[pt2tkn.get(examples['patient_id'][i], unk_tkn)] + examples['stream'][i] for i in range(len(examples['stream']))]
