...
def _type_week(text):
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(
            "argument must be one of {{{}}}.".format(", ".join(calendar.WEEK_CHOICES))
        ) from error
...
def _file_exists(text):
    ...
    if os.path.exists(text):
        return text
    raise argparse.ArgumentTypeError("'{}' must be an existing file.".format(text))
...
def commandline_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="scal", description="A year calendar producer."
    )
