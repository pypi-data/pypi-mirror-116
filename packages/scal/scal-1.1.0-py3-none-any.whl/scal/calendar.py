...
def parse_weeks(text):
    ...
    if text not in WEEK_CHOICES:
        raise KeyError(text)
    return {"work": text in ["both", "work"], "iso": text in ["both", "iso"]}
...
RE_FROMTO = re.compile(
    r"^From *{} *to *{}$".format(RE_DATE_STR.format(id="0"), RE_DATE_STR.format(id="1"))
)
RE_HOLIDAY = re.compile(
    r"^(?P<start>{})? *(?P<end>{}) *(?P<name>.*)$".format(
        RE_DATE_STR.format(id="0"), RE_DATE_STR.format(id="1")
    )
)
RE_AFFECTATION = re.compile(r"^(?P<name>\w*) *= *(?P<value>.*)$")

DEFAULT_CONFIG = {"language": "english", "papersize": "a4paper"}
...
class Calendar:
    ...
    def __init__(self, file):
        ...
        for line in file:
            ...
            if RE_FROMTO.match(stripped):
                ...
                self.set_start(
                    match.group("year0"), match.group("month0"), match.group("day0")
                )
                self.set_end(
                    match.group("year1"), match.group("month1"), match.group("day1")
                )
                ...
                raise errors.ConfigError(
                    "Could not parse line {}: '{}'.".format(linenumber, line[:-1])
                )
        ...
        if "lang" in self.config:
            logging.warning(
                "Obsolete 'lang' configuration option should be renamed to 'language'."
            )
            self.config["language"] = self.config["lang"]

    def set_start(self, year, month, day):
        """Defin the start date of the calendar."""
        if self.start is not None:
            raise errors.ConfigError("Calendar boundaries configured twice.")
        if year is None:
            raise errors.ConfigError("Missing year in 'From <DATE> to <DATE>' line.")
    ...
    def add_holiday(self, date0, date1, name="", line=None):
        ...
        if ((year0 is None) and (year1 is not None)) or (
            ...
            raise errors.ConfigError(
                "Either one or both years may be omitted (line '{}').".format(str(line))
            )
        if year0 is None:
            for year in range(self.start.year, self.end.year + 1):
                try:
                    if is_between(
                        self.start, date(year, month0, day0), self.end
                    ) and is_between(self.start, date(year, month1, day1), self.end):
                        self.holidays.append(
                            Period(
                                date(year, month0, day0), date(year, month1, day1), name
                            )
                        )
                    ...
                    LOGGER.warning(
                        (
                            ...
                            "calendar boundaries)."
                        ).format(date(year, month0, day0), date(year, month1, day1))
                    )
        else:
            try:
                self.holidays.append(
                    Period(date(year0, month0, day0), date(year1, month1, day1), name)
                )
            except errors.ConfigError:
                LOGGER.warning(
                    (
                        ...
                        "boundaries)."
                    ).format(date(year0, month0, day0), date(year1, month1, day1))
                )
