...
def gen_html_report(summary, report_template=None, report_dir=None, report_file=None):
    ...
    dt_obj = datetime.fromtimestamp(summary['time']['start_at'])
    summary["time"]["start_datetime"] = datetime.strftime(dt_obj, '%Y年%m月%d日 %H:%M:%S')
    ...
    if report_file:
        ...
        report_file_name = "{}.html".format(datetime.strftime(dt_obj, '%Y%m%d%H%M%S'))
