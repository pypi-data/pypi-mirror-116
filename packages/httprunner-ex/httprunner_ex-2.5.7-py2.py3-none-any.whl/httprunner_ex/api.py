...
class HttpRunner(object):
        ...
        def _add_test(test_runner, test_dict):
            ...
            def test(self):
                try:
                    ...
                    self.meta_datas = parser.parse_lazy_data(test_runner.meta_datas,
            ...
            if isinstance(test.__doc__, parser.LazyString):
                try:
                    ...
                    test.__doc__ = parser.parse_lazy_data(test.__doc__, variables)
    ...
    def _aggregate(self, tests_results):
        ...
        for tests_result in tests_results:
            ...
            # 解析records中的validate_extractor数据中的LazyString
            for record in testcase_summary['records']:
                record['meta_datas']['validators']['validate_extractor'] = parser.parse_lazy_data(
                    record['meta_datas']['validators']['validate_extractor'],
                    utils.DP.get_variables()
                )
    ...
    def run_tests(self, tests_mapping):
        ...
        # project_mapping.get('functions').get('VARIABLE_INIT')()
        ...
        # 用例解析之前，用例加载已经完成
        try:
            project_mapping.get('functions').get('BEFORE_PARSE')(tests_mapping)
            logger.log_info("BEFORE_PARSE函数运行成功")
        except:
            logger.log_info("BEFORE_PARSE函数运行失败")
        ...
        # 加载到套件之前，用例已经解析完成
        try:
            project_mapping.get('functions').get('BEFORE_ADD')(parsed_testcases)
            logger.log_info("BEFORE_ADD函数运行成功")
        except:
            logger.log_info("BEFORE_ADD函数运行失败")
        ...
        # 运行之前，用例已经加载到套件
        try:
            project_mapping.get('functions').get('BEFORE_RUN')(test_suite)
            logger.log_info("BEFORE_RUN函数运行成功")
        except:
            logger.log_info("BEFORE_RUN函数运行失败")
        ...
        # 结果聚合之前，测试套件已运行结束
        try:
            project_mapping.get('functions').get('BEFORE_AGGREGATE')(results)
            logger.log_info("BEFORE_AGGREGATE函数运行成功")
        except:
            logger.log_info("BEFORE_AGGREGATE函数运行失败")
        ...
        # 生成测试报告之前，结果聚合已经完成
        try:
            project_mapping.get('functions').get('BEFORE_REPORT')(self._summary)
            logger.log_info("BEFORE_REPORT函数运行成功")
        except:
            logger.log_info("BEFORE_REPORT函数运行失败")
