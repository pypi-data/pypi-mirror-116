...
def load_csv_file(csv_file, functions_mapping=None, check_variables_set=None, variables_mapping=None):
    ...
    with io.open(csv_file, encoding='utf-8') as csvfile:
        ...
        for row in reader:
            ...
            csv_content_list.append({key: utils.data_change(value) for key, value in row.items()})

    from httprunner import parser
    prepare_csv_content_list = parser.prepare_lazy_data(csv_content_list, functions_mapping, check_variables_set)
    utils.DP.set_value(variables_mapping)
    parsed_csv_content_list = parser.parse_lazy_data(prepare_csv_content_list, utils.DP.get_variables())
    return parsed_csv_content_list
