...
def get_mapping_variable(variable_name, variables_mapping):
    ...
    try:
        ...
        return utils.DP.get_variables().get(variable_name) or variables_mapping[variable_name]
    except KeyError:
...
def get_mapping_function(function_name, functions_mapping):
    ...
    try:
        ...
        pass
    return utils.DP.generate_function(function_name)
...
class LazyFunction(object):
    ...
    def to_value(self, variables_mapping=None):
        ...
        if self.func_name == 'load_csv_file':
            kwargs['functions_mapping'] = self.functions_mapping
            kwargs['check_variables_set'] = self.check_variables_set
            kwargs['variables_mapping'] = variables_mapping
        if self.func_name in utils.DP.get_functions():
            return utils.DP.get_functions().get(self.func_name)(*args, **kwargs)
        ...
        for arg in self._args:
            if isinstance(arg, LazyFunction):
                if self.cached and arg.cache_key and arg.cache_key in cached_functions_mapping:
                    ...
                    value = utils.DP.get_variables().get(arg.cache_key) or cached_functions_mapping[arg.cache_key]
                else:
