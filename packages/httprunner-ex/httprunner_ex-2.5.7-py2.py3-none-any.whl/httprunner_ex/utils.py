...
def create_scaffold(project_name):
    ...
    """
    # 此处修改源码
    if os.path.isdir(project_name) and project_name != '.':
...
-
    ...
    testcase: path/to/login_complex_testcase.yml
    variables:
        device_sn: $device_sn
-
...
def BEFORE_PARSE(tests_mapping):
    """
    用例解析之前，用例加载已经完成
    :param tests_mapping: 用例字典数据
    :return:
    """


def BEFORE_ADD(parsed_testcases):
    """
    加载用例到套件之前，用例已经解析完成
    :param parsed_testcases: 已解析的用例数据（解析之后的变量为LazyString对象）列表
    :return:
    """


def BEFORE_RUN(test_suite):
    """
    运行之前，用例已经加载到套件
...
def BEFORE_AGGREGATE(results):
    """
    结果聚合之前，测试套件已运行结束
    :param results: [unittest.suite.TestSuite, HtmlTestResult]
    :return:
    """


def BEFORE_REPORT(summary):
    """
    生成测试报告之前
    :param summary: 运行结果字典
    :return:
    """
...
class DP:
    ...
    def get_value(cls, key: str):
        return cls.get_data().get(str(key))

    @classmethod
    def set_value(cls, value: dict):
        if not isinstance(value, dict):
            raise ValueError('值必须为字典类型')
        if isinstance(__builtins__, dict):
            for k, v in value.items():
                if v == 'N/A':
                    tmp = __builtins__.get(k) or v
                    __builtins__[k] = tmp
                else:
                    __builtins__[k] = v
            # __builtins__.update(value)
        else:
            for k, v in value.items():
                if v == 'N/A':
                    tmp = getattr(__builtins__, k, None) or v
                    setattr(__builtins__, str(k), tmp)
                else:
                    setattr(__builtins__, str(k), v)

    @classmethod
    def get_data(cls):
        if isinstance(__builtins__, dict):
            return __builtins__
        elif inspect.ismodule(__builtins__):
            return {k: v for k, v in __builtins__.__dict__.items()}

    @classmethod
    def get_type_data(cls, data_type_method, filter_names_tuple=()):
        if isinstance(__builtins__, dict):
            return {k: v for k, v in __builtins__.items()
                    if data_type_method(v) and k not in filter_names_tuple}
        elif inspect.ismodule(__builtins__):
            return {k: v for k, v in __builtins__.__dict__.items()
                    if data_type_method(v) and k not in filter_names_tuple}
    ...
    def get_functions(cls):
        filter_names_tuple = ('execfile', 'runfile', '_pydev_stop_at_break')
        return cls.get_type_data(inspect.isfunction, filter_names_tuple)

    @classmethod
    def get_builtins(cls):
        return cls.get_type_data(inspect.isbuiltin)

    @classmethod
    def get_class(cls):
        return cls.get_type_data(inspect.isclass)

    @classmethod
    def get_modules(cls):
        return cls.get_type_data(inspect.ismodule)

    @classmethod
    def get_methods(cls):
        return cls.get_type_data(inspect.ismethod)

    @classmethod
    def get_variables(cls):
        function_names_set = set(cls.get_functions())
        builtin_names_set = set(cls.get_builtins())
        class_names_set = set(cls.get_class())
        module_names_set = set(cls.get_modules())
        method_names_set = set(cls.get_methods())
        all_names_set = set(cls.get_data())
        other_names_set = {'credits', 'False', 'copyright', 'Ellipsis', '__spec__', 'NotImplemented', 'True', 'help',
                           'None', '__debug__', 'exit', 'quit', '__package__', '__name__', '__doc__', 'license',
                           'runfile', 'execfile'}
        return {k: cls.get_value(k) for k in (all_names_set - function_names_set - builtin_names_set - class_names_set
                                              - module_names_set - method_names_set - other_names_set)}

    @classmethod
    def generate_function(cls, name):
        from types import FunctionType
        fun = lambda *args, **kwargs: None
        return FunctionType(fun.__code__, globals(), name)


integer_compile = re.compile(r'^([0-9])|([1-9][0-9]+)$')
float_compile = re.compile(r'^([0-9]\.[0-9]+)|([1-9][0-9]+\.[0-9]+)$')
bool_compile = re.compile(r'^true|false|True|False|on|off|On|Off$')
null_compile = re.compile(r'^null|None|none$')


def data_change(raw_data):
    # for one_compile in [integer_compile, float_compile]:
    if integer_compile.match(str(raw_data)) or float_compile.match(str(raw_data)):
        return eval(raw_data)
    elif bool_compile.match(str(raw_data)):
            return True if bool_compile.match(str(raw_data)).group(0) in ['true', 'True', 'on', 'On'] else False
    elif null_compile.match(str(raw_data)):
        return
    else:
