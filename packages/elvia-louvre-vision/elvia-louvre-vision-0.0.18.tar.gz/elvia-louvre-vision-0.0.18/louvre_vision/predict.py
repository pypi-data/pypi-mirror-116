...
from louvre_vision.data_models import AppProperties, ImageRegion, ObjectDetectionPredictionResult, PluginEndpointRequest, RegionBoundingBox, ResultBundle
...
class ObjectDetectionPrediction:
    ...
    def detect_from_image_data(
        ...
        if image_entry.sasuri:
            predictions = prediction_client.detect_image_url(
                project_id=model.project_id,
                published_name=model.publish_name,
                url=image_entry.sasuri).predictions
        else:
            predictions = prediction_client.detect_image(
                project_id=model.project_id,
                published_name=model.publish_name,
