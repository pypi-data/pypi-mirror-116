...
class RollingStd():
    ...
    def calculate(self, **kwargs):
        if self.confidence:
            sigma_level = scipy.stats.norm.ppf(1-(1-self.confidence)/2) * 2
        else:
            sigma_level = 1
    ...
    def plot(self, **kwargs):
        ...
        title_type = 'confidence' if self.confidence is not None else 'standard deviation'
        title = kwargs.pop('title', f'{self.series.name} rolling {title_type}')
        ...
        # area plot
        ax.stackplot(self.rolling.index, self.rolling, **style.graphs.area.stackplot)
        ax.plot(self.rolling.index, self.rolling, **style.graphs.area.plot)
