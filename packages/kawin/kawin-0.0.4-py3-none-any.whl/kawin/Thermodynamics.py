...
class GeneralThermodynamics:
    ...
    def getEq(self, x, T, gExtra = 0, precPhase = None):
        ...
        eq = equilibrium(self.db, self.elements, [self.phases[0], precPhase], cond,
                         model=self.models, phase_records=self.phase_records, calc_opts={'pdens': self.pDens})
        return eq
    ...
    def _interdiffusivitySingle(self, x, T):
        ...
        cond[v.GE] = 0
        cond['N'] = 1
        eqPh = equilibrium(self.db, self.elements, self.phases[0], cond, model=self.models, callables=self.callables, calc_opts = {'pdens': self.pDens})
    ...
    def _getDrivingForceSampling(self, x, T, precPhase = None, returnComp = False, training = False):
        ...
        if self._pointsPrec[precPhase] is None or self._prevTemperature != T:
            self._pointsPrec[precPhase] = calculate(self.db, self.elements, precPhase, P = 101325, T = T, GE=self.gOffset, pdens = self.sampling_pDens, model=self.models, output='GM', callables=self.callables)
            if self.orderedPhase[precPhase]:
                self._orderingPoints[precPhase] = calculate(self.db, self.elements, precPhase, P = 101325, T = T, GE=self.gOffset, pdens = self.sampling_pDens, model=self.models, output='OCM', callables=self.OCMcallables[precPhase])
...
class BinaryThermodynamics (GeneralThermodynamics):
    ...
    def _interfacialCompositionFromEq(self, T, gExtra = 0, precPhase = None):
        ...
        eq = equilibrium(self.db, self.elements, [self.phases[0], precPhase], cond, model=self.models, callables=self.callables, calc_opts = {'pdens': self.pDens})
