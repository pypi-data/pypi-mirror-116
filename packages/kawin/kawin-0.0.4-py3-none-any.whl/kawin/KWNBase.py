...
class PrecipitateBase:
    ...
    def __init__(self, t0, tf, steps, phases = ['beta'], linearTimeSpacing = False, elements = ['solute']):
        ...
        self.gElastic = np.zeros(len(self.phases), dtype=np.float32)
    ...
    def setElasticEnergy(self, gElastic = None, phase = None):
        '''
        Parameters
        ----------
        gElastic : float
            Elastic energy contributions from precipitate in J/m3
        phase : str (optional)
            Phase to input elastic energy (defaults to first precipitate in list)
        '''
        index = self.phaseIndex(phase)
        self.gElastic[index] = gElastic
    ...
    def particleGibbs(self, radius, phase = None):
        ...
        '''
        index = self.phaseIndex(phase)
        return self.VmBeta[index] * (self.gElastic[index] + 2 * self.shapeFactors[index].thermoFactor(radius) * self.gamma[index] / radius)
    ...
    def printModelParameters(self):
        ...
        for p in range(len(self.phases)):
            ...
            print('\tElastic Energy (J/m3):         {:.3e}'.format(self.gElastic[p]))
