...
def imadjust(img, lhin, lhout):
    ...
    return ts.scale(img, st=lhout, sf=lhin, istrunc=True, rich=False)
...
def imadjustlog(img, lhin=None, lhout=None):
    ...
    img = 20 * ts.log10(img + ts.EPS)
    medianv = ts.median(img)
    meanv = ts.mean(img)
    maxv = ts.max(img)
    ...
    img = ts.scale(img, st=lhout, sf=lhin, istrunc=True, rich=False)
