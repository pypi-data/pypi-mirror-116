...
def dte_dtime(dte):
    """convertit une date au format date en datetime à minuit"""
    if type(dte) is date:
        midnight = datetime.min.time()
        dte = datetime.combine(dte, midnight)
...
def dtestr(dte=None, fm='%Y-%m-%dT%H:%M:%S'):
...
def dte_dayed(dte=None, b=True):
    ...
    return dte if b else dteadd(dte, 1)
...
def setdte(p_year, p_month, p_day):
...
def dtets(dte=None):
    ...
    if dte is None:
        dte = maintenant()
    elif type(dte) is date:
        pass

    return int(dte.timestamp())  # int => sans les ms


def strdte(dte, fm='%d-%m-%Y %H:%M:%S'):
...
def isotodte(dte):
    ...
    :return:
    """
    return strdte(dte, fm=ISO_FORMAT)
...
def dteferies(y=None):
    ...
    :Exemple:
        >>> dteferies ()		#Jours fériés année en cours
        >>> dteferies (2018)	# jours fériés année 2018
    ...
    for month, day in l_dte:
        dt_feries.append(date(y, month, day))
...
def timeadd(dte, nb):
    ...
    """
    return dteadd(dte, nb, 'h')


def dte_add_workday(dte, nb):
...
def fullmonth(dte):
    ...
    :rtype: str
    """
    return dtestr(dte, "%B %Y")


def dterss(dte=None,short=False):
    """Dtate au format RSS """
...
def dteTimeZoned (dte, tz=tz_cay):
    ...
    return dtestr(dte, '%Y%m%d%H%M00%z')