...
@dutils
def cli_pi_play_sound(sound_file, sound_source="@defaults/ding.mp3", mode="async", **kwargs):
	"""
	...
	sound_source_load = os.path.dirname(os.path.abspath(__file__))+sound_source.replace('@', '/') \
		if '@' in sound_source else sound_source

	if mode == "async":
		return cli_async_call(['omxplayer', '--no-keys', '-o', sound_source_load, sound_file, '&'])
	elif mode == "normal":
		return cli_normal_call(['omxplayer', '--no-keys', '-o', sound_source_load, sound_file, '&'])
