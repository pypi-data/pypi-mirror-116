...
class OrderInfo:
	...
	* payment_method
	...
	def __init__(self, current_doday_item=[], current_info={}, **kwargs):
		...
		self._handle_direct_mapping(self.DIRECT_MAPPING_LIST, **kwargs)
		self._handle_special_mapping(self.SPECIAL_ORDER_INFO_JSON_MAPPING, **kwargs)
	...
	# -------------------------------------------------------
	def _handle_direct_mapping(self, direct_mapping_list, **kwargs):
		"""
	...
	def _handle_special_mapping(self, special_mapping_dict, **kwargs):
		"""
		...
		for key in special_mapping_dict:
			if isinstance(special_mapping_dict[key], dict):
				...
				setattr(self, key, kwargs.get(value_tuple[0], value_tuple[1]))
