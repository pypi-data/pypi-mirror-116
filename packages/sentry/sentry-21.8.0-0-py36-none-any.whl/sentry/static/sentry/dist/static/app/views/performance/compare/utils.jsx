...
exports.generateCSSWidth = exports.boundsGenerator = exports.isOrphanDiffSpan = exports.getSpanDescription = exports.getSpanOperation = exports.getSpanID = exports.getSpanDuration = exports.diffTransactions = exports.isTransactionEvent = void 0;
var tslib_1 = require("tslib");
var wink_jaro_distance_1 = tslib_1.__importDefault(require("wink-jaro-distance"));
...
function diffTransactions(_a) {
    ...
    try {
        ...
        for (var _f = tslib_1.__values(Object.entries(baselineTrace.childSpans)), _g = _f.next(); !_g.done; _g = _f.next()) {
            var _h = tslib_1.__read(_g.value, 2), parentSpanId = _h[0], children = _h[1];
    ...
    try {
        for (var _j = tslib_1.__values(Object.entries(regressionTrace.childSpans)), _k = _j.next(); !_k.done; _k = _j.next()) {
            var _l = tslib_1.__read(_k.value, 2), parentSpanId = _l[0], children = _l[1];
    ...
    var spansToBeCompared = [
        {
            type: 'root',
            baselineSpan: utils_1.generateRootSpan(baselineTrace),
            regressionSpan: utils_1.generateRootSpan(regressionTrace),
        },
    ];
    while (spansToBeCompared.length > 0) {
        ...
        if (matchableSpans({ baselineSpan: baselineSpan, regressionSpan: regressionSpan }) === 0) {
            if (currentSpans.type === 'root') {
                ...
                rootSpans.push.apply(rootSpans, tslib_1.__spreadArray([], tslib_1.__read(spanComparisonResults)));
            }
        ...
        spansToBeCompared.push.apply(spansToBeCompared, tslib_1.__spreadArray([], tslib_1.__read(comparablePairs)));
...
exports.diffTransactions = diffTransactions;
function createChildPairs(_a) {
    ...
    var comparablePairs = [];
    var children = [];
    var remainingRegressionChildren = tslib_1.__spreadArray([], tslib_1.__read(regressionChildren));
    var _loop_1 = function (baselineSpan) {
        ...
        var _d = candidates.reduce(function (bestCandidate, nextCandidate) {
            ...
            return bestCandidate;
        }), regressionSpan = _d.regressionSpan, index = _d.index;
    ...
    try {
        for (var baseChildren_1 = tslib_1.__values(baseChildren), baseChildren_1_1 = baseChildren_1.next(); !baseChildren_1_1.done; baseChildren_1_1 = baseChildren_1.next()) {
    ...
    try {
        ...
        for (var remainingRegressionChildren_1 = tslib_1.__values(remainingRegressionChildren), remainingRegressionChildren_1_1 = remainingRegressionChildren_1.next(); !remainingRegressionChildren_1_1.done; remainingRegressionChildren_1_1 = remainingRegressionChildren_1.next()) {
...
function jaroSimilarity(thisString, otherString) {
    ...
    var jaroDistance = wink_jaro_distance_1.default(thisString, otherString).distance;
...
function getSpanOperation(diffSpan) {
    switch (diffSpan.comparisonResult) {
        case 'matched': {
            return diffSpan.op;
        }
...
function getSpanDescription(diffSpan) {
    switch (diffSpan.comparisonResult) {
        case 'matched': {
...
exports.getSpanDescription = getSpanDescription;
function isOrphanDiffSpan(diffSpan) {
    switch (diffSpan.comparisonResult) {
        case 'matched': {
            return utils_1.isOrphanSpan(diffSpan.baselineSpan) || utils_1.isOrphanSpan(diffSpan.regressionSpan);
        }
        case 'baseline': {
            return utils_1.isOrphanSpan(diffSpan.baselineSpan);
        }
        case 'regression': {
            return utils_1.isOrphanSpan(diffSpan.regressionSpan);
        }
        default: {
            throw Error('Unknown comparisonResult');
        }
...
exports.isOrphanDiffSpan = isOrphanDiffSpan;
...
function boundsGenerator(rootSpans) {
    ...
    //            comparison page.
    var largestDuration = Math.max.apply(Math, tslib_1.__spreadArray([], tslib_1.__read(rootSpans.map(function (rootSpan) {
...
function generateCSSWidth(width) {
    if (!width) {
        return undefined;
    }
    switch (width.type) {
        ...
        case 'WIDTH_PERCENTAGE': {
            return utils_2.toPercent(width.width);
        }
        default: {
...
exports.generateCSSWidth = generateCSSWidth;
