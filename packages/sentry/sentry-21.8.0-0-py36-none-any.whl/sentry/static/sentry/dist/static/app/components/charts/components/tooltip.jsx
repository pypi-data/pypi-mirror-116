...
function defaultFormatAxisLabel(value, isTimestamp, utc, showTimeInTooltip, bucketSize) {
    ...
    if (!bucketSize) {
        var format = ("MMM D, YYYY " + (showTimeInTooltip ? dates_1.getTimeFormat() : '')).trim();
        return dates_1.getFormattedDate(value, format, { local: !utc });
    }
    var now = moment_1.default();
    var bucketStart = moment_1.default(value);
    var bucketEnd = moment_1.default(value + bucketSize);
    var showYear = now.year() !== bucketStart.year() || now.year() !== bucketEnd.year();
    var showEndDate = bucketStart.date() !== bucketEnd.date();
    var formatStart = ("MMM D" + (showYear ? ', YYYY' : '') + " " + (showTimeInTooltip ? dates_1.getTimeFormat() : '')).trim();
    var formatEnd = ("" + (showEndDate ? "MMM D" + (showYear ? ', YYYY' : '') + " " : '') + (showTimeInTooltip ? dates_1.getTimeFormat() : '')).trim();
    return dates_1.getFormattedDate(bucketStart, formatStart, {
        local: !utc,
    }) + " \u2014 " + dates_1.getFormattedDate(bucketEnd, formatEnd, { local: !utc });
}
...
function getFormatter(_a) {
    ...
    var formatter = function (seriesParamsOrParam) {
        ...
        if (!Array.isArray(seriesParamsOrParam) &&
            ...
            var truncatedName = utils_1.truncationFormatter(seriesParamsOrParam.data.labelForValue, truncate);
        ...
        return [
            ...
            seriesParams
                ...
                .map(function (s) {
                var _a;
                var formattedLabel = nameFormatter(utils_1.truncationFormatter((_a = s.seriesName) !== null && _a !== void 0 ? _a : '', truncate));
...
function Tooltip(_a) {
    ...
    return tslib_1.__assign({ show: true, trigger: 'item', backgroundColor: 'transparent', transitionDuration: 0, padding: 0, 
