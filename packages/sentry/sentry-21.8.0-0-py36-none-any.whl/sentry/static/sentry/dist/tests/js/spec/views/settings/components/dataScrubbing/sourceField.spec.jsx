...
var sourceField_1 = tslib_1.__importDefault(require("app/views/settings/components/dataScrubbing/modals/form/sourceField"));
var utils_1 = require("app/views/settings/components/dataScrubbing/utils");
function renderComponent(_a) {
    var _b = _a.value, value = _b === void 0 ? '$string' : _b, _c = _a.onChange, onChange = _c === void 0 ? jest.fn() : _c, props = tslib_1.__rest(_a, ["value", "onChange"]);
    return enzyme_1.mountWithTheme(<sourceField_1.default isRegExMatchesSelected={false} suggestions={utils_1.valueSuggestions} onChange={onChange} value={value} {...props}/>);
}
describe('Source', function () {
    ...
    it('display binaryOperatorSuggestions if penultimateFieldValue has type string', function () {
        ...
        // &&
        expect(suggestions.at(0).text()).toEqual(utils_1.binarySuggestions[0].value);
        // ||
        expect(suggestions.at(1).text()).toEqual(utils_1.binarySuggestions[1].value);
    });
    it('display defaultSuggestions + unaryOperatorSuggestions, if penultimateFieldValue has type binary', function () {
        ...
        expect(suggestions).toHaveLength(18);
        // !
        expect(suggestions.at(17).text()).toEqual(utils_1.unarySuggestions[0].value);
    });
    ...
    it('display defaultSuggestions if penultimateFieldValue has type unary', function () {
        ...
        // everywhere
        expect(suggestions.at(0).text()).toEqual(utils_1.valueSuggestions[0].value + "(" + utils_1.valueSuggestions[0].description + ")");
    });
    it('click on a suggestion should be possible', function () {
        ...
        expect(wrapper.state().fieldValues[2].value).toBe(utils_1.valueSuggestions[1].value);
    });
