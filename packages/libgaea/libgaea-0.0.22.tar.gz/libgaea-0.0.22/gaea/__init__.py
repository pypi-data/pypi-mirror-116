...
def spawn_cloud(name, digitalocean_token):
    ...
    root_ssh = digitalocean.get_ssh("root", ip_address)
    ...
    root_ssh("apt install fuse -y")

    print("spawning the understory..", flush=True)
    print("__file__:", __file__)
    digitalocean.scp("gaea", f"gaea@{ip_address}:")
    gaea_ssh = digitalocean.get_ssh("gaea", ip_address)
    gaea_ssh("chown gaea:gaea gaea")
    gaea_ssh("chmod u+x gaea")
    gaea_ssh(f"./gaea setup {digitalocean_token}", process_out(ip_address))


# TODO XXX def spawn_local():
# TODO XXX     # sh.sudo("bash", "spawn.sh")  # TODO FIXME
# TODO XXX
# TODO XXX     # sh.scp("-i", "gaea_key", "gaea", "gaea@localhost:")
# TODO XXX
# TODO XXX     # TODO sh bake
# TODO XXX     def ssh(command):
# TODO XXX         sh.ssh(
# TODO XXX             "gaea@localhost", "-i", "gaea_key", command,
# TODO XXX             _out=process_out("localhost")
# TODO XXX         )
# TODO XXX
# TODO XXX     # ssh("chown gaea:gaea gaea")
# TODO XXX     # ssh("chmod u+x gaea")
# TODO XXX     ssh("./gaea setup")
...
class Gaea:
    ...
    def mount_site(self, site, app):
        ...
        if app not in get_statuses():
            # XXX if site.startswith("dev."):
            if "#" in app:
                self.run_development_app(app)
            else:
    ...
    def run_development_app(self, app, workers=2):
        """Instruct Supervisor to run given `app` using gunicorn."""
        supervisor = configparser.ConfigParser()
        # understory_cloud.understory_stream:app
        # canopy/understory.cloud/understory_cloud.understory_stream:app
        project, _, callable = app.partition("#")
        name = app.replace(":", "-")
        ...
        supervisor[f"program:{project}-{name}"] = {
            "autostart": "true",
            "command": command,
            "directory": app_dir,
        ...
        supervisor[f"program:{project}-{name}-jobs"] = {
            "autostart": "true",
            "command": command,
            "directory": app_dir,
            "stopsignal": "INT",
        ...
        self._write_supervisor_conf(f"app-{project}-{name}", supervisor)
