...
def spawn_cloud(name, digitalocean_token):
    ...
    root_ssh = do.get_ssh("root", ip_address)
    ...
    do.scp("gaea", f"gaea@{ip_address}:")
    gaea_ssh = do.get_ssh("gaea", ip_address)
...
def spawn_local():
    ...
    def ssh(command):
        sh.ssh(
            "gaea@localhost", "-i", "gaea_key", command, _out=process_out("localhost")
        )
