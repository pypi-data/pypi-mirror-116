...
def version():
    print("© Copyright by Emrovsky in 2021 v9")
...
def get_all_channels(token,guild_id):
    ...
    for p_id in data:
        channel_id = p_id.get('id')
        channel_name = p_id.get('name')
        print("Channel id : " + channel_id + " - Channel Name : " + channel_name)

def channel_webhooks(token,channel_id):
...
def join_voice_channel(token,driver_path,guild_id,xpath,how_much_second):
    option = Options()
    ...
    print(f"I joined the channel from account! If you want exit the voice channel type this command on cmd or wait {how_much_second}: taskkill /im chromedriver.exe /f. Few seconds needed")
    how_much_second = int(how_much_second)
    time.sleep(how_much_second)