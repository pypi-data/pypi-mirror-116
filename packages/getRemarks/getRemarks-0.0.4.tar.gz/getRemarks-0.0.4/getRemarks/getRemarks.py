...
class Grade:#This module will get the grades of the students according to their marks
    ...
    def __init__(self,marksSecured,marksOutOf=None):
        if marksOutOf is None:
            exit("Total marks part is missing, should be after marks scored followed by comma. Say (200, 500)")
            os._exit(1)
        elif marksOutOf <0 or marksSecured<0:
            exit("This module works with postive values only!")
            os._exit(1)
        elif marksOutOf==0:
            exit("How can total marks be 0?")
            os._exit(1)
        else:
