class Grade:#This module will get the grades of the students according to their marks
    ...
    def __init__(self,marksSecured,marksOutOf=None):
        if marksOutOf is None:
            print("Total marks part is missing, should be after marks scored followed by comma. Say (200, 500). WARNING: Re-run the package from the start!")
            
        elif marksOutOf <0 or marksSecured<0:
            print("This module works with postive values only. WARNING: Re-run the package from the start!")
            
        elif marksOutOf==0:
            print("How can total marks be 0? WARNING: Re-Run the code!")
    ...
    def getGrade(self):#this will calculate grade
        totalVal=str(self.total)
        total= "{:.2f}".format(totalVal)
        if self.marksOutOf=='': 
            self.grade=99
