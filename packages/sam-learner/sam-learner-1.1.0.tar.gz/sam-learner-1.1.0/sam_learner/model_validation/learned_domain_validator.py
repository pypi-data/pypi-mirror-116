...
class DomainValidator:
	...
	def validate_plan(self, domain_path: str, problem_path: str, plan_path: str) -> ValidationResult:
		"""Validate the correctness of the learned domain against the domain learned using the learner algorithm.

		:param domain_path: the path to the learned domain file.
		...
		trajectory_generator = TrajectoryGenerator(domain_path, problem_path)
		try:
	...
	def log_model_safety_report(
		...
		for generated_domain_path in Path(generated_domains_directory_path).glob("*.pddl"):
			...
			for problem_path, plan_path in domain_successful_plans.items():
				...
				validation_status = self.validate_plan(str(generated_domain_path), problem_path, plan_path)
			...
			if all(validated_plans):
				self.on_validation_success(output_statistics_path, plan_paths, test_set_statistics)
				return
	...
	def on_validation_success(
			self, output_statistics_path: str, plan_paths: List[str],
		...
		:param output_statistics_path: the path to the output file.
		:param plan_paths: the paths to the generated plan files.
...
if __name__ == '__main__':
	try:
		...
		validator.log_model_safety_report(
			domain_name=args[1],
			generated_domains_directory_path=args[2],
			test_set_directory_path=args[3],
			output_statistics_path=args[4])
