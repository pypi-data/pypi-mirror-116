...
from quicly.result import QxResult, QxError, QxFinish, QxHttpResult
...
class QxRequest(object):
  def __init__(self, method: str, url: str, headers: Optional[Dict[str, str]] = None, cookies: Optional[Dict[str, str]] = None, body: Optional[bytes] = None, request_uuid: str = None, request_time: Optional[datetime] = None):
...
class QxServer(object):
  ...
  def handle(self, request: QxRequest):
    ...
    if isinstance(result.data, QxHttpResult):
      response = QxResponse(
        request=request,
    ...
    response.headers['Server'] = 'Quicly'
