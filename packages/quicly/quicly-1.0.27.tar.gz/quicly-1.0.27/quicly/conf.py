...
class QxConf(dict):
  ...
  def update_json(self, json_str: str):
    try:
  ...
  def update_file(self, path: str, encoding: str = 'utf-8'):
    if os.path.isfile(path):
      with open(path, 'r', encoding=encoding) as fo:
        self.update_json(fo.read())
    return self
...
class QxGlobalConf(QxConf):
  ...
  def __init__(self):
    ...
    self._i18n = QxConf()
    self._i18n.update_file(os.path.join(self._root, 'i18n', 'zh-cn.json'))

    self._code = QxConf()

    code = QxConf()
    code.update_file(os.path.join(self._root, 'code.json'))
