class FixedTarget:
    ...
    ???
    """
    def __init__(self,propfile,verbal=True,templatefile='template_fixed_target.json',read_propfile=True,read_templatefile=True):
        import rolling_snapshot_proposal_editor as RNPE
        self.propfile = propfile
        self.verbal = verbal
        self.templatefile = RNPE.__path__ + '/' + templatefile
        if read_propfile: self.prop = self._read_propfile() and (if verbal: print('Read propfile. Access by self.prop. \n'))
        if read_templatefile: self.template = self._read_templatefile() and (if verbal: print('Read templatefile. Access by self.template. \n'))
    def _read_propfile(self):
        f = open(self.propfile,'r')
        self.prop = f.readlines()
        f.close()
    def _read_templatefile(self):
        import json
        f = open(self.templatefile,'r')
        self.template = json.loads(f.readlines()[0])
        f.close()
