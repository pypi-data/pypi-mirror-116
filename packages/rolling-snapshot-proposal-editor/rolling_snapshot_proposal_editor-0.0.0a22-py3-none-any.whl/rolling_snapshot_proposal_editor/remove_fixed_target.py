def remove_fixed_target(propread,target_number,outname,do_remove_visits=True,verbal=True):
    ...
    target_name_list = find_targetname_in_targetlist(propread)
    ...
    for ii,i in enumerate(propread):
        ...
        try:
            ...
            pass
    if verbal: print(linenum)
    #####
    if verbal: print('remove ...\n')
    t1 = propread[:linenum-1]
    t2 = propread[linenum+N:]
    #####
    if not do_remove_visits:
        print('Creating self.newprop ...\n')
        f = open(outname,'w')
        f.writelines(t1)
        f.writelines(t2)
        f.close()   
