...
def dispatch_descr(names, min_dir, z_dif, pl_dif):
    dispatch = dict(
        ...
        nodedata=nodedata_descr(
            names["nodes"], min_dir["hydro_exists"], shape=None, pos=6
        ),
    ...
    if min_dir["building_exists"]:
        dispatch["building"] = building_descr(
            names["buildings"], z_dif, pl_dif, shape=None, pos=5
        )
    return dispatch
...
def predicted_descr(names, min_dir, z_dif, pl_dif):
    predicted = dict(
        ...
        nodedata=nodedata_descr(
            names["nodes"], min_dir["hydro_exists"], shape="h", pos=8
        ),
    ...
    if min_dir["building_exists"]:
        predicted["building"] = building_descr(
            names["buildings"], z_dif, pl_dif, "h", pos=7
        )
    return predicted
...
def solution_descr(names, min_dir):
    ...
    if min_dir["building_exists"]:
        solution["building"] = building_descr(
            names["buildings"], None, None, "h", pos=9
        )
    return solution
...
def result_description(names, dimensions, minimization_directives, zones, pl):
    ...
    disp = realize_all_columns(
        dispatch_descr(names, minimization_directives, z_dif, pl_dif), dimensions
    )
    # Update predicted description.
    pred = realize_all_columns(
        predicted_descr(names, minimization_directives, z_dif, pl_dif), dimensions
    )
    # Update solution description.
    solu = realize_all_columns(
        solution_descr(names, minimization_directives), dimensions
    )
