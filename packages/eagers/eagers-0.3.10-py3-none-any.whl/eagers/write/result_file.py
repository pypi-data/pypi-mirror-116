...
def new_result_file(proj_name, names, dimensions, minimization_directives, zones, pl):
...
def result_file_setup(preload, plant, date, zones, pl):
    ...
    for net in preload["subnet"]["network_names"]:
        ...
        names["lines"].extend(
            [
                snl["node1"][i] + "_to_" + snl["node2"][i]
                for i in range(len(snl["node1"]))
            ]
        )
    ...
    if "district_heat" in preload["subnet"]:
        names["heating_nodes"] = [
            n[0] for n in preload["subnet"]["district_heat"]["nodes"]
        ]
    else:
        names["heating_nodes"] = []
    if "district_cool" in preload["subnet"]:
        names["cooling_nodes"] = [
            n[0] for n in preload["subnet"]["district_cool"]["nodes"]
        ]
    else:
        names["cooling_nodes"] = []
    names["hydro"] = [
        g["name"] for g in preload["gen_qp_form"] if g["type"] == "HydroStorage"
    ]
    dimensions = dict(
        horizon=len(date) - 1,
        zone=max(zones),
    ...
    return names, dimensions, minimization_directives
