...
def dispatch_record(gen, dispatch, observer, market, subnet, actual_data, solution, project_name):
...
def predict_record(gen, predicted, observer, market, subnet, date, forecast, solution, project_name):
    n_s = len(date) - 1  # Numer of steps.
    predicted['timestamp'] = forecast['timestamp']
    predicted['cost'] = net_cost(gen, solution['dispatch'], date, market)
    for i,k in enumerate(observer['gen_names']):
...
def record_initial_conditions(data_t0, dispatch, subnet, observer, project_name):
