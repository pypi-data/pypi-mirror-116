...
def dispatch_step(
    ...
    for t in range(n_s):
        ...
        marginal = net_marginal_cost(gen,net_abbrev,market,step_profile,sc,dt[t],any([v_h[n][t] for n in range(len(v_h))]))#update marginal cost
        fb = building_forecast_now(forecast['building'],t)
        min_power, max_power = constrain_min_max(gen, limit, ic, dt[t], sum(dt[0:t+1]))
        update_matrices_step(gen,market,subnet,options,one_step,fb,renewable,demand,sc,marginal,stor_power[t],dt[t],min_power, max_power,step_profile,flt,flc,bat)
