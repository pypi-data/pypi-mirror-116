...
def make_sql_str_util(sql_type, table, select_target=None, where=None, update_target=None, insert_target=None,
                      order_by=None):
    where_info = f" where {' and '.join([f'{k}={format_quot_str_util(v)}' for k, v in where.items()])}" if where else ''
    if sql_type == 'insert':
        ...
        order_str = ' order by ' + ', '.join(
            [f'{k} {v}' for k, v in collections.OrderedDict(order_by).items()]) if order_by else ''
        return f'select {target_str} from {table}{where_info}{order_str};'
...
# def date_to_time_stamp(date_str):
