...
"""
    ...
    Правила записи данных в файлах .sd, примеры и комментарии находятся в файле documentation/syntax_of_sd_format.txt

"""
import re

version = '2021.08.02-p[3.8]wr-01.000.000'  # year.month.day-version_type-major.minor.patch
...
class SdDataDecoding:
    ...
    def multiline_string_value_parsing(self):
        ...
        if cur_primary_key in raw_data_dict:
            ...
            if res_dict[cur_primary_key] != '' and 'error' not in res_dict[cur_primary_key]:
                a = res_dict[cur_primary_key].replace('\n', ' ')
    ...
    def triple_quoted_single_string_parsing(self):
        """
            Метод осуществляющий поиск списка или строкового значения в строке, в которой могут быть тройные кавычки
...
class SdDataEncoding:
    ...
    def sd_data_encoding_management(self):
        source_dictionary = self.source_dictionary
        with open(self.file_path, mode='a', encoding='utf-8') as self.file:
...
def documentation_output_about_sd_format():
    file_path = '11_documentation/syntax_of_sd_format.txt'
    with open(file_path, mode='r', encoding='utf-8') as file:
        content = file.read()
        print()
        print(content)
