...
def check_json_format(_s):
    ...
    if type(_s[0]) == dict:
        # 校验 key 是否一致
        keys = [_.keys() for _ in _s]
        # print(keys)
        if keys.count(keys[0]) != len(keys):
            return False
        # 校验 value 是否一致
        value_types = [[type(value) for value in _.values()] for _ in _s]
        # print(value_types)
        if value_types.count(value_types[0]) != len(value_types):
            return False
        # 对 value 为 dict 类型的值进行递归校验
        for key in _s[0].keys():
            if type(_s[0][key]) == dict:
                if not check_json_format([_[key] for _ in _s]):
                    return False
    elif type(_s[0]) == list:
        if not check_json_format([sub_ for _ in _s for sub_ in _]):
            return False
    return True
...
class ProxyPool:
    def __init__(self, data5u_order=None, xiaoxiang_app_key=None, xiaoxiang_app_secret=None, zhima_app_key=None):
        """
        代理池初始化
        :param data5u_order:
        :param xiaoxiang_app_key:
        :param xiaoxiang_app_secret:
        :param zhima_app_key:
        """
        self.proxies_list = []
        self.data5u_order = data5u_order
        self.xiaoxiang_app_key = xiaoxiang_app_key
        self.xiaoxiang_app_secret = xiaoxiang_app_secret
        self.zhima_app_key = zhima_app_key
            ...
            if self.data5u_order:
                ...
                while True:
                    try:
                        ...
                        print(repr(e))
                    time.sleep(1)
            if self.xiaoxiang_app_key and self.xiaoxiang_app_secret:
                while True:
                    try:
                        self.proxies_list.append(
                            self.get_proxies_by_xiaoxiang(self.xiaoxiang_app_key, self.xiaoxiang_app_secret))
                    except Exception as e:
                        print(repr(e))
                    time.sleep(10)
            if self.zhima_app_key:
                while True:
                    try:
                        self.proxies_list.append(self.get_proxies_by_zhima(self.zhima_app_key))
                    except Exception as e:
                        print(repr(e))
                    time.sleep(10)
    ...
    def get_proxies(self):
        ...
        :return:
        """
        return random.choice(self.proxies_list[-2:])
    ...
    def get_proxies_by_data5u(order, is_random=1):
        ...
        if response.status_code != 200:
            raise Exception("proxy error, status code: {}, response: {}".format(response.status_code, response.text))
        ...
        if response["success"]:
            ...
            raise Exception("proxy error, status code: 200, response: {}".format(response["msg"]))

    @staticmethod
    def get_proxies_by_xiaoxiang(app_key, app_secret):
        """
        通过小象代理（https://www.xiaoxiangdaili.com/）API接口获取代理
        :param app_key:
        :param app_secret:
        :return:
        """
        url = "https://api.xiaoxiangdaili.com/ip/get?appKey={}&appSecret={}&cnt=1&wt=json".format(app_key, app_secret)
        proxies = dict()
        response = requests.get(url, timeout=(10, 10))
        if response.status_code != 200:
            raise Exception("proxy error, status code: {}, response: {}".format(response.status_code, response.text))
    ...
    @staticmethod
    def get_proxies_by_zhima(app_key):
        """
        通过芝麻HTTP代理（https://www.zhimaruanjian.com/）API接口获取代理
        :param app_key:
        :return:
        """
        url = "http://api.wandoudl.com/api/ip?app_key={}&num=1&type=2".format(app_key)
        proxies = dict()
        response = requests.get(url, timeout=(10, 10))
        if response.status_code != 200:
            raise Exception("proxy error, status code: {}, response: {}".format(response.status_code, response.text))
...
class Cracker:
    def __init__(self, process_number=1):
        self.passwords_queue = multiprocessing.Manager().Queue(100000)
...
if __name__ == '__main__':
    p = ProxyPool(zhima_app_key="2e9d7ca82c5d8f2f91cde0eea8d54a16")
    pass
