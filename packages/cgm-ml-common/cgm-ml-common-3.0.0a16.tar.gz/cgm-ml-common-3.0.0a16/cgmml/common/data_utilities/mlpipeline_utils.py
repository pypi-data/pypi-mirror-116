...
def preprocess(depthmap: np.ndarray) -> np.ndarray:
    ...
    assert width / IMAGE_TARGET_WIDTH == height / IMAGE_TARGET_HEIGHT
...
def create_layers_rgbd(depthmap_fpath: str, rgb_fpath) -> Tuple[np.ndarray, dict]:
    dmap = Depthmap.create_from_zip_absolute(depthmap_fpath, rgb_fpath, CALIBRATION_FPATH, should_rotate_rgb=True)
...
class ArtifactProcessor:
    def __init__(self, input_dir: str, output_dir: str, dataset_type: str):
    ...
    def create_and_save_pickle(self, artifact_dict: dict) -> str:
        ...
        if self.dataset_type == 'depthmap':
            ...
            zip_input_full_path = f"{self.input_dir}/{artifact_dict['file_path_rgb']}"
            layers, metadata = create_layers_rgbd(zip_input_full_path)
        else:
