...
class Depthmap:
    ...
    def convert_2d_to_3d_oriented_point(self, sensor: int, x: float, y: float, depth: float) -> list:
        """Convert point in pixels into point in meters (applying rotation)"""
        res: np.ndarray = self.convert_2d_to_3d(sensor, x, y, depth)
        if not res.any():
            return res
        ...
        if not self.device_pose:
            logger.warn("Device pose matrix was not provided.")
            return res
        try:
            res = matrix_transform_point(np.array(res), self.device_pose_arr)
            res[1] = -res[1]
        except NameError:
            pass
        return res
    ...
    def get_angle_between_camera_and_floor(self) -> float:
        ...
        vector = self.convert_2d_to_3d_oriented_point(1, centerx, centery, 1.0)
