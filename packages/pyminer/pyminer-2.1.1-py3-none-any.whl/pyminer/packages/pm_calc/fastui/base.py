...
class BaseOperationDialog(QDialog):
    ...
    def help(self) -> None:
        ...
        center_window(dlg)
        dlg.showEvent = lambda evt: dlg.setGeometry(dlg.x(), dlg.y(), 600, 500)
        dlg.exec_()
    ...
    def get_assignment_code(self) -> str:
        ...
        if hasattr(self, "combo_box"):
            default_name = self.combo_box.currentText() if self.combo_box.currentText().isidentifier() else "a"
        identifier = input_identifier(parent=self, default_name=default_name, allow_existing_name=True)
        if identifier != '':
