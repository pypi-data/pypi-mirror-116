...
def domainsearch(BASE_DIR):
    ...
    cursor.execute("SELECT id, dsc_sequencia, cd_requisicao FROM pipeline_requisicao WHERE sn_cdsearch = False and sn_conclusao = False")
    ...
    for job in jobs:
        ...
        for f in os.listdir(os.path.join(job[2], "domainsearch")):
            if f.endswith(".txt"):
                ...
                break
        if flag:
            cursor.execute(f"""UPDATE pipeline_requisicao SET sn_cdsearch = True, sn_error = True, sn_conclusao = False WHERE id = {job[0]}""")
            conn.commit()
    cursor.close()
    conn.close()
