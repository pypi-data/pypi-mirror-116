...
class DialogueStateTracker:
    ...
    app_events = {'capture_utterance', 'dtmf', 'get_speech', 'hangup', 'send_sip_info'}
    ...
    def add_conversation_steps(self, event: Event) -> None:
        ...
        if isinstance(event, BotUttered):
            if event.data and 'custom' in event.data and 'type' in event.data['custom'] and event.data['custom']['type'] in self.app_events:
                should_add_step = True
            else:
                should_add_step = False
        elif isinstance(event, UserUttered):
            if event.text != None:
                should_add_step = not (event.text.startswith('/greet') or event.text.startswith('/restart')) and not (event.text == '' and not event.metadata)
        if should_add_step:
            self.total_steps += 1
