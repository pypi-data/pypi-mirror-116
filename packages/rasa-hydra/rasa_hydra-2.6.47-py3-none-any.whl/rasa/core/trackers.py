...
class DialogueStateTracker:
    ...
    def add_conversation_steps(self, event: Event) -> None:
        ...
        if isinstance(event, BotUttered):
            ...
            if event.text != None:
                should_add_step = not (event.text.startswith('/greet') or event.text.startswith('/disconnect') or event.text.startswith('/restart')) and not (event.text == '' and not event.metadata)
