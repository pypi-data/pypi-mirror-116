...
class IndexedUnpool(torch.nn.Module):
    ...
    def __init__(self, indices):
        ...
        for i in torch.unique(indices):
            if i != -1:
                position_per_pixel.append(torch.where(indices == i)[1])
...
def resize_hexagonal_image(feature, net, camera_parameters):
    ...
    camera_parameters (dict): contains information about the camera frame
    ...
    initial_matrix = ic_utils.create_index_matrix(camera_parameters['nbRow'],
    ...
    for child in net.feature.children():
        if hasattr(child, 'pooled_matrix'):
            matrices.append(child.pooled_matrix)
...
class OpenBlackBox:
    ...
    def __init__(self, net, class_index=0, cuda=True, camera_type='LST_LSTCam'):
        ...
        self.camera_parameters = utils.load_camera_parameters(camera_type)
    ...
    def plot_activation(self, tasks=None, threshold=0.0, blend=True, pixel_size=10, normalize=True, cmap=None):
        ...
        if self.image.dim() == 3:
            ...
            im = axes[0, 0].scatter(list(map(lambda x: x[0], self.camera_parameters['pixelsPosition'])),
                                    list(map(lambda x: x[1], self.camera_parameters['pixelsPosition'])), s=pixel_size,
                                    c=self.image[0], marker=(6, 0, 0))
            axes[0, 0].axis('equal')
            peak = axes[1, 0].scatter(list(map(lambda x: x[0], self.camera_parameters['pixelsPosition'])),
                                      list(map(lambda x: x[1], self.camera_parameters['pixelsPosition'])), s=pixel_size,
                                      c=self.image[1], marker=(6, 0, 0))
            axes[1, 0].axis('equal')
        else:
        ...
        if self.true_signal is not None:
            if self.true_signal.dim() == 3:
                ...
                im = axes[0, 1].scatter(list(map(lambda x: x[0], self.camera_parameters['pixelsPosition'])),
                                        list(map(lambda x: x[1], self.camera_parameters['pixelsPosition'])),
                                        s=pixel_size, c=self.true_signal[0], marker=(6, 0, 0))
                axes[0, 1].axis('equal')
                peak = axes[1, 1].scatter(list(map(lambda x: x[0], self.camera_parameters['pixelsPosition'])),
                                          list(map(lambda x: x[1], self.camera_parameters['pixelsPosition'])),
                                          s=pixel_size, c=self.true_signal[1], marker=(6, 0, 0))
                axes[1, 1].axis('equal')
            else:
                print('Unknown image dimension, should be 2 or 3')
                raise ValueError
        ...
        for i, task in enumerate(tasks, start=1+col_true_signal):
            ...
            if self.image.dim() == 3:
                ...
                im = axes[0, i].scatter(list(map(lambda x: x[0], self.camera_parameters['pixelsPosition'])),
                                        list(map(lambda x: x[1], self.camera_parameters['pixelsPosition'])),
                                        s=pixel_size,
                                        c=charge_activation, marker=(6, 0, 0))
                axes[0, i].axis('equal')
                peak = axes[1, i].scatter(list(map(lambda x: x[0], self.camera_parameters['pixelsPosition'])),
                                          list(map(lambda x: x[1], self.camera_parameters['pixelsPosition'])),
                                          s=pixel_size,
                                          c=peakpos_activation, marker=(6, 0, 0))
                axes[1, i].axis('equal')
            else:
                print('Unknown image dimension, should be 2 or 3')
                raise ValueError
...
class GradCam(OpenBlackBox):
    ...
    camera_type (str): camera type of input images

    """
    def __init__(self, net, class_index=0, cuda=True, camera_type='LST_LSTCam'):

        super(GradCam, self).__init__(net, class_index=class_index, cuda=cuda, camera_type=camera_type)
    ...
    def _resize_image(self, cam):
        if self.image.dim() == 3:
            ...
            return resize_hexagonal_image(cam, self.net, self.camera_parameters)
...
class GuidedBackprop(OpenBlackBox):
    ...
    def __init__(self, net, class_index=0, cuda=True, camera_type='LST_LSTCam'):
        ...
        super(GuidedBackprop,  self).__init__(net, class_index=class_index, cuda=cuda, camera_type=camera_type)
...
class GuidedGradCam(OpenBlackBox):
    ...
    def __init__(self, net, class_index=0, cuda=True, camera_type='LST_LSTCam'):
        ...
        super(GuidedGradCam, self).__init__(net, class_index=class_index, cuda=cuda, camera_type=camera_type)

        self.camera_parameters = utils.load_camera_parameters(camera_type)
        self.guided_backprop = GuidedBackprop(net, class_index=class_index, cuda=cuda, camera_type=camera_type)
        self.gradcam = GradCam(net, class_index=class_index, cuda=cuda, camera_type=camera_type)
...
class AttentionMaps(OpenBlackBox):
    ...
    def __init__(self, net, class_index=0, cuda=True, camera_type='LST_LSTCam'):
        ...
        super(AttentionMaps, self).__init__(net, class_index=class_index, cuda=cuda, camera_type=camera_type)
