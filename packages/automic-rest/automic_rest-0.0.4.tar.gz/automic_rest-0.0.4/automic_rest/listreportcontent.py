...
class listReportContent:
   def __init__(self, client_id:int=0, run_id:int=0, report_type=None, query=None):
       ...
       self.text = None 
       self.status = None 
       self.path = config().setArgs('/{client_id}/executions/{run_id}/reports/{report_type}', locals())
       if query != None:
            self.query = '?'+query
       else:
            self.query = ''
