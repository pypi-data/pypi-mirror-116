...
class Client(object):
    ...
    def get_all_packages(self, retry_limit=0):
    ...
    def create_cloud_routed_network(self, name, ros_distro, shared, parameters=None):
        ...
        """
        if ros_distro not in list(ROSDistro.__members__.values()) or not isinstance(ros_distro, ROSDistro):
    ...
    def query_metrics(self, query_metrics_request):
        ...
        if not query_metrics_request.tags.get(query_metrics_request.ORGANIZATION_ID_TAG):
            user_details = self._core_api_client.get_user()
            organization_guid = user_details.get('organization', {}).get('guid')
