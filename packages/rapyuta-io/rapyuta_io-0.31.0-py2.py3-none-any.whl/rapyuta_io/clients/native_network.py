...
class Parameters(ObjBase):
    ...
    def validate(limits, device, network_interface, restart_policy):
        ...
        if restart_policy is not None and (restart_policy not in list(rapyuta_io.clients.package.RestartPolicy.__members__.values()) \
                or not isinstance(restart_policy, rapyuta_io.clients.package.RestartPolicy)):
