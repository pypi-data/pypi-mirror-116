...
class Build(PartialMixin, ObjDict):
    ...
    def __init__(self, buildName, strategyType, repository, architecture, rosDistro='',
        ...
        if not isinstance(strategyType, StrategyType):
            strategyType = StrategyType(strategyType)

        if not isinstance(architecture, DeviceArch):
            architecture = DeviceArch(architecture)

        if rosDistro and not isinstance(rosDistro, ROSDistro):
            rosDistro = ROSDistro(rosDistro)
