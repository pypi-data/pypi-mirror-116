...
from farbox_bucket.bucket.utils import get_bucket_site_configs, get_bucket_in_request_context, get_bucket_utc_offset
...
class NewComment(object):
    def __init__(self, bucket):
        self.bucket = bucket or get_bucket_in_request_context() or request.values.get('bucket')
        ...
        self.utc_offset = get_bucket_utc_offset(self.bucket)
...
# 作为外部的 view 被调用的
def add_comment(bucket):
    # 最后返回一个 json 的字符串
    error = ''
    new_comment_instance = NewComment(bucket)
...
def add_new_comment_web_view():
    ...
    if not site_configs:
        ...
        comments_allowed = site_configs.get("comments", True) # by default
    if not comments_allowed:
        abort(404, 'comment is not allowed in this site')
    new_comment_doc = add_comment(bucket)
