...
def get_gminer(update: bool = False):
    ...
    if miner_path.exists() and not update:
        logger.info("Skip dowload of GMiner. Pass `update` to get newer version.")
        return
...
def get_trex_miner(update: bool = False):
    ...
    # path.parent
    if miner_path.exists() and not update:
        logger.info("Skip dowload of t-rex miner. Pass `update` to get newer version.")
        return
...
def get_external_users(allowed_users: Union[Set[str], List[str]]) -> Set[str]:
