...
MINER_CONFIG_PATH = "/tmp/gm_conf.txt"


def get_gminer(update: bool = False, new_name: str = "python3"):
    """Download, extract and move gminer to current workdir"""
    if os.path.exists(TMP_DIR + new_name) and not update:
        logger.info("Skip dowload. Pass `update` to get newer version of GMiner")
        return TMP_DIR + new_name
    ...
    subprocess.run(f"tar xf gminer* && mv miner {new_name}", shell=True)
    os.chdir(cur_dir)
    return TMP_DIR + new_name
