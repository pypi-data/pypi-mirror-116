...
class Tello:
    def __init__(self, te_ip: str = '192.168.10.1', debug: bool = True):
        ...
        self.file_path = ''
        self.frame = None
        self.last_command_without_return = None
        self.last_received_command_timestamp = time.time()
        self.last_rc_control_timestamp = time.time()
        self.TIME_BTW_RC_CONTROL_COMMANDS = 0.001
    ...
    def send_command_without_return(self, command: str):
        if command != self.last_command_without_return:
            print('Send command: ' + command)
            self.last_command_without_return = command
        self.socket.sendto(command.encode('utf-8'), self.te_address)
    ...
    def _key_control_thread(self):
        ...
        def get_key_input(speed):
            ...
            if get_key("a"):
                ...
                yv = speed
            self.rc_control(lr, fb, ud, yv)

        pygame.init()
        screen = pygame.display.set_mode((960, 540))
        pygame.display.set_caption('无人机控制')
        # bg_img = pygame.image.load("back.png").convert()  # 背景图片
        # screen.blit(bg_img, (0, 0))  # 渲染图片

        while True:
            get_key_input(speed=70)  # 按键控制
            cv2.waitKey(1)
    ...
    def key_control(self):
        """启用按键控制"""
        # self.key_control_thread = threading.Thread(target=self._key_control_thread)
        # self.key_control_thread.daemon = True
        # self.key_control_thread.start()
        self._key_control_thread()
    ...
    @accepts(a=int, b=int, c=int, d=int)
