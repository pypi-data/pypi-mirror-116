...
class VisionClassifierTrainer:
  ...
  def evaluate_f1_score(self):
    ...
    print(self.ids2labels.keys())
    print(self.ids2labels.values())
    print(set(all_target,all_preds))

    from sklearn import metrics
    table = metrics.classification_report(
      all_target,
      all_preds,
      target_names = self.ids2labels.keys(),
      labels = self.ids2labels.values(),
    )
    print(table)

    # # Get the labels as a list
    # labels = list(self.labels2ids.values())
    ...
    #   if precision[i] > 0:
    #     str_precision = str(round(precision[i] * 100,2)) + " %"
    #   else:
    #     str_precision = "0 %"

    #   if recall[i] > 0:
    #     str_recall = str(round(recall[i] * 100,2)) + " %"
    #   else:
    #     str_recall = "0 %"

    #   if fscore[i] > 0:
    #     str_fscore = str(round(fscore[i] * 100,2)) + " %"
    #   else:
    #     str_fscore = "0 %"
