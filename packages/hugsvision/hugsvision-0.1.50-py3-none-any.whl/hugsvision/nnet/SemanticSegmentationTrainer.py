...
class ObjectDetectionTrainer:
  ...
  def __splitDatasets(self):
    ...
    indices = numpy.random.randint(low=0, high=len(dataset), size=50)

    print("Load Datasets...")
    
    # Train Dataset in the COCO format
    self.train_dataset = torch.utils.data.Subset(dataset, indices[:40])

    # Dev Dataset in the COCO format
    self.val_dataset = torch.utils.data.Subset(dataset, indices[40:])
