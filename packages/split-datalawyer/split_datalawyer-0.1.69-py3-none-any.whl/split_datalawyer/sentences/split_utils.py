...
from split_datalawyer.utils.utils import contains_only_closing_parenthesis
...
def split_by_length(sentences: List[str], split_by_semicolon: bool, split_parenthesis_ending: bool, limit: int) \
        -> List[str]:
    results = []
    for sentence in sentences:
        if len(sentence.split()) > limit:
            if split_by_semicolon:
                ...
                results.extend(split_by_sentence(text=sentence, split_by_semicolon=True,
                                                 split_parenthesis_ending=False, limit=limit))
        else:
            results.append(sentence)
    return results
...
def split_by_sentence(text, split_by_semicolon=False, split_parenthesis_ending: bool = False, limit=500):
    if split_by_semicolon:
    ...
    if type(text) == str:
        ...
        sentences = split_by_length(sentences, split_by_semicolon, split_parenthesis_ending, limit)
        return sentences
        ...
        for subtext in text:
            ...
            result.extend(split_by_sentence(subtext, split_by_semicolon, split_parenthesis_ending, limit))
        return result


def split(text, split_by_semicolon=False, split_parenthesis_ending: bool = False):
    segments_by_break = split_by_break(text)
    return split_by_sentence(segments_by_break, split_by_semicolon, split_parenthesis_ending)
