...
class Executor(base_executor.BaseExecutor):
  ...
  def Do(self, input_dict: Dict[Text, List[types.Artifact]],
    ...
    ############## Yulong 20210719 start ##############
    # Set up InferenceService spec.

    # If the model didn't be blessed, 

    logging.info("***************** Yulong 20210708 v1.1.2 start.")
    print("***************** Yulong 20210708 v1.1.2 start.")
    ...
    print("--------------fs_config.base_directory:", base_directory)
...
#                           spec=V1beta1InferenceServiceSpec(predictor=predictor_create))
    ...
    if not self.CheckBlessing(input_dict):
      ...
      ############## Yulong 20210719 start ##############
      # Apply an InferenceService with last blessed model, if current model didn't be blessed and there is no existing InferenceService.

      try:
        kfs_client.get(name=pipeline_name, namespace=namespace)
      except RuntimeError as error:
        kfs_client.create(inferenceservice=isvc, namespace=namespace)
        print("No existing InferenceService")
        print("Error iofo about No existing InferenceService: ", error)
        logging.info("***************** The model didn't be blessed. Create an inferenceservice with last blessed model.")
        print("***************** The model didn't be blessed. Create an inferenceservice with last blessed model.")
    ...
    try:
      ...
      # kfs_client.replace(name=pipeline_name, inferenceservice=isvc, namespace=namespace)
      logging.info("***************** Delete an old inferenceservice.")
      print("***************** Delete an old inferenceservice.")
    ...
    print("***************** Create an inferenceservice.")
