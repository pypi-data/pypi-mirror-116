...
def ingest_data(  # NOSONAR
    ...
    if inference:
        ...
        # Create an Empty Dataframe with indexes covering the next 67 hours
        ...
        for station_id in hist_cons_df['trafo_id'].unique():
            data_set = hist_cons_df.loc[hist_cons_df.trafo == station_id]
        ...
        # Remove outliers
        for station_id in hist_cons_df['trafo_id'].unique():
            v = hist_cons_df.loc[df['trafo_id'] == station_id, 'value_wh']
...
def enrich(
    ...
    if inference:
        ...
        df.drop(['value_wh', 'trafo_id', 'datetime_utc'],
                axis=1,
                inplace=True)
