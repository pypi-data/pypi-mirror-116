...
from grid.sdk._gql.queries import (
    check_github_repo_accessible,
    get_user_credentials,
    repository_presigned_url,
    start_run_mutation,
)
...
def _generate_default_grid_config(
    ...
    submitted one.
    """
    _default_credential = get_user_credentials()

    provider = {
        'vendor': _default_credential['provider'],
        'credentials': _default_credential['credentialId'],
        # this is not currently enabled. we end up defaulting to the default value 100% of the time
        'region': _default_credential.get("region", "us-east-1"),
    }
