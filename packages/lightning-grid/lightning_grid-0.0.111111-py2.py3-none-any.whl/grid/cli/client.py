...
"""
    ...
    def validate_and_configure_datastore(grid_config: dict):
        ...
        if not datastore_id:
            raise click.ClickException(
                f'No ready-to-use datastore found with name {datastore_name} and version {datastore_version}'
            )

        if datastore_version and datastore_version != str(usable_version):
            raise click.ClickException(
                f"Unable to list datastores with name {datastore_name} and version {datastore_version}"
            )
