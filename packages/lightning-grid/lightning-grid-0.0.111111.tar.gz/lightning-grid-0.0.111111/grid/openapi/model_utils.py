...
from grid.openapi.exceptions import ApiAttributeError, ApiKeyError, ApiTypeError, ApiValueError
        ...
        if len_values == 0:
            ...
            raise ApiValueError(
                ...
                "the same".format(name,
                                  type(self).__name__), [e for e in [self._path_to_item, name] if e]
            )
...
def is_json_validation_enabled(schema_keyword, configuration=None):
    ...
    return (
        configuration is None or not hasattr(configuration, '_disabled_client_side_validations')
...
def check_validations(validations, input_variable_path, input_values, configuration=None):
    ...
    if (
        is_json_validation_enabled('multipleOf', configuration) and 'multiple_of' in current_validations
        and isinstance(input_values,
                       (int, float)) and not (float(input_values) / current_validations['multiple_of']).is_integer()
    ):
    ...
    if (
        is_json_validation_enabled('maxLength', configuration) and 'max_length' in current_validations
    ...
    if (
        is_json_validation_enabled('minLength', configuration) and 'min_length' in current_validations
    ...
    if (
        is_json_validation_enabled('maxItems', configuration) and 'max_items' in current_validations
    ...
    if (
        is_json_validation_enabled('minItems', configuration) and 'min_items' in current_validations
    ...
    if (
        is_json_validation_enabled('exclusiveMaximum', configuration) and 'exclusive_maximum' in current_validations
    ...
    if (
        is_json_validation_enabled('maximum', configuration) and 'inclusive_maximum' in current_validations
    ...
    if (
        is_json_validation_enabled('exclusiveMinimum', configuration) and 'exclusive_minimum' in current_validations
    ...
    if (
        is_json_validation_enabled('minimum', configuration) and 'inclusive_minimum' in current_validations
    ...
    if (
        is_json_validation_enabled('pattern', configuration) and 'regex' in current_validations
    ...
    try:
        if klass in {datetime, date}:
            ...
            if klass == datetime:
                ...
                date_only = (
                    parsed_datetime.hour == 0 and parsed_datetime.minute == 0 and parsed_datetime.second == 0
                    and parsed_datetime.tzinfo is None and 8 <= len(data) <= 10
                )
                if date_only:
