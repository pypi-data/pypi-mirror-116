...
class BotStatsUpdateJob(BaseBackgroundJob, ABC):
    ...
    def exec(self, data: BotStatsUpdateJobData):
        ...
        bot.messages_received = received_messages.get(data.bot_name, 0) if data.bot_name else received_messages.get(data.dedicated_bot_id, 0)
        bot.messages_filtered = filtered_messages.get(data.bot_name, 0) if data.bot_name else filtered_messages.get(data.dedicated_bot_id, 0)
