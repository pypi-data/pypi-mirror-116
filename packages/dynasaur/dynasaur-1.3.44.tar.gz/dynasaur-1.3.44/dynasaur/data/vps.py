        ...
        if args[1] == "time":
            ...
            with h5py.File(self._vps_file_path, 'r') as f:
                if args[0] == VPSDataConstant.SECFORC:
                    ...
                    # might be a help for debugging

                    # entids = np.genfromtxt(r"01_const_attributes_entid.txt", dtype=np.int)
                    # entid_translation = np.genfromtxt(r"03_const_identifiers_node_entid.txt", dtype=np.int)
                    # uids = np.genfromtxt(r"04_const_identifiers_node_uid.txt", dtype=np.int)
                    #
                    # import csv
                    # with open(r"02_const_attributes_title.txt", newline='') as f:
                    #     reader = csv.reader(f)
                    #     title = list(reader)
