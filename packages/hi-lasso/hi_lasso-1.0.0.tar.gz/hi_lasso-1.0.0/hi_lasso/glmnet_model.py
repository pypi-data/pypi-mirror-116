...
def AdaptiveLasso(X, y, logistic=False, sample_weight=None, weight_Adaptive=None):
    """
...
def ElasticNet(X, y, logistic=False, sample_weight=None):
    """
    ...
    if logistic:
        ...
        for i in np.arange(0, 1.1, 0.1):
            cv_enet = glmnet.ElasticNet(standardize=False, fit_intercept=False, n_splits=5, scoring='mean_squared_error',
                                        alpha=i).fit(X, y, sample_weight=sample_weight)
            cv_enet.fit(X, y, sample_weight=sample_weight)
            mses = np.append(mses, cv_enet.cv_mean_score_.max())
            cv_result_dict[f'cv_result_{i}'] = cv_enet
