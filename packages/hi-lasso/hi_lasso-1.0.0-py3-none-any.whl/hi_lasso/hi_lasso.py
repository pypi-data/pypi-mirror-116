...
class HiLasso:
    ...
    def __init__(self, q1='auto', q2='auto', L=30, logistic=False, random_state=None, parallel=False, n_jobs=None):
        self.q1 = q1
        self.q2 = q2
        self.L = L
    ...
    def fit(self, X, y, sample_weight=None, significance_level=0.05):
        ...
        self.p_values = self._compute_p_values(b2)
        self.coef_ = np.where(self.p_values < significance_level, b2_mean, 0)
    ...
    def _estimate_coef(self, bootstrap_number):
        ...
        rs = np.random.RandomState(bootstrap_number + self.random_state) if self.random_state else np.random.default_rng()
        ...
        if self.method == 'ElasticNet':
            coef = glmnet_model.ElasticNet(X_sc, y_sc, sample_weight=self.sample_weight, random_state=rs)
        else:
           coef = glmnet_model.AdaptiveLasso(X_sc, y_sc, sample_weight=self.sample_weight, random_state=rs,
