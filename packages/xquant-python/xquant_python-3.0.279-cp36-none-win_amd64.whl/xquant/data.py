...
def cpp_data_frame_to_3ddf__(tfs):
    ...
    for field in tfs:
        for time_stop in tfs[field].get_int_column("time_stop"):
            ...
            index_second.append(TimeUtil.ms_to_datetime(time_stop))
    ...
    def get_values(stf, name, typ):
        if typ == 2:
            ...
            return [TimeUtil.ms_to_datetime(x) for x in stf.get_int_column(name)]
        elif typ == 4:
...
class DataProxy(object):
    ...
    def get_bars(self, symbols, timespan, start_date=None, end_date=None, count=None, price_mode='pre', fields=None, skip_suspended=True):
        ...
        fields = check_columns(fields, [], GlobalUtil.bar_columns)
        op = BarOptions()
        ...
        if count is None:
            if isinstance(symbols, str):
                ...
                op.fields = GlobalUtil.bar_column
            ...
            if isinstance(symbols, str):
                op.fields = GlobalUtil.bar_column
                return pd.DataFrame(cpp_data_frame_to_df__(self.data_api.kbar.get_bar_by_count(TimeUtil.date_int_to_timestamp_ms(end_date) + (18 * 3600) * 1000, count, symbols, timespan, op)[symbols]))
            else:
                symbols = check_symbols(symbols)
                return cpp_data_frame_to_3ddf__(self.data_api.kbar.get_bars_by_count(TimeUtil.date_int_to_timestamp_ms(end_date) + (18 * 3600) * 1000, count, symbols, timespan, op))
