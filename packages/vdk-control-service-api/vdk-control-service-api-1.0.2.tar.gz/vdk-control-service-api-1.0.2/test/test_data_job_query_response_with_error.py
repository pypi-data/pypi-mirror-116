        ...
        if include_optional :
            return DataJobQueryResponseWithError(
                ...
                data = taurus_datajob_api.models.data_job_page.DataJobPage(
                    content = {"application/json":[{"jobName":"starshot-processing-vmc-fact-daily","config":{"team":"starshot","description":"Data Job responsible for transforming vmc realted fact tables on daily basis","schedule":{"scheduleCron":"5 0 * 8 *","nextRun":1618914371},"sourceUrl":"https://github.com/product-analytics/data-jobs/tree/master/starshot-processing-vmc-fact-daily","contacts":{"notifiedOnJobFailureUserError":"[starshot_ops@vmware.com]","notifiedOnJobFailurePlatformError":"[starshot_ops@vmware.com, auser@vmware.com]","notifiedOnJobSuccess":"[starshot_ops@vmware.com]","notifiedOnJobDeploy":"[starshot_ops@vmware.com, auser@vmware.com]"}}},"..."]}, 
                    total_items = 100, 
                    total_pages = 5, )
            )
        else :
