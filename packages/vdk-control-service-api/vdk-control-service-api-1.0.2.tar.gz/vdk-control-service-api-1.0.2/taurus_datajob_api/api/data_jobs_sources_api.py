...
class DataJobsSourcesApi(object):
    ...
    def sources_upload(self, team_name, job_name, body, **kwargs):  # noqa: E501
        ...
        <i>(Introduced in v1.0)</i> | Uploads Data Job source code and returns its version. Upload is persisted in source (git) repo so that all deployed code is versioned. The format of the payload is expected to be a zip in which the directory content can be found. <br>Example:<br> <br> If zip is <br> foo/file.txt<br> foo/nested_dir/file2.txt<br> <br>then data job dir uploaded will look like <br> job-name/file.txt<br> job-name/nested_dir/file2.txt<br>   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
