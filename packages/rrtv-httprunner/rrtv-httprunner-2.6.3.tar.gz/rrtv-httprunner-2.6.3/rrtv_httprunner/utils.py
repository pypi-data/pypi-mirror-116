...
def write_excel(content: Dict, target_dir):
    ...
    try:
        ...
        df = pd.DataFrame({"所属服务": [], "method": [], "url": [], "name": [], "界面位置": [], "90天是否有调用": [], "curl": []})  # 结构
    ...
    add_data = [service.replace("_", "-"), content["method"], content["url"], content["name"], content["location"], content["call"],
                content["curl"]]
