...
class NNTouchStone:
    ...
    def preprocess(self):
        if self.segment:
            import jieba
            self.X = [' '.join(jieba.lcut(e)) for e in self.X]

        self.X, self.Xt, self.y, self.yt = train_test_split(
            self.X, self.y, random_state=2021, test_size=self.validation_split)
        ...
        if self.max_len < 0:
            self.max_len = int(np.percentile(list(map(len, self.X)), 80))
    ...
    def embed(self, pretrained_vector: str = ''):
        ...
        cf.info('embed completes, size of embedding matrix {}. Missed {}, hit {}'.format(
            self.embedding_matrix.shape, missed, hit))
