...
class SourceLookup():
    ...
    def __init__(self, config):
        ...
        if zip_index is None:
            raise Exception('Expect to find names file weight_files.zip')

        self.pt_dirs=zip_index['coord']
        self.pt_names = zip_index['name']

        if config.catalog_fits is None or not Path(config.catalog_fits).expanduser().is_file():
            print('Expected to find the 4FGL catalog file: set "catalog_fits" in your config.yaml file',
                 file=sys.stderr)
