...
from ..cr.quadrature import get_quadrature_params, make_quadrature_spaces
...
class AssembleWithCRBlock(TapeBlock):
    ...
    def evaluate_tlm_matrix_component(
        ...
        mesh = compat.extract_mesh_from_form(self._form)
        ...
        for r_idx, drdq_array_r in zip(relevant_cr_outputs, drdq_arrays_relevant):
            ...
            for q_idx, drdq_array in zip(relevant_cr_inputs, drdq_array_r):
                ...
                Telem = backend.TensorElement(
                    "Quadrature",
                    mesh.ufl_cell(),
                    shape=tlm_shape,
                    degree=self.quad_params["quadrature_degree"],
                    quad_scheme=self.quad_params["quadrature_rule"],
                )
                T = backend.FunctionSpace(mesh, Telem)
