...
from crownstone_uart.core.uart.UartTypes import UartRxType, UartMessageType, UartTxType
...
class UartParser:
    ...
    def parse(self, wrapperPacket: UartWrapperPacket):
        ...
        if msgType == UartMessageType.UART_MESSAGE:
            ...
            # uartMsg = UartMessagePacket()
            # encryptionHeader = wrapperPacket.payload[0:4]
            # validation = wrapperPacket.payload[4:(4 + 4)]
            # uartMsgSize = Conversion.uint8_array_to_uint16(wrapperPacket.payload[(4 + 4):(4 + 4 + 2)])
            #
            # if uartMsg.parse(wrapperPacket.payload[(4+4+2):(4+4+2+uartMsgSize)]):
            #     _LOGGER.debug(f"Received enrypted msg: opCode={uartMsg.opCode} payload={uartMsg.payload}")
            #     UartEventBus.emit(SystemTopics.uartNewMessage, uartMsg)
            return
        else:
    ...
    def handleUartMessage(self, messagePacket: UartMessagePacket):
        ...
        ################
            ...
            _LOGGER.debug(f"Received HUB_DATA: {messagePacket.payload}")
            replyPacket = [0, 0]
            replyPacket.extend(messagePacket.payload)
            uartMessage = UartMessagePacket(42, UartTxType.HUB_DATA_REPLY, replyPacket).getPacket()
