...
class UartReadBuffer:
    ...
    def add(self, byte):
        ...
        if byte is START_TOKEN:
            if self.active:
                ...
                _LOGGER.info(f"Multiple start tokens: sizeToRead={self.sizeToRead} bufLen={len(self.buffer)} buffer={self.buffer}")
    ...
    def process(self):
        ...
        if calculatedCrc != sourceCrc:
            ...
            _LOGGER.info(f"Failed CRC: sourceCrc={sourceCrc} calculatedCrc={calculatedCrc} bufSize={len(self.buffer)} buffer={self.buffer}")
