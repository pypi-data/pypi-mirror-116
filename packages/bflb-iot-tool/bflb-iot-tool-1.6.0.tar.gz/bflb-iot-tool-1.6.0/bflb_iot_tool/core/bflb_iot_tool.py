...
#             bflb_utils.printf(e)
    ...
    def bl60x_mfg_ota_bin_gen(self, chipname="bl60x", chiptype="bl60x", cpu_type=None, version=None, dir_ota=None):
        fw_header_len = 4096
        fw_ota_bin = bytearray()
        if dir_ota: 
            ota_path = dir_ota 
...
def iot_download_cmd(args):
    ...
    if chiptype == "bl702":
        ...
        config["input_path"]["boot2_bin_input"] = os.path.join(chip_path, chipname, "builtin_imgs", 'boot2_iap_v3.0', "boot2_iap_release.bin")
        if not args.xtal: 
