...
class ContainerEntry(NamedTuple):
    ...
    async def run_finalize(self, instance: Any) -> None:
    ...
    async def run_cleanup(self, instance: Any) -> None:
        """Run the finalize and cleanup functions on an instance of the service."""
        await self.run_finalize(instance)
...
class Container:
    ...
    async def finalize(self, key: str) -> "Container":
        ...
        if key in self.services:
            ...
            entry = self.container_definition.get(key)
            await entry.run_finalize(instance)
        return self

    async def finalize_all(self) -> "Container":
        """Iterate through all services that have been created, and call the associated finalize
        methods for that service, if there is one.
        """
        for key in self.services:
            await self.finalize(key)
        return self

    async def cleanup(self, key: str) -> "Container":
        """Finalize and cleanup a single service if it has been booted and it has a finalize or
        cleanup function registered.
        """
        if key in self.services:
            instance = self.services[key]
            entry = self.container_definition.get(key)
            await entry.run_cleanup(instance)
        return self

    async def cleanup_all(self) -> "Container":
        """Iterate through all services that have been created, and call the associated finalize
        and cleanup methods for that service, if there are any.
        """
        for key in self.services:
            await self.cleanup(key)
        return self
