    ...
    def get_filelist(self, metadata_filter=None, get_annotation=False, file_path=None):
        ...
        if file_path == "absolute":
            ...
            if self.download_dir is None:
                raise Exception("Local download dir not found")
            gcs_path = self.download_dir
        elif file_path is not None:
            raise Exception("filepath value is either 'absolute' or 'local' or None")

        annotation_stat = self.get_annotation_stats()
        annotation_dict = dict(map(lambda t: (t[1], t[0]), enumerate(annotation_stat.keys())))
