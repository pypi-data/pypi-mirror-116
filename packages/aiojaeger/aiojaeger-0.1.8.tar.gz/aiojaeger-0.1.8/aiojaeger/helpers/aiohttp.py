...
def _set_span_properties(span: SpanAbc, request: Request) -> None:
    ...
    if resource and getattr(resource, "canonical", None) is not None:
...
def make_trace_config(tracer: Tracer) -> aiohttp.TraceConfig:
    ...
    tc.on_request_start.append(jaeger.on_request_start)
    tc.on_request_end.append(jaeger.on_request_end)
    tc.on_request_exception.append(jaeger.on_request_exception)
    return tc
