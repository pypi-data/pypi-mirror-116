...
class TolokaProjectHandler:
    ...
    def __init__(self, ouath_token, project_id=None, sandbox=True, verbose=True, project_params_data=None):
    ...
    def create_toloka_project(self, project_params_data=json_data['validating_segmentation']):
        ...
        :param project_params_data: json data in a dictionary format
        :return:
        '''
        req = requests.post(self.url + 'projects', headers=self.headers, json=project_params_data)
        assert req.ok
    ...
    def update_toloka_project(self, project_params_data=None):
        ...
        if project_params_data is not None:
            req = requests.put(self.url + f'projects/{self.project_id}', headers=self.headers, json=project_params_data)
            if req.ok:
    ...
    def get_files_from_pool(self, pool_id, download_folder_name):
        req = requests.get(self.url + f'attachments?pool_id={pool_id}&limit=100', headers=self.headers)
        if req.ok:
            if self.verbose:
                print(req)
                self.print_json(req.json())
            file_ids = (item['id'] for item in req.json()['items'])
            file_names = [item['name'] for item in req.json()['items']]
    ...
    @staticmethod
    def write_config_to_json_files(self, config_data, file_name: str):
...
if __name__ == '__main__':
    ...
    handler.get_files_from_pool(960087, 'photos')
    # handler.write_config_to_json_files({"key": "value"}, file_name='example')
    # handler.get_answers(960087)
