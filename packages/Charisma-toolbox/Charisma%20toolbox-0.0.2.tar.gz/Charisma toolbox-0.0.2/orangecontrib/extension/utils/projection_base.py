...
__all__ = ["LinearCombinationSql", "ProjectorExt", "Projection", "SklProjectorExt",
           "LinearProjectorExt", "DomainProjectionExt"]
...
class ProjectorExt(ReprableWithPreprocessors):
    ...
    def fit(self, X, Y=None):
        raise NotImplementedError(
            "Classes derived from ProjectorExt must overload method fit")

    def __call__(self, data, testdata):
        data = self.preprocess(data)
        testdata = self.preprocess(testdata)
        self.domain = data.domain
        clf = self.fit(data.X, data.Y, testdata.X, testdata.Y)
    ...
    # ProjectorExts implemented using `fit` access the `domain` through the
...
class ComputeValueProjectorExt(SharedComputeValue):
...
class DomainProjectionExt(Projection):
    var_prefix = "C"
    ...
    def __init__(self, proj, domain, n_components):
        ...
        def proj_variable(i, name):
            v = Orange.data.ContinuousVariable(
                name, compute_value=ComputeValueProjectorExt(self, i, transformer)
            )
...
class LinearProjectorExt(ProjectorExt):
    name = "Linear Projection"
    supports_sparse = False
    preprocessors = [RemoveNaNRows()]
    projection = DomainProjectionExt
...
class SklProjectorExt(ProjectorExt, metaclass=WrapperMeta):
    __wraps__ = None
    _params = {}
    name = 'skl projection'
    supports_sparse = False
