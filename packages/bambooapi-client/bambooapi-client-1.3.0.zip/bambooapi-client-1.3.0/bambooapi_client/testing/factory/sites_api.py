...
def forecast_factory_from_datapoints(
    ...
    Examples
    --------
    >>> from bambooapi_client.testing.factory.models import DataPointFactory
    ...
    ['availability', 'humidity', 'irradiance', 'mode', 'power', 'powersp', 'quality', 'schedule', 'soc', 'status', 'temperature', 'tempsp', 'vref']
...
def _dataframe_factory(df: pd.DataFrame) -> pd.DataFrame:
    df = df.filter(DataPoint.attribute_map.keys(), axis=1)
    datapoints = [
...
def _datapoints_list_to_dataframe(
    ...
    """Transform a list of DataPoint instances into a pd.DataFrame."""
