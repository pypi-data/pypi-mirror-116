    ...
    while still_querying:
        ...
        if position >= total_results:
            if result.get("processed_segments") != result.get("total_segments"):
                # XXX TODO re-enter the query loop and wait some more? Just one more time?
                # This can happen when CBC is bogged down.. however, it also may be the process hasn't terminated
                # as sometimes CBC will not return all segments at a given time.
                LOGGER.warning(
