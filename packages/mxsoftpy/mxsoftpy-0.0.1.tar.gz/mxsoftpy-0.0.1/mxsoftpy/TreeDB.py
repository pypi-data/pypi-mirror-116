...
from mx.exception import DBError
...
class TreeDB(object):
    ...
    def exec_bs(self, operate: str, *args: Any, **kwargs: Any) -> Union[int, tuple, None]:
        """
        用于执行bs开头的函数
    ...
    def insert_items(self, items: List[tuple], overwrite=True) -> None:
        """
        批量插入数据
        ...
        for item in items:
            value_type = self._check_item(item)
            self.exec_bs('bs_treedb_insert_property', item[0], item[1], value_type, overwrite)

    def insert_key_items(self, items: List[tuple], key: str = None, overwrite: bool = True) -> int:
        """
        批量插入子键及k,v
        ...
        for item in items:
            value_type = self._check_item(item)
            items_list.append({'name': item[0], 'value': item[1], 'valuetype': value_type})
