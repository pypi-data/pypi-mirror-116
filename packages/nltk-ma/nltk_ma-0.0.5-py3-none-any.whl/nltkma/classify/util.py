...
import nltkma.classify.util  # for accuracy & log_likelihood
...
class CutoffChecker:
    ...
    def check(self, classifier, train_toks):
        ...
        new_ll = nltkma.classify.util.log_likelihood(classifier, train_toks)
        ...
        if "max_acc" in cutoffs or "min_accdelta" in cutoffs:
            new_acc = nltkma.classify.util.log_likelihood(classifier, train_toks)
...
def names_demo(trainer, features=names_demo_features):
    from nltkma.corpus import names
    import random
...
def wsd_demo(trainer, word, features, n=1000):
    from nltkma.corpus import senseval
    import random
