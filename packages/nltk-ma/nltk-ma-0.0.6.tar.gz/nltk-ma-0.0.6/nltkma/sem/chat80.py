...
def _str2records(filename, rel):
    ...
    """
    recs = []
    contents = nltkma.data.load("corpora/chat80/%s" % filename, format="text")
...
def make_valuation(concepts, read=False, lexicon=False):
    ...
    if lexicon:
        read = True
    if read:
        from nltkma.sem import Valuation
