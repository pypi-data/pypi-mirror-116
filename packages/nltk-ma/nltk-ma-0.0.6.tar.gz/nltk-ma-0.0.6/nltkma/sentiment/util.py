...
import nltkma
from nltkma.corpus import CategorizedPlaintextCorpusReader
from nltkma.data import load
from nltkma.tokenize.casual import EMOTICON_RE
...
def extract_bigram_feats(document, bigrams):
    ...
    features = {}
    for bigr in bigrams:
        features["contains({0} - {1})".format(bigr[0], bigr[1])] = bigr in nltkma.bigrams(
            document
        )
    return features
...
def demo_tweets(trainer, n_instances=None, output=None):
    ...
    """
    from nltkma.tokenize import TweetTokenizer
    from nltkma.sentiment import SentimentAnalyzer
    from nltkma.corpus import twitter_samples, stopwords
...
def demo_movie_reviews(trainer, n_instances=None, output=None):
    ...
    :param output: the output file where results have to be reported.
    """
    from nltkma.corpus import movie_reviews
    from nltkma.sentiment import SentimentAnalyzer
...
def demo_subjectivity(trainer, save_analyzer=False, n_instances=None, output=None):
    ...
    """
    from nltkma.sentiment import SentimentAnalyzer
    from nltkma.corpus import subjectivity
...
def demo_sent_subjectivity(text):
    ...
    """
    from nltkma.classify import NaiveBayesClassifier
    from nltkma.tokenize import regexp
...
def demo_liu_hu_lexicon(sentence, plot=False):
    ...
    """
    from nltkma.corpus import opinion_lexicon
    from nltkma.tokenize import treebank
...
def demo_vader_instance(text):
    ...
    """
    from nltkma.sentiment import SentimentIntensityAnalyzer
...
if __name__ == "__main__":
    from nltkma.classify import NaiveBayesClassifier, MaxentClassifier
    from nltkma.classify.scikitlearn import SklearnClassifier
    from sklearn.svm import LinearSVC
    from nltkma.twitter.common import _outf_writer, extract_fields
