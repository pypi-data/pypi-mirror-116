...
class DealContract:
    ...
    def __init__(self, reserve_contract, credix_wallet, principal=1_000_000, time_to_maturity=12, financing_fee=0.15, underwriter_fee=0.2, leverage_ratio=4):
        self.reserve_contract = reserve_contract
        self.credix_wallet = credix_wallet
        self.principal = principal
        self.time_to_maturity = time_to_maturity # in months
    ...
    def repay_interest(self, interest_USDC_amount):
        print("-------------")
        print(f"repaying interest of {interest_USDC_amount} USDC")

        # pay out credix first
        credix_interest = interest_USDC_amount * GlobalSettings.BRIDGE_FINANCE_FEE
        self.credix_wallet.USDC_balance += credix_interest
        print(f"payout credix interest: {credix_interest} USDC")

        # pay out investors
        investor_interest = self.financing_fee * self.senior_tranche_max() * (1 - GlobalSettings.BRIDGE_FINANCE_FEE - self.underwriter_fee) / self.time_to_maturity
        if not interest_USDC_amount - investor_interest > 0:
            investor_interest = interest_USDC_amount
    ...
    def repay_principal(self, principal_USDC_amount):
        ...
        payout_investors = self.senior_tranche_max() + self.expected_yield_investors() - self.USDC_payout_investors
