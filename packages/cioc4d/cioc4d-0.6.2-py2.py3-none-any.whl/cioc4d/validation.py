...
class ValidateAbsoluteWindowsPaths(Validator):
    def run(self, _):
        ...
        if bad_assets:
            msg = "Some assets have absolute paths. These files may not be found correctly on Conductor's Linux render nodes.\n"
            msg += "For best result, use the Project Asset Inspector to localize all asset paths."
            c4d.WriteConsole("-" * 30)
            c4d.WriteConsole("\n")
            c4d.WriteConsole(msg)
            c4d.WriteConsole("\n")
            for asset in bad_assets:
                ob = asset["owner"]
                c4d.WriteConsole("{} -->> {}\n".format(ob.GetName(), asset["assetname"]))
            c4d.WriteConsole("\n")

            msg += "See the console for a list of files that could cause issues."
            self.add_warning(msg)

        for out_path in utils.get_image_paths():
            msg = f"The output image path '{out_path}' is absolute and the render is likely to fail. You'll have more success if you convert output paths to relative paths as shown here https://youtu.be/Rk8rcviGgrs"
            if Path(out_path).absolute:
                self.add_warning(msg)
