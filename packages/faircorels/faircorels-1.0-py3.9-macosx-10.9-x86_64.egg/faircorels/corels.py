...
class FairCorelsClassifier:
    ...
    geomRReason : double optional (default=1.5)
     ...
     klBound : double optional (default=0.1)
        max acceptable kl divergence
    ...
    def __init__(self, c=0.01, n_iter=10000, map_type="prefix", policy="lower_bound",
                 ...
                 mode=4, useUnfairnessLB=False, epsilon=0.0, klBound=0.1, kbest=1, forbidSensAttr=False,
        ...
        if fr:
            ...
            try:
                if time_limit is None: 
                    while fit_wrap_loop(self.n_iter, self.beta, self.fairness, self.mode, self.useUnfairnessLB, self.epsilon, self.klBound, self.kbest, performRestarts, initNBNodes, geomRReason):
                        pass
                else:
                    import time
                    start = time.clock()
