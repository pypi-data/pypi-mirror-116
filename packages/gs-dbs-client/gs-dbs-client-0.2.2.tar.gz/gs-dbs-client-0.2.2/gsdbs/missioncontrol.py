...
class MissionControl(Thread):
        ...
        if message.startswith("a"):
            if "{" in message:
                try:
                    ...
                    self.execute(self._gsdbs, mssgbdy)
                    self.markJobAsDone(mssgbdy["jobid"])
                except Exception as e:
                    print("JobFailed")
    ...
    def getQueue(self):
        ...
        try:
            ...
            userinfo = resp.json()
            return userinfo["mandant"]["mandantName"] + "/" + self.cnode
        except:
            return ""
    ...
    def markJobAsDone(self, jobid):
        ...
        requests.post("http://" + self._host + ":" + str(self._port) + "/missioncontrol/job",
                          json={"jobid": jobid, "cnode": self.cnode},
