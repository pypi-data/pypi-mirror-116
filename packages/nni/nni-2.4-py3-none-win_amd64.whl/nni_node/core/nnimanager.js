...
class NNIManager {
        ...
        if (this.trainingService === undefined) {
            ...
            switch (key) {
                case 'kubeflow_config': {
                    const kubeflowModule = await Promise.resolve().then(() => require('../training_service/kubernetes/kubeflow/kubeflowTrainingService'));
                    this.trainingService = new kubeflowModule.KubeflowTrainingService();
                    break;
                }
                case 'frameworkcontroller_config': {
                    const fcModule = await Promise.resolve().then(() => require('../training_service/kubernetes/frameworkcontroller/frameworkcontrollerTrainingService'));
                    this.trainingService = new fcModule.FrameworkControllerTrainingService();
                    break;
                }
                case 'adl_config': {
                    const adlModule = await Promise.resolve().then(() => require('../training_service/kubernetes/adl/adlTrainingService'));
    ...
    async initTrainingService(config) {
        ...
        if (reuseMode) {
            const module_ = await Promise.resolve().then(() => require('../training_service/reusable/routerTrainingService'));
            return await module_.RouterTrainingService.construct(config);
        }
        else if (platform === 'local') {
            const module_ = await Promise.resolve().then(() => require('../training_service/local/localTrainingService'));
            return new module_.LocalTrainingService(config);
        }
        else if (platform === 'kubeflow') {
            const module_ = await Promise.resolve().then(() => require('../training_service/kubernetes/kubeflow/kubeflowTrainingService'));
            return new module_.KubeflowTrainingService();
        }
        else if (platform === 'frameworkcontroller') {
            const module_ = await Promise.resolve().then(() => require('../training_service/kubernetes/frameworkcontroller/frameworkcontrollerTrainingService'));
            return new module_.FrameworkControllerTrainingService();
        }
        else if (platform === 'adl') {
            const module_ = await Promise.resolve().then(() => require('../training_service/kubernetes/adl/adlTrainingService'));
