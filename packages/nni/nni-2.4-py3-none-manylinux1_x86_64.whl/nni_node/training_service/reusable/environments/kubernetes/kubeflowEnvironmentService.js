...
const kubeflowApiClient_1 = require("../../../kubernetes/kubeflow/kubeflowApiClient");
const kubeflowConfig_1 = require("../../../kubernetes/kubeflow/kubeflowConfig");
const kubernetesConfig_1 = require("../../../kubernetes/kubernetesConfig");
let KubeflowEnvironmentService = class KubeflowEnvironmentService extends kubernetesEnvironmentService_1.KubernetesEnvironmentService {
    ...
    constructor(config, info) {
        ...
        this.config = experimentConfig_1.flattenConfig(config, 'kubeflow');
    ...
    async startEnvironment(environment) {
        ...
        const kubeflowJobName = `nni-exp-${this.experimentId}-env-${environment.id}`.toLowerCase();
    ...
    async prepareKubeflowConfig(envId, kubeflowJobName) {
        ...
        if (this.config.worker !== undefined) {
            workerPodResources.requests = this.generatePodResource(this.config.worker.memorySize, this.config.worker.cpuNumber, this.config.worker.gpuNumber);
        }
        ...
        if (this.config.operator === 'tf-operator') {
            if (this.config.ps !== undefined) {
                nonWorkerResources.requests = this.generatePodResource(this.config.ps.memorySize, this.config.ps.cpuNumber, this.config.ps.gpuNumber);
            ...
            if (this.config.master !== undefined) {
                nonWorkerResources.requests = this.generatePodResource(this.config.master.memorySize, this.config.master.cpuNumber, this.config.master.gpuNumber);
                nonWorkerResources.limits = { ...nonWorkerResources.requests };
            }
        }
