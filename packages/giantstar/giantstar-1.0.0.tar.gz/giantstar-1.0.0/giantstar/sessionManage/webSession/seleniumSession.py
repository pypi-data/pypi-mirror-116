...
from giantstar.sessionManage.webSession.element import FindElement, GiantElement
from giantstar.sessionManage.webSession.selenoidController import SelenoidController
from giantstar.utils.logger import logger
from giantstar.utils.dataParser import JsonParser
from giantstar.utils.error import SelenoidError, NoSuchBrowser
from giantstar.utils.js import send_message
from giantstar.utils.system import is_windows, get_canonical_os_name
...
class WEBSession(BaseSession):
    ...
    def build_session_by_step(cls, session_name, step_content):
        if isinstance(step_content, str) and step_content.startswith('$'):  # 自定义函数登录
            ...
            for step in step_content.get("login_step"):
                ...
                eve_func = getattr(GiantElement, event)
    ...
    @classmethod
    def video_url(cls):
        from giantstar.drivers.compatorDriver import CompatWEBDriver
