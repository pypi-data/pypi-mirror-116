...
class OtherEvent:
    ...
    @classmethod
    def ele_not_exits(cls, ele: WebElement):
        if ele:
            assert False, '元素不存在断言失败'
        else:
            assert True
...
class GiantElement(_Select, Button, Iframe, Input, MouseEvent, WindowEvent, CompatElement, ElementEvent, OtherEvent):
    """元素对象处理 """
