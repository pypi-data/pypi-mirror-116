...
class HttpDriver(BaseDriver):
    ...
    @classmethod
    def run(cls, step_name, user, param, assert_content, extract=(), locust_session=None, unique_uuid=None, var_map=None):
        param = cls.analyze(param, var_map=var_map)
        url = param.get("url")
        ...
        assert_content = cls.analyze(assert_content, var_map=var_map) or []
        cls.check(assert_content=assert_content, response=response)
        extract = cls.analyze(extract, var_map=var_map)
        cls.extract(response, extract, var_map=var_map)
