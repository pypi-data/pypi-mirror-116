...
class ExperimentalFeatures:
    ...
    def embed_texts(
        ...
        if self.embeddings is None:
            raise Exception('No embeddings initialized.  Please run '
                            'initialize_embeddings first.')
        ...
        for text in tokenized_texts:
            word_embeddings = [self.embeddings[word] for word in text if word
                            in self.embeddings]
            if len(word_embeddings) == 0:
                output_embeddings.append(np.zeros([emb_length]))
            else:
                v = np.sum(np.asarray(word_embeddings).reshape(-1, emb_length),
                        axis=0)
    ...
    def upload_dataset_with_nlp_embeddings(
        ...
        for k, input_df in dataset.items():
            ...
            emb_df = pd.DataFrame(embeddings,
                                columns=[f'_emb_{text_field_to_index}_{i}'
                                        for i in
    ...
    def nlp_similarity_search(
        ...
        if drop_emb_cols:
            ...
            return pd.DataFrame(data, columns=info['columns']).drop(emb_cols,
                                                                    axis=1)
        else:
    ...
    def tabular_similarity_search(
        ...
        feature_dtype_hash = {x['column-name']:
                            x['data-type'] for x
                            in info['model_schema']['inputs']}
        ...
        # model's inputs.
        missing_features = \
            set(feature_dtype_hash.keys()) - set(feature_point.index)

        if missing_features:
            raise Exception(f'feature_point_to_match is missing: '
        ...
        unknown_excludes = \
            set(exclude_features_in_dist)-set(feature_dtype_hash.keys())

        if unknown_excludes:
            raise Exception(f'Exclude features include: '
        ...
        scalable_fields = [fname for fname
                        in features_in_dist if
                        self.DIST_METRIC_BY_DATA_TYPE[
                            feature_dtype_hash[fname]] ==
                        self.DIST_NUMERIC
                        and fname not in exclude_features_in_dist]

        if scalable_fields:
            q_items = [f'cast(stddev({self._fix_db_name(fname)}) as FLOAT)'
        ...
        def generate_dist_query():
            ...
            for feature_name in features_in_dist:
                ...
                if feature_name in exclude_features_in_dist:
                    continue
                if self.DIST_METRIC_BY_DATA_TYPE[feature_type] ==\
                        self.DIST_NUMERIC:
                    query_items.append(f'POWER(({self._fix_db_name(feature_name)}-{feature_point[feature_name]})/{scale_factors[feature_name]},2)')
                else:
                    query_items.append(f'CASE WHEN {self._fix_db_name(feature_name)}=\'{feature_point[feature_name]}\' THEN 0.0 ELSE 1.0 END')
        ...
        out_cols = [orig_fields[x] if x in orig_fields else x for x in out[0]['columns']]
    ...
    def run_nlp_feature_impact(
        ...
        for text in texts:
            afi.get_prediction(None, ' '.join(
                text))  # Full texts is base-prediction; no words ablated.

            for word in text:
                afi.get_prediction(word,
        ...
        return afi.requested_key, afi.prediction_keys,\
            [(impact, key, afi.word_counts[key]) for impact, key in
                afi.sorted_impact]
    ...
    class FeatureImpactWBatchedRetrieval:
        ...
        def __init__(
            ...
            self.dataset = api.get_dataset(
                project_id=project_id,
                dataset_id=dataset_id,
                max_rows=1000000 if num_texts is None
                else num_texts
            )
        ...
        def run_predictions(self, texts):
            if len(texts) > 0:
                ...
                prediction = self.api.run_model(project_id=self.project_id,
                                                model_id=self.model_id,
                                                df=pd.DataFrame(
                                                    {self.text_field_name: texts}))
        ...
        def flush_cache(self):
            ...
            for word, pred in zip(self.cache_word, preds):
                if word is None:
                    ...
                    self.total_word_impact[
                        word] += self.last_base_prediction - pred
