...
def get_device_list(config: MutableMapping[str, str], entry: str, out: bool) -> Tuple[List[str], int]:
    ...
    for idx, device in enumerate(sd.query_devices()):
        ...
        if idx == default_idx:
            ...
            default_name = name
    my_assert(len(devices) > 0, "No device found")  # TODO: improve
    my_assert(default is not None, "Default not found")
    if selected is None:
        logger.warning(f"Device from ({remove_non_ascii(config_device)}) config not in device list; switch to default")
        selected = default
