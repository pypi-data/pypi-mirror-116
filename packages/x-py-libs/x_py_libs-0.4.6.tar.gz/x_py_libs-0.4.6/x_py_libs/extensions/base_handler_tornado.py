...
class BaseTornadoHandler(tornado.web.RequestHandler):
    ...
    def get_decrypt_request_query_argument(self, decrypt=None):
    ...
    def write_sc_list_response(self, query, decrypt=None, rst=None, cnt=0):
        data = None
        if rst is None:
            data = self.get_decrypt_request_query_argument(decrypt=decrypt)
            # if data is not None:
            rst, cnt = query(data)
        data = {
            'list': rst,
