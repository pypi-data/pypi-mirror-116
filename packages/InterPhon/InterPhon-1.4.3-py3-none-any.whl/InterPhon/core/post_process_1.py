        ...
        if code_name == 'vasp':
            if sym_flag:
                ...
                for i, require in enumerate(sym.require_atom):
                    ...
                    _independent_displace = sym.independent_by_single_displacement_cart[i]
                    _additional_displace = sym.independent_additional_displacement_cart[i]
                    if _additional_displace:
                        _independent_displace.extend(_additional_displace)
                    ...
                    force_constant_cart = np.linalg.inv(np.transpose(independent_displace)) @ np.transpose(self.force_constant.copy()[:, 3 * require: 3 * (require + 1)])
                    self.force_constant[:, 3 * require: 3 * (require + 1)] = np.transpose(force_constant_cart)
