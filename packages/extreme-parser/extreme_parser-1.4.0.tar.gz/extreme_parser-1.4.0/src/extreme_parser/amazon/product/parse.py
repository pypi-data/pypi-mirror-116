...
def parse_price(p: Product, html: str = None, selector: parsel.Selector = None):
    price_str = selector.xpath("//span[@id='priceblock_ourprice']/text()").get()
    if price_str is None:
        p.price_min = None
        p.price_max = None
        return

    prices: Union[None, list] = parse_number(price_str, first=False)
...
def parse_in_stock(p: Product, html: str = None, selector: parsel.Selector = None):
    ...
    if in_stock == "In Stock.":
        p.in_stock = True
    elif in_stock in ["In stock soon.", "Currently unavailable."]:
        p.in_stock = False
    else:
        p.in_stock = None
