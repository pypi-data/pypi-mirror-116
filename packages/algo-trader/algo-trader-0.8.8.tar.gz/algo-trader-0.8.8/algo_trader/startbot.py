...
parser.add_argument(
    'run', help='You have to say algotrader to \'run\' a strategy.')
parser.add_argument(
    'strategy', help='Define which strategy to run')
parser.add_argument(
...
if args.run in ['run', 'Run', 'RUN'] and args.strategy in ['lotus', 'Lotus', 'LOTUS']:
    symbol = ''
    if args.symbol in ['BTCUSD', 'Bitcoin', 'bitcoin', 'XBTUSD', 'BTC']:
        symbol = 'XBTUSD'
    elif args.symbol in ['ETHUSD', 'Ehtereum', 'ETH']:
        symbol = 'ETHUSD'
    else:
        print('This symbol can\'t be traded with the Lotus strategy.')
        sys.exit()
    ...
    print("Sorry, your entries were not matched with a valid strategy, symbol and broker. You need to input e.g. 'algotrader run lotus btcusd bitmex_testnet' to run a strategy.")
