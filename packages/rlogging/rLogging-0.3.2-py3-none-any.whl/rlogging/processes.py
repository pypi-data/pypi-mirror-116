...
class SubProcessWorker(object):
    ...
    def _handle_logs_and_settings(self):
        print(_M.current_process())

        print(id(self.logQueue))

        print('Запуск воркера контроллера логов')
        while self.onWhile.value:
            self._get_settings()
            self.__empty_logs_queue()
            sleep(0.1)

        print('Воркер контроллера логов завершает работу')
        self.__empty_logs_queue()

        print(_M.current_process())
        print(id(self.logQueue))
        print(self.logQueue.qsize(), 'объектов в очереди')
        print('Воркер контроллера логов завершил работу')
...
class SubProcessController(object):
    ...
    def stop(self):
        ...
        print('Ручная смена значения параметра "self.onWhile"')
