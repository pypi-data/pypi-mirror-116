...
def controle_arguments(verificateur_arguments, conversion=False) :
...
def controle_temps_execution(temps_maximal=0.1, affichage=False) :
    ...
    def decorateur(fonction) :
        ...
        def fonction_chronométrée(*args, **kwargs) :
            ...
            if affichage :
                print("Temps d'execution : %rs.", t_execution)
            if t_execution > temps_maximal :
                raise Warning("Le temps d'exécution maximal (%r) est dépassé ! La fonction/méthode a mis %rs à s'exécuter.", temps_maximal, t_execution)
            return retour
