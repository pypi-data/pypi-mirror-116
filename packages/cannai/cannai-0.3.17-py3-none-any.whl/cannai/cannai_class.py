...
class Cannaibase:
    ...
    def __init__(self, classtype, dirct=None, save_model=True, binary_class = False):
        ...
        self.model_list = []  # list of [model_name,model_output,date,model]
...
#get model and model_name
    def add_model(self, model, model_n=None):
        ...
        sub_ml = self.add_model_base(model, model_n, ii, None)
...
class Cmodel(Cannaibase):
    ...
    def add_model_base(self, model, model_n, ii, target_label):
...
class multi_Cmodel(Cmodel):
    ...
    def save_model(self, model,target_label, model_name=None):
        ...
        sub_ml = self.add_model_base(model, model_name, ii, target_label)
        self.save_df(sub_ml[1], model_name)
        s_res.append(sub_ml)
        self.model_dict = s_res
        s_mn_dic.append(sub_ml[0])
        self.model_name_list.append(s_mn_dic)
