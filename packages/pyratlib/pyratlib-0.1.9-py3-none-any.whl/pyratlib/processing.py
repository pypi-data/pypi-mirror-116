...
def DrawLine(x, y, angle, **kwargs):
    ...
    r = 10 
    if type(ax) == type(None):
        return plt.arrow(x, y, r*np.cos(angle), r*np.sin(angle),width = arrow_width,head_width=head_width,fc = arrow_color)
    else:
        return ax.arrow(x, y, r*np.cos(angle), r*np.sin(angle),width = arrow_width,head_width=head_width,fc = arrow_color)
...
def HeadOrientation(data, step, head = "cervical", tail = "tailBase", **kwargs):
    ...
    if type(ax) == type(None):
        ...
        for i in range(0,len(tailY),step):
            rat.DrawLine(tailX[i], tailY[i], (rad[i]), ax = ax,arrow_color = arrow_color)
