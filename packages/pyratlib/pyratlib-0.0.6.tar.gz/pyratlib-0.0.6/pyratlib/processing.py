...
def MotionMetrics (data, bodyPart, filter=0,  fps=30):
    """
    ...
    filter : int
        Threshold to remove motion artifacts.   
    fps : int
    ...
    dataX = scaleConverter(dataX,dataX.max(),dataX.min(), 60,0)
    dataY = scaleConverter(dataY,dataY.max(),dataY.min(), 60,0)
