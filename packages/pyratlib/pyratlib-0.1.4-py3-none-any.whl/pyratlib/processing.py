...
def Reports(paths,bodypart,fields=None,filter=0.3,fps = 30):
    """
    ...
    ----------
    path : dict
        Dictionary with all data paths.
    bodypart : str
        Body part you want use as reference.
    fields : pandas DataFrame
        The DataFrame with the coordinates of the created fields (output of FieldDetermination()).   
    ...
    for item in paths:
        lista = [item]
        data = pd.read_csv(paths[item])

        DF = rat.MotionMetrics(data, bodypart, filter=filter, fps=fps)
        ...
        lista.append(vMedia)
        if type(fields) != type(None): 
            interacts = rat.Interaction(data, bodypart, fields)
            for i in range(len(fields)):
